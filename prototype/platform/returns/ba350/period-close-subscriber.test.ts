// platform/returns/ba350/period-close-subscriber.test.ts
//
// M3 Slice 5 — Unit tests for the AccountingPeriodClosed → BA 350
...
// =====================================================================
// 1. Per-entity guard — BA_350_SUBSCRIBER_ENTITIES
// =====================================================================

describe("BA350 period-close subscriber — per-entity guard", () => {
  it("BA_350_SUBSCRIBER_ENTITIES contains only LE-ZA-HOZ-BANK", () => {
    expect(BA_350_SUBSCRIBER_ENTITIES).toEqual(["LE-ZA-HOZ-BANK"]);
  });

  it("skips LE-ZA-HOZ-SECURITIES (not bank-licence-bound)", () => {
    const store = new EventStore(":memory:");
    const result = ba350PeriodCloseSubscriber({
    ...
    expect(result.skipReason).toContain("not in BA_350_SUBSCRIBER_ENTITIES");
  });
});
...
describe("BA350 period-close subscriber — zero-position baseline", () => {
  it("generates a BA 350 with zero capital for empty event store", () => {
    const store = new EventStore(":memory:");
    const result = ba350PeriodCloseSubscriber({
    ...
    expect(result.ba350Output).toBeDefined();
    expect(result.ba350Output.meta.form).toBe("BA 350");
    expect(result.ba350Output.meta.entity).toBe(ENTITY_BANK);

    // Zero positions ⇒ zero capital + zero RWA.
    expect(result.ba350Output.totalMarketRiskCapitalMinor).toBe(0);
    expect(result.ba350Output.totalMarketRiskRwaMinor).toBe(0);
  });
  ...
  it("all numeric cells are non-NaN", () => {
    ...
    expect(Number.isNaN(result.ba350Output.totalMarketRiskCapitalMinor)).toBe(false);
    expect(Number.isNaN(result.ba350Output.totalMarketRiskRwaMinor)).toBe(false);
    expect(Number.isNaN(result.ba350Output.fx.capitalMinor)).toBe(false);
    expect(Number.isNaN(result.ba350Output.interestRateGeneral.capitalMinor)).toBe(false);
    expect(Number.isNaN(result.ba350Output.interestRateSpecific.capitalMinor)).toBe(false);
    expect(Number.isNaN(result.ba350Output.equity.capitalMinor)).toBe(false);
    expect(Number.isNaN(result.ba350Output.commodity.capitalMinor)).toBe(false);
  });
  ...
  it("placeholders populated (rehearsal-grade marker)", () => {
    ...
    expect(result.ba350Output.placeholders.length).toBeGreaterThanOrEqual(1);
    expect(result.ba350Output.placeholders.some((p) => p.includes("[citation: TBC"))).toBe(true);
  });
});
...
describe("BA350 period-close subscriber — caller-supplied inputs", () => {
  it("IR general maturity ladder flows into output", () => {
    ...
    // |1000-400| = 600
    expect(result.ba350Output.interestRateGeneral.capitalMinor).toBe(600);
    expect(result.ba350Output.totalMarketRiskCapitalMinor).toBe(600);
  });
  ...
  it("equity rows flow into output (8% net + 4% gross for liquid/diversified)", () => {
    ...
    expect(result.ba350Output.equity.capitalMinor).toBe(16_000);
  });
  ...
  it("RWA = 12.5 × total capital across sub-charges", () => {
    ...
    expect(result.ba350Output.totalMarketRiskRwaMinor).toBe(12_500);
  });
});
...
describe("BA350 period-close subscriber — XML serialiser", () => {
  it("renders well-formed XML with declared SARB envelope", () => {
    ...
    expect(xml.includes("<BA350")).toBe(true);
    expect(xml.includes(`xmlns="${BA_350_NAMESPACE}"`)).toBe(true);
    expect(xml.includes(`xsdUri="${BA_350_XSD_URI}"`)).toBe(true);
    expect(xml.includes("</BA350>")).toBe(true);
  });
  ...
  it("structural validator passes for all required elements", () => {
    ...
    const validation = validateSarbXmlStructural({
      xml,
      formId: "BA350",
      namespaceUri: BA_350_NAMESPACE,
      requiredElements: [...BA_350_REQUIRED_ELEMENTS],
    });
  ...
  it("XML is byte-identical across runs (determinism)", () => {
    ...
    const makeXml = (): string => {
      ...
      return renderSarbXml(ba350ToXmlPayload(result.ba350Output), {
