...
import type {
  SuspenseChartEntry,
} from "../platform/accounting/suspense-report";
...
function makeTrialBalance(rows: Array<{ accountId: string; currency: string; amountMinor: number }>): TrialBalance {
  return {
    rows: rows.map((r) => ({ leafAccountId: r.accountId, currency: r.currency, amountMinor: r.amountMinor })),
    perCurrencyTotals: [],
...
describe("generateSuspenseReport", () => {
  it("returns ok:true when all suspense accounts have zero balance", () => {
    ...
    const result = generateSuspenseReport({ trialBalance: tb, chartOfAccounts: chart, asOf: AS_OF });
  ...
  it("returns ok:false when a suspense account has a non-zero balance", () => {
    const tb = makeTrialBalance([{ accountId: "ACC-1100-004", currency: "ZAR", amountMinor: 50_000 }]);
    const chart: SuspenseChartEntry[] = [SUSPENSE_ENTRY];
  ...
  it("flags aged balances when oldestPostingDates map is supplied", () => {
    const tb = makeTrialBalance([{ accountId: "ACC-1100-004", currency: "ZAR", amountMinor: 25_000 }]);
    ...
    const result = generateSuspenseReport({ trialBalance: tb, chartOfAccounts: chart, asOf: AS_OF, oldestPostingDates });
  ...
  it("uses default 3-day clearance horizon when clearanceHorizonDays is not set", () => {
    ...
    const tb = makeTrialBalance([{ accountId: "ACC-1100-006", currency: "ZAR", amountMinor: 5_000 }]);
    // 4 days old — exceeds the 3-day default
    const oldestPostingDates = new Map([["ACC-1100-006", "2026-05-27"]]);

    const result = generateSuspenseReport({ trialBalance: tb, chartOfAccounts: [entryWithoutHorizon], asOf: AS_OF, oldestPostingDates });
