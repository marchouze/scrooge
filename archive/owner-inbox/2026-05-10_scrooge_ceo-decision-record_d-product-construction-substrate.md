...
- `agent:Mira (Compliance / RegTech engineer)` — bind `[register: route to Mira]` items: confirm Banks Act Reg 39 sub-clauses bind on product approval; populate Domain N (M1 markets-foundation citation URNs) and Domain J (markets/trading) URN slugs; cross-link into `_obligations-register.md`. **[done in PR #112 — `Regulations/_obligations-register.md` v1.11; 3 Domain A Reg-39 rows added (`ORG-PR-24..26`); 6 Domain J URN-slug rows added (`ORG-MK-09..14`); URN-slug column populated for all 26 Domain N entries.]**
