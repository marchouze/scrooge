...
- **Typed validation events** — `ModelValidationApproved`, `ModelValidationWithheld`, `ValidationFindingRaised`, `ValidationFindingClosed`, `ModelTierClassified`, `BacktestBreachDisposed`, `ValidationMethodologyPublished` are not yet in `prototype/platform/event-store/event-types.ts` or `registry.ts`. Until they land, Nadia's outputs are markdown deliverables in the Owner Inbox; Atlas emits on Nadia's behalf in V1; Nadia wires direct in V2 once the agent-runtime substrate lands. Owner: Atlas (next typed-event slice). Target: alongside Wave-4 #14 (`AgentEscalation`) and Linnea's `BrandAssetPublished`.
