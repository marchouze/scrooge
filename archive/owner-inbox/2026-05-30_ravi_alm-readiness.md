...
asOf: 2026-05-30T05:37:29.377Z
decision-required: false
---

# Ravi — ALM readiness, 2026-05-30
...
**Headline:** 9 ALM pipelines tracked · readiness 0 ready / 1 drafting / 8 specified / 0 not-yet-specified · 7 Ravi-owned obligations indexed (0 PARTIAL / drafting) · 168 ALM-domain events (last 7d).

## Eitan's latest snapshot

Latest `LiquiditySnapshot` event: 2026-05-29T09:52:04.554Z
...
| `CollateralUpdated` | 0 |
| `FundingDrawnDown` | 0 |
| Prior `ALMReadinessSnapshot` (this agent) | 1 |
...
Headline: ALM substrate is **specified-not-wired** across the board — eight of nine pipelines have engine specs, but `HQLAObserved`, `FXPositionReported`, `CollateralUpdated`, and `FundingDrawnDown` are all flat at zero over the trailing seven days. The `LCRComputed` / `NSFRComputed` / `IRRBBChecked` counts (14 / 14 / 140) are degraded-mode synthetic cycles firing against placeholder inventory — they are doing their job as the daily SLA stand-in for Eitan's LiquiditySnapshot, but they are not yet a measured LCR / NSFR. The load-bearing block on Eitan's first live LCR / NSFR sign-off is **`alm:hqla-inventory`**: until the Banks Act Reg 26 eligibility / haircut table is specified and the inventory projection is wired against postable balance-sheet events, the LCR numerator stays synthetic and `alm:lcr-net-outflow` cannot promote off its placeholder. All seven Ravi-owned obligations (`ORG-PR-06/-07/-08/-11/-14/-15`, `ORG-MK-08`) are still at `[TBD]` pending the policy substrate Eitan + Helena are authoring — none of them are blocked on engineering today, but `ORG-PR-08` and `ORG-PR-15` (Funding Strategy) will become engineering-blocked the moment the FTP market-rate feeds are needed for a live curve, and `ORG-MK-08` (Excon + funding) is gated by the correspondent-bank connector below.

Two consequential downstream gaps behind HQLA. First, **`alm:ftp-attribution`** is the only pipeline already in `drafting`, and the binding constraint is the deferred ZARONIA / JIBAR / OIS / FX market-rate feed integration — the curve registry exists, but without ingested reference rates the first FTP cycle cannot run, which in turn means `ORG-PR-08` / `ORG-PR-15` cannot move from policy [TBD] to a measurable Funding Strategy attestation. Second, **`alm:samos-funding`** remains correspondent-mediated per our indirect-participant posture — `FundingDrawnDown` will stay at zero until Tomas and I draft the correspondent-bank SAMOS-mediation API contract, and that contract is also the hook BCBS 248 intraday monitoring will eventually attach to. `alm:fx-position` and `alm:collateral-inventory` are both one upstream event away from green (first FX-denominated event; first GMRA / repo execution) and are not on the critical path this week.

Next engineering move, in order: (1) specify the **HQLA-eligibility table per Banks Act 94 of 1990 Reg 26** — Level-1 / Level-2A / Level-2B with haircuts — and wire the inventory projection against the synthetic capital line so the first real `HQLAObserved` fires; this immediately unblocks a non-synthetic `LCRComputed` and lets `alm:nsfr-asf` / `alm:nsfr-rsf` (Reg 27 / BCBS d365-adjacent BCBS D335) follow on the same projection runtime. (2) Open the vendor-selection ticket for **ZARONIA + JIBAR + OIS curve ingestion** so FTP can exit `drafting` and `ORG-PR-08` / `-15` have an engine to bind to. (3) Draft the **correspondent-bank SAMOS-mediation API contract** with Tomas — explicitly framed as indirect-participant mediation, not direct SAMOS membership — so `FundingDrawnDown` and the Currency & Exchanges Manual A.4 reporting hook for `ORG-MK-08` have a wire to land on. IRRBB / BCBS d365 EVE-shock specification can wait one more cycle behind HQLA; the 140 synthetic `IRRBBChecked` events are adequate degraded-mode cover until the banking-book event stream is real.
