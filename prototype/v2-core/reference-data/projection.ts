// v2-core/reference-data/projection.ts
//
// Reference-data event-list projection — the canonical, store-independent view
// the Wave 2 batch-1 parity gate folds over BOTH stores (the authoritative V1
// event store and the v2 control-plane store).
//
// WHY AN EVENT-LIST PROJECTION (not a domain register):
...
//   fold logic to drift. The four migrated domains are heterogeneous reference
//   data with no shared register semantics, so a per-domain register would add
//   surface without adding fidelity.
//
 ...
 */
export interface ReferenceDataEventView {
...
export interface ReferenceDataRegister {
 ...
 * rather than throwing, mirroring the tolerant-fold convention of the posture
 ...
 */
export function foldReferenceDataRegister(
  views: Iterable<ReferenceDataEventView>,
  typeSet: ReadonlySet<string>,
): ReferenceDataRegister {
