...
//
// recon:bucket-c-batch3-v2-parity — Bucket C bulk batch-3 parity gate
// (D-BANK-WIDE-V2-MIGRATION).
//
// PURPOSE
// -------
// Proves byte-equivalence of the 29 (FINAL) non-load-bearing bucket-C substrate
// types (C-3 agent-lifecycle/runtime remainder incl SubstrateAlert +
// SubstrateAgentRun* + ScheduledTrigger; C-9 audit/readiness/regulator-process;
// C-10 agent-performance remainder; sourced from BUCKET_C_BATCH3_TYPES) between:
...
// recon:bucket-c-batch2-v2-parity (#1409); identical comparable shape + harness.
//
...
// the money-free batches, and bucket-C batches 1 + 2.
//
// STATUS
// ------
// ENFORCING (bucket-C batch-3 flip, 2026-06-17). All 29 types flipped
...
// where real runtime/audit/regulator populations live). Because the codec is
// verbatim (identity), the empty-store flip is safe — there is no codec to get
// wrong on real data; the wiring is what CI proves, the byte-equivalence is
...
import { BUCKET_C_BATCH3_TYPES } from "../../v2-core/bucket-c-batch3";
...
const PIPELINE = "bucket-c-batch3-v2-parity";

const TYPE_SET: ReadonlySet<string> = new Set<string>(BUCKET_C_BATCH3_TYPES);
...
function readV1Tuples(): readonly VerbatimTuple[] {
  ...
  for (const type of BUCKET_C_BATCH3_TYPES) {
...
export function run(overridePath?: string): ReconResult {
  ...
  const vacuous = runParityCheck<readonly VerbatimTuple[]>({
    label: "bucket-c-batch3-v2-parity:structural-check",
    readV1: () => [],
    readV2: () => [],
  });
  if (vacuous.length > 0) {
    violations.push({
      subject: "bucket-c-batch3-v2-parity:harness-structural-check",
      message:
  ...
  try {
    ...
    const parity = runParityCheck<readonly VerbatimTuple[]>({
      label: "bucket-c-batch3-verbatim-tuples",
      readV1: () => v1Snap,
      readV2: () => v2Snap,
    });
    ...
    violations.push({
      subject: "bucket-c-batch3-v2-parity:fold-error",
      message: `bucket-c batch-3 verbatim tuple fold threw: ${err instanceof Error ? err.message : String(err)}. Inspect the V1 store and the v2 control-plane store for malformed events.`,
      severity: "fail",
    });
  }
  ...
  result.asOf =
    `bucket-c-batch3-v2-parity [ENFORCING]: ${BUCKET_C_BATCH3_TYPES.length} type(s); ` +
