// platform/reporting/ba-325-xml-adapter.ts
//
// Thin adapter that maps a typed `Ba325Output` to the generic
// `SarbXmlReportPayload` consumed by `xml-render.ts`.
//
// D-REPORTING-CAPABILITY-M2-M3-BUILD-PLAN Slice 5 — BA 325 (LCR) XML adapter.
// Companion to `ba-350-xml-adapter.ts` (market risk) and
// `ba-600-xml-adapter.ts` (operational risk).
//
// Architectural placement:
//
//   BA 325 PROJECTION    (pure typed Ba325Output)
//      → ba325ToXmlPayload()   ← this module
...
import type { Ba325LineItem, Ba325Output } from "./ba-325-lcr";
import type { SarbXmlReportPayload, SarbXmlSection } from "./xml-render";

export const BA_325_XSD_URI = "https://hoz.bank/xsd/ba-325/v0.1-rehearsal.xsd"; // [citation: TBC]
export const BA_325_NAMESPACE = "https://hoz.bank/ns/ba-325/v0.1";

function lineItem(it: Ba325LineItem): SarbXmlSection {
  return {
    LineId: it.lineId,
...
/**
 * Map a typed `Ba325Output` (LCR return) to a `SarbXmlReportPayload`
 ...
 *   BCBS-LCR-2013.
 */
export function ba325ToXmlPayload(report: Ba325Output): SarbXmlReportPayload {
  ...
  return {
    formId: "BA325",
    formVersion: report.meta.formVersion,
    xsdUri: BA_325_XSD_URI,
    namespaceUri: BA_325_NAMESPACE,
    body,
  };
}

export const BA_325_REQUIRED_ELEMENTS: readonly string[] = [
  "Meta",
  "Hqla",
  "CashFlows",
  "LcrRatio",
