...
// across the six BCBS d368 shock scenarios (bonds folded; IRS DV01 converged
...
//
// which is exactly the register line's `measurementBinding` (BCBS d368
...
//   2026-06-08); RAS §B4; BCBS d368 §A-3.4; Banks Act Regulations 26 and 27;
...
export interface IrrbbDeltaEveMeasured {
  ...
  /** Number of EVE shock scenarios in the latest run (BCBS d368 set: 6). */
...
export function getIrrbbDeltaEveMetric(store: EventStore): IrrbbDeltaEveMetric {
  ...
  if (latestAsOf === null) {
    return {
      ...
      note: [
        ...
        "First run seeds the BCBS d368 §A-3.4 supervisory-outlier measurement.",
  ...
  return {
    ...
    note: [
      ...
      `(worst shock: ${worstShockLabel}; ${scenarioCount} BCBS d368 scenario(s); latest ALM run as-of ${latestAsOf}).`,
      `RAS §B4 thresholds: ${thresholdsPrinted}.`,
      "Source: Ravi (Treasury and ALM engineer, engineering)'s ALM run IRRBBChecked events of record (BCBS d368 §A-3.4).",
    ].join(" "),
  };
}
