...
(() => {
  ...
  async function load() {
    ...
    if (tilesEl) {
      ...
      const tiles = [
        SC.renderTile({ label: "Open Decisions", value: decisions, href: "/decision.html", status: decisions > 0 ? "warn" : "ok" }),
        SC.renderTile({ label: "Workstreams", value: workstreams, href: "/ops.html", status: "info" }),
        SC.renderTile({ label: "Open Findings", value: findings, href: "/audit.html", status: findings > 0 ? "warn" : "ok" }),
        SC.renderTile({ label: "Obligations Due", value: obligations, href: "/obligations.html", status: obligations > 0 ? "warn" : "ok" }),
      ];
      tilesEl.innerHTML = "";
      tiles.forEach(t => tilesEl.appendChild(t));
    }

    // ── Decisions for CEO ─────────────────────────────────────
    const decisionsSection = $("decisions-section");
    if (decisionsSection) {
      $("decisions-header").innerHTML = SC.renderSectionHeader("Decisions for CEO", { label: "All decisions →", href: "/decision.html" });
      ...
      if (openList.length === 0) {
        ...
        cardsEl.innerHTML = openList.slice(0, 6).map(d => `
    ...
    if (drHeader && drGrid) {
      ...
      if (reports.length === 0) {
        ...
        drGrid.innerHTML = reports.map(r => `
    ...
    if (wsHeader && wsTable) {
      wsHeader.innerHTML = SC.renderSectionHeader("In-Flight Workstreams", { label: "All →", href: "/ops.html" });
      ...
      SC.renderTable({
        ...
        onRowClick: (w) => SC.openModal({ title: w.title || w.name || "Workstream", body: SC.renderDefList(w) }),
    ...
    if (evHeader && evTable) {
      evHeader.innerHTML = SC.renderSectionHeader("Recent Events", { label: "Event store →", href: "/events.html" });
