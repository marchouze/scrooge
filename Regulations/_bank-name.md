...
| **Citation chain** | Principle 2 downward: D-BANK-NAME-SELECTION → bank-name register → brand substrate (CSS tokens, SVG wordmarks, HTML `<title>` tags, dashboard `state.bankName`) |
...
- **External presentations.** Board packs, regulator submissions, customer-facing pages, and marketing all read the name through the canonical chain. Manual assembly of name strings is forbidden by Principle 2 downward derivation; any presentation that hand-codes the name is a reportable finding.
