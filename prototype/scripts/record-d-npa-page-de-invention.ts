...
const base = {
  ...
  recommendation:
    "Remove all hand-maintained 'invented' static lists from the V2 NPA product-detail page (dashboard/products-detail.ts) and source every functionality claim from the real substrate, honestly flagging gaps. (1) Delete POSTING_RULE_INDEX and rebuild the lifecycle-event journal from the canonical POSTING_RULE_REGISTRY via findEntriesForEventType — real ruleId/IFRS-detail/stage/module, 'no posting rule' only when the registry truly has none. (2) Validate DIMENSION_METADATA.triggeredBy/.emits against the real event-type registry; render any unregistered event-type name as a flagged gap, not a silent claim. (3) Flip the 7 baseline product fixtures + the synthesiseProductFromProposal fallback from the legacy V1 entity identifier to the canonical LE-ZA-HOZ-BANK via a single exported CANONICAL_LEGAL_ENTITY_ID constant, and extend recon:no-legacy-le-identity to scan source trees (not only event payloads). (4) Add a recon gate asserting the page asserts no event type or posting rule that is not either real-in-registry or surfaced-as-a-gap. Defer a real event-sourced per-product timeline.",
  rationale:
    "The page is the bank's canonical product-functionality surface, but its 'Lifecycle event journal' was built from POSTING_RULE_INDEX — a hand-authored static map (including a row flagged DEPRECATED 2026-05-20) duplicating the real POSTING_RULE_REGISTRY that the adjacent 'Accounting treatment' section already reads correctly. Describing the same posting rules twice from two sources — one real, one invented — let the page show 'mapped' for rules that may not exist and made its only gap signal ('no posting rule') untrustworthy. Separately, the baseline product fixtures still hardcode the stale pre-Hoz-Bank V1 entity identifier; PR #1205's backfill rewrote event payloads only, and recon:no-legacy-le-identity scanned payload text only, so the fixtures — TS source compiled straight into the view — surfaced the stale id past both. Sourcing every claim from the real registry/event-type registry/desk register/model declarations (the pattern the lenses and accounting-treatment sections already follow), plus a recon gate that fails on any asserted-but-unbacked event type or rule, makes the page show actual/planned behaviour and prevents invented static lists regressing in. Aligns with the Engineering Charter (root-cause, source-don't-hardcode, no-silent-deferral) and Principle 1 (events/registries are truth; the page is a render).",
  sourceDocHashes: [],
  citations: [
