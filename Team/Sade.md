...
- **Mode:** Hybrid — continuous (event-triggered) for the AgentOps slice (every agent run is logged, every capability change is an event); scheduled for fleet-roster integrity, persona-coherence drift sweeps, and capability reviews.
