...
asOf: 2026-05-15T05:19:49.860Z
decision-required: false
---

# Tomas — daily payments-readiness, 2026-05-15
...
Payments substrate is build-phase: of nine capability lines, three are *drafting* (SWIFT MT/MX, ISO 20022 pacs/camt, plus the SAMOS cut-off encoding) and the remainder are *specified* but not deployed; zero live messages on any rail in the last 7 days (no PaymentInitiated, no PaymentSettled, no SAMOSWindowEntered, no SWIFTMessageProcessed), which is the expected signal pre-BIC and pre-sponsor-seam. The load-bearing block on Devon's first end-to-end resilience rehearsal is `capability:samos-rtl-gateway`: until a synthetic pacs.009 / pacs.002 round-trip clears against a rehearsal endpoint and lands inside the 06:00 / 16:00 SAST cut-off windows under the SAMOS operating-hours circular, there is no payment-leg for the three-way reconciler to bite on. The indirect-participant model (NPS Act 78 of 1998, § access-to-settlement; reinforced by Banks Act 94 of 1990 prudential framing) collapses both the SAMOS and BankservAfrica builds onto a single sponsor-bank API seam — meaning the binding artefact for rehearsal is a sponsor-bank contract shape, not direct SAMOS membership, and CLS sits behind the same pattern via a member-bank seam (deferred to first FX position).

Three observations rank above the rest. One: `capability:iso-20022-pacs-camt` is one ticket from green — envelopes are drafted but not yet pinned to the current scheme-published schema version, and since BankservAfrica, SAMOS, and SWIFT CBPR+ all run lock-step to scheme cadence, the pin is the unblock for every downstream generator. Two: `obligation:swift:csp` (SWIFT CSP 2024 control framework, annual attestation cycle) has no owning capability deployed — it activates the moment the BIC application lodges, and the BIC unblock sits inside `capability:swift-mt-mx` drafting; co-ownership with Senna is named but the attestation calendar is not yet wired to the cut-off engine. Three: the cash-leg recon gap — `capability:gl-ledger-reconciliation` is specified but the three-way contract (Kai trade-leg ↔ Tomas payment-leg ↔ Bea ledger-leg) is not yet written, and Bea cannot post a first cash leg without camt.053 / camt.054 as recon source-of-truth under PASA settlement-finality semantics.

Next engineering move, in order: (1) draft the synthetic SAMOS gateway against pacs.009 / pacs.002 with the 06:00 / 16:00 SAST window bound to the calendar engine, indirect-participant shape (sponsor-bank adapter); (2) pin the ISO 20022 envelope library to the current scheme-published version across pacs.008 / pacs.009 / pacs.002 / camt.052 / camt.053 / camt.054 — one ticket, unblocks three connectors; (3) commission the three-way trade-leg ↔ payment-leg ↔ ledger-leg reconciler against synthetic flows alongside the SAMOS draft, so Bea's first cash-leg posting has a recon spine before Saskia's pre-licence go-live readiness gate is exercised. Strate participant-onboarding specification stays on the 1 March 2027 Joint Notice 2 of 2024 trajectory and does not contend with the rehearsal critical path.
