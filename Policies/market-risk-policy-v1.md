...
- **PA regulatory returns (`ORG-PR-60`):** Camille files BA-325 (FRTB SA capital) and BA-326 (FRTB IMA capital, if applicable) as part of the BA-return suite, per the SARB reporting calendar. The BA-return filing is a `RegulatoryReturnFiled { returnType, period, amount }` event.
...
- `Procedures/by-policy/frtb-sa-capital-computation.md` — step-by-step SA computation procedure per BCBS FRTB SA rules.
- `Procedures/by-policy/backtesting-governance.md` — daily back-testing run procedure, zone transition escalation steps.
- `Procedures/by-policy/pla-test-governance.md` — monthly PLA test run procedure, remediation plan requirements.
- `Procedures/by-policy/market-risk-limit-monitoring.md` — limit tables (numerical values), daily monitoring checklist, breach escalation steps.
