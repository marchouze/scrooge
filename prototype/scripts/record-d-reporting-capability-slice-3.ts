...
// the BA 325 (LCR) end-to-end harness, the first SARB return rendered.
//
...
const ENTRY = {
  ...
  title:
    "Reporting capability Slice 3 — BA 325 LCR end-to-end (semantic + projection + JSON render + CLI)",
  outcome:
    "Bea (Accounting & financial reporting engineer, engineering — reports to Camille CFO; BA-form line mapping owner) + Eitan (Treasurer, governance — reports to Camille CFO; LCR methodology owner) + Anya (Data / analytics engineer, engineering — reports to Devon COO; semantic-layer integration) ship the first SARB return end-to-end. Six new liquidity-classification semantic entries (HqlaLevel1, HqlaLevel2A, HqlaLevel2B, LcrCashOutflows30D, LcrCashInflows30D, LiquidityCoverageRatio) extend the Slice 1 registry under the same SemanticEntry shape — each cites Banks Act 94/1990 §70 + Regulations Relating to Banks Reg 26 + BCBS D295, and carries one [citation: TBC] marker per pack §9 Q1 default pending Mira's WS-INSTRUMENT-ANALYSES SARB BA 325 published-schema ingestion. New @platform/reporting package lands two pure projections — generateBa325Lcr (typed Ba325GeneratorInput → Ba325Output, with applyHqlaCaps closed-form 40%/15% cap arithmetic per BCBS D295 §47, 75% inflow cap per §142, 25% net-outflow floor) + renderBa325ToJson + canonicaliseBa325 (deterministic key-sorted bytes, validated against Ba325RenderSchema Zod schema, hash-store-friendly for downstream ReportGenerated events). CLI wrapper at scripts/render-ba-325.ts replays the event store, resolves the trial balance from periodAuditChain (Slice 2 substrate) or ad-hoc compute, applies a per-account liquidity-classification map (built-in default pins ACC-1100-001 → HQLA Level 1 per Slice 1's CashAndBalancesAtSARB worked entry), and writes canonical BA 325 JSON to stdout or --out. Per-entity isolation enforced — assertBankEntity throws on Hoz Securities / Hoz Group per D-REGULATORY-PERIMETER. Determinism witnessed via classificationsFingerprint + sortKeys canonicaliser. Tests: synthetic event-stream → period-close → BA 325 generation → JSON render with computed LCR; per-entity isolation refusal; HQLA cap-arithmetic regimes (no-cap, L2B-binding, L2A-binding); inflow cap binding; provenance passthrough via trialBalanceSnapshotEventId chain. M8 Azure mapping: rendered bytes lift to Azure Blob via the M8 RMS doc-store contract (PR #142 + spec §4.1); SARB-portal XML render (Slice 5) takes the canonical JSON as input contract; classification map migrates onto the chart-of-accounts schema (hqlaLevel + lcrOutflowCategory fields) at Slice 6+ once Mira's WS-INSTRUMENT-ANALYSES lands the SARB BA 325 published taxonomy. Substrate gaps surfaced: liquidity-projection executable form deferred to Slice 6 (v0 reads trial balance + supplied map directly); per-currency LCR per Reg 26(13) deferred to Slice 6 (v0 functional-currency only); BCBS 248 intraday liquidity monitoring is operational lens, not BA 325 input; live customer / wholesale / derivative balances populate at commencement-of-trading per project_rules_bind_at_commencement; Tier-1 LCR-model independent validation per RAS B7 gates production use post-licence-day. Forward-link to Slices 4-5 (BA 700 / BA 350 / BA 600 + regulator-portal handshake) and Slice 6+ (IFRS AFS skeleton + capital-stack + RWA projections). No changes to event-types.ts, registry.ts, period-close.ts, semantic/entries.ts, projections/runtime.ts, or dashboard/derive.ts (respect parallel work — Provenance Slice 3, RMS Slice 5, Phase B scenario script).",
  comment:
    "downstream dispatch from D-REPORTING-CAPABILITY-M2-M3-BUILD-PLAN Slice 3; no new policy decision",
  sourceDoc: "Owner Inbox/2026-05-10_bea-eitan-anya_d-reporting-capability-slice-3-ba-325-lcr.md",
