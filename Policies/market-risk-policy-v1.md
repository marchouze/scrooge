...
- **PA regulatory returns (`ORG-PR-60`):** Camille files BA-325 (FRTB market-risk capital — SA, and IMA where approved) as part of the BA-return suite, per the SARB reporting calendar. The BA-return filing is a `RegulatoryReturnFiled { returnType, period, amount }` event.
