// platform/reporting/ba-600-op-risk.ts
//
// D-REPORTING-CAPABILITY-M2-M3-BUILD-PLAN Slice 5 (this dispatch) — BA 600
// (operational-risk return) projection.
//
// Standing authority: D-REPORTING-CAPABILITY-M2-M3-BUILD-PLAN (CEO-
// approved 2026-05-10), pack §6 Slice 4 (BA 600 sub-scope) consolidated
...
export interface Ba600BiaSection {
  readonly perYearGrossIncome: readonly Ba600LineItem[];
...
export interface Ba600TsaSection {
  readonly perYearWeighted: readonly Ba600LineItem[];
  /** 1/3 × sum_y max(0, sum_i β_i × gi). */
  readonly capitalMinor: number;
}

export interface Ba600Output {
  readonly meta: {
  ...
  readonly bia?: Ba600BiaSection;
  readonly tsa?: Ba600TsaSection;
...
function assertBankEntity(entity: string): void {
  if (!BA_600_BANK_ENTITIES.includes(entity)) {
    throw new Ba600GeneratorError(
      `BA 600 (operational risk) is bank-licence-bound; entity '${entity}' is not in BA_600_BANK_ENTITIES (${BA_600_BANK_ENTITIES.join(", ")}). See Regulations/_legal-entity-tree.md + D-REGULATORY-PERIMETER.`,
    );
  }
}
...
function buildBia(rows: readonly OpRiskGrossIncomeRow[], ccy: string): Ba600BiaSection {
  ...
  const perYear: Ba600LineItem[] = sortedYears.map(([year, gi]) => ({
...
export function generateBa600OpRisk(input: Ba600GeneratorInput): Ba600Output {
