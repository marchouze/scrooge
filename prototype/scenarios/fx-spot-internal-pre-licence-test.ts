...
async function runPhase2(): Promise<PhaseResult> {
  ...
  r.assert(
    "TradeMatured (FX-spot) emitted (lifecycle close)",
    countEventsOfType("TradeMatured") === 1,
    `count=${countEventsOfType("TradeMatured")}`,
  );

  // Wave-2 (Kai PR — this PR): emit GL posting for TradeMatured (FX-spot)
  ...
  try {
    ...
    for (const confirmed of eventStore.replay({ type: "TradeMatured" })) {
      ...
      if (!isFxSpotMaturity(confirmedPayload as never)) continue;
      const legs = fxSettlementJournals(confirmedPayload.product);
      await emitPosting({
...
async function runPhase6(): Promise<PhaseResult> {
  ...
  // TradeMatured (FX-spot) by pair / counterparty / book. We assert the
  ...
  try {
    ...
    r.assert(
      "Block B — Phase 2 happy-path trade is marked 'settled' (TradeMatured (FX-spot) seen)",
    ...
    r.assert(
      "Block B — Phase 3 Herstatt trade stays 'live' (no TradeMatured (FX-spot))",
  ...
  // FxPositionRevalued (revaluation), TradeMatured (FX-spot) (settlement),
