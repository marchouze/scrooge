// scripts/render-ba-120.ts
//
// CLI wrapper for the BA 120 (Off-Balance-Sheet Activities) generator +
// renderer.
//
// Usage:
//   bun run scripts/render-ba-120.ts \
...
//   2. Loads a BA 120 classification map, guarantees entries, and
//      commitments entries from JSON files (defaults to built-in
//      build-phase fixtures if files are not supplied).
//   3. Generates the BA 120 projection (pure function).
...
const BUILD_PHASE_DEFAULT_CLASSIFICATION_MAP: Ba120ClassificationMap = [
  {
    category: "fx-spot",
    section: "derivatives",
    formLine: "BA 120 — FX derivative notionals (spot)",
    subLine: "FX spot",
  },
  {
    category: "fx-forward",
    section: "derivatives",
    formLine: "BA 120 — FX derivative notionals (forward)",
    subLine: "FX forward",
  },
  {
    category: "fx-swap",
    section: "derivatives",
    formLine: "BA 120 — FX derivative notionals (swap)",
    subLine: "FX swap",
  },
];
 ...
 */
const BUILD_PHASE_DEFAULT_GUARANTEES: readonly Ba120GuaranteesEntry[] = [
  {
 ...
 */
const BUILD_PHASE_DEFAULT_COMMITMENTS: readonly Ba120CommitmentsEntry[] = [
  {
...
function parseArgs(argv: readonly string[]): CliArgs {
  ...
  if (!entity || !asOf || !periodId) {
    throw new Error(
      "render-ba-120: --entity, --as-of, --period-id are required. See script header for usage.",
    );
  }
  return {
    entity,
...
function loadJson<T>(path: string, label: string): T {
  ...
  if (parsed === null || parsed === undefined) {
    throw new Error(`render-ba-120: --${label} file at ${path} parsed to null/undefined`);
  }
  return parsed as T;
}
...
function main(argv: readonly string[]): number {
  ...
  const classificationMap = args.classificationMapPath
    ? loadJson<Ba120ClassificationMap>(args.classificationMapPath, "classification-map")
    : BUILD_PHASE_DEFAULT_CLASSIFICATION_MAP;

  const guarantees = args.guaranteesPath
    ? loadJson<readonly Ba120GuaranteesEntry[]>(args.guaranteesPath, "guarantees")
    : BUILD_PHASE_DEFAULT_GUARANTEES;

  const commitments = args.commitmentsPath
    ? loadJson<readonly Ba120CommitmentsEntry[]>(args.commitmentsPath, "commitments")
  ...
  const output = generateBa120OffBalanceSheet({
    entity: args.entity,
    asOf: args.asOf,
