// tests/ba-100-xml-adapter.test.ts
//
// Unit tests for the BA 100 (Capital Adequacy) XML adapter.
//
// Tests:
//   1. ba100ToXmlPayload() — formId is "BA700".
//   2. ba100ToXmlPayload() — formVersion, xsdUri, namespaceUri correctly set.
//   3. ba100ToXmlPayload() — body.Meta contains Form, Entity, PeriodId.
//   4. ba100ToXmlPayload() — body.CapitalStack.Cet1.NetStockMinor maps correctly.
//   5. ba100ToXmlPayload() — body.Rwa.TotalRwaMinor maps correctly.
//   6. ba100ToXmlPayload() — body.Ratios carries CET1 + Tier1 + Total fields.
//   7. ba100ToXmlPayload() — Infinite ratios serialise as "Infinity".
//   8. ba100ToXmlPayload() — LeverageRatio section absent when not supplied.
//   9. renderSarbXml() round-trip — valid XML with BA700 root element.
//  10. validateSarbXmlStructural() — passes for well-formed BA 100 XML.
//  11. validateSarbXmlStructural() — catches missing required elements.
//  12. ba100ToXmlPayload() — trialBalanceSnapshotEventId forwarded when present.
//
...
describe("ba100ToXmlPayload() → renderSarbXml() round-trip", () => {
