...
The accounting substrate is empty — zero `SubLedgerEntryPosted`, zero `IFRSClassificationAssigned`, zero `CloseApproved` ever — and the load-bearing block on Camille's first signed close is the monthly close cycle, specifically the absence of a sub-ledger projection over postable events. Until that projection exists, nothing downstream can fire: the BA-return generator has no sub-ledger cells to map from (Banks Act 94 of 1990 Reg 38), the AFS line-item engine (IAS 1) has no trial balance to structure, and the capital-base derivation has no CET1 numerator to read. All 26 Bea-owned obligations sit at PLANNED or DRAFTING — none IN FORCE, none even PARTIAL — which is the honest state for build phase but means every IFRS 9 / 7 / 13 / 15 / 16 and IAS 1 / 7 / 12 disclosure obligation is gated on the same upstream substrate ticket.

The consequential observation is that the monthly close is one engineering ticket — the sub-ledger projection — away from being able to produce its first synthetic `SubLedgerEntryPosted`. The chart-of-accounts schema and the first posting rule (`FundingDrawn` → debit cash / credit funding-liability, per IFRS 9 initial recognition at amortised cost and IAS 1 presentation) are both specified and waiting on the projection to land against. The BA 100 cell map and the AFS line-item schema (IAS 1 structure, IAS 12 deferred-tax co-owned with Yael) can be drafted in parallel but cannot dry-run until the sub-ledger emits its first event. IFRS 9 ECL staging and Income Tax Act 58 of 1962 current-tax postings stay correctly deferred — no exposure book, no taxable profit, nothing to classify.

Next engineering move, in order: (1) specify the chart-of-accounts schema and commit it to the substrate this week; (2) build the sub-ledger projection over postable events, joint with Atlas, so `SubLedgerEntryPosted` becomes a real event type; (3) publish the `FundingDrawn` → debit cash / credit funding-liability posting rule as the first end-to-end test of the projection; (4) commission Anya's BA 100 cell-map register against the synthetic capital line per Reg 38 so the first `BAReturnGenerated` dry-run can fire the moment the sub-ledger lands. The close-engine orchestrator and `CloseApproved` producer are the last wire — they connect engineer-posted to Camille-signed, and they should not be built before the projection underneath them is real.
