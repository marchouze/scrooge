...
| IRRBB — interest-rate risk in the banking book (ΔEVE / ΔNII outlier test) | Risk-taxonomy node + PA directive | `urn:risk:bank:RT-IRRBB` (`Regulations/_risk-taxonomy.md` §4.4, citing BCBS D368 + Reg 30 + PA Directive 8/2023). **Tracked gap:** no dedicated Basel IRRBB *standard text* (SRP31 / D368) is acquired into `Regulations/BCBS/` — see §20. | The oracle for the banking-book rate-risk measure where it touches Rohan's scope (ΔEVE outlier, the trading-/banking-book boundary). Owned jointly with Ravi (ALM) and Helena (CRO). |
| Basel CVA — credit-valuation-adjustment capital framework | Standard / framework | **Tracked gap — no graph node.** The CVA standard group was NOT acquired by the MAR/CRE/CAP `D-REGULATORY-LIBRARY-V1` pass; no `urn:reg:bcbs:cva:*` node exists. The live `cva-engine.ts` + `recon:cva-derivatives-sim-drive` operate without an acquired-standard oracle. Flagged in §16 / §20 — **not** cited to an invented URN. | Would be the oracle for the *CVA capital charge* (BA-CVA / SA-CVA). Until the standard is acquired, Rohan's CVA outputs are validated only against the engine's internal logic + the sim-drive gate, which §20 records as a domain-truth deficiency. |

- **Standards (authoritative oracles):** Basel MAR (`mar:20.1` / `33.1` / `32.18`), Basel CRE (`cre:22.67` SA-CCR; `cre:20.8` / `20.32` standardised RW), Basel CAP + RBC (`rbc:20.2` / `30.2`), Basel LCR / NSF. These are the bodies of rule Rohan's measures MUST conform to — FRTB for market-risk capital, SA-CCR for counterparty EAD, CAP/RBC for the capital floors. Each is a real node in `Regulations/BCBS/`, acquired and structured per `D-REGULATORY-LIBRARY-V1`.
...
- **(a) Domain-invariant recon gates** — fail-closed gates encoding "a market- / counterparty-credit-risk expert would never do X":
  ...
  | …leave the CVA charge unbacked by an acquired CVA standard (domain-truth oracle gap — **advisory**, see §20) | `recon:cva-derivatives-sim-drive` (sim-drive only; no standard oracle) | `warn` |
...
- **Validation cadence:** the domain-invariant gates (a) run every CI run; the golden anchors (b) are asserted on every model-version publish and every daily risk run (`RiskRunCompleted`). A new domain-invariant gate or golden case is **harden-only** (per the lessons-to-gates reflex, §20 / PROC-GOV-ADC-01 §5) — added, never weakened, without a recorded Decision. The CVA-oracle gap (§18, §20) is itself a standing harden-only item: when the Basel CVA standard is acquired, a `recon:cva-standard-conformance` gate replaces the advisory `warn`.
...
- **Confirm-or-challenge gate:** on receiving this dispatch, Rohan states the verdict on each domain premise with a §18 citation before implementing:
  ...
  - **CHALLENGED — "CVA capital framework … as a citable domain-truth ORACLE":** the brief's *substance* is right (CVA = credit-valuation-adjustment capital, distinct from FRTB and SA-CCR), but its *premise that a citable CVA oracle exists* is wrong for this seat right now. The `D-REGULATORY-LIBRARY-V1` pass acquired MAR, CRE, CAP, RBC, LCR, NSF, LEX, MGN, SCO, OPE, LEV, SRP, DIS, BCP — **not** the Basel CVA standard group. There is no `urn:reg:bcbs:cva:*` node. Per the brief's own instruction ("if none exists, flag the gap … do NOT invent an id"), CVA is cited as a **tracked gap** in §18 / §19, and the live `cva-engine.ts` is recorded as operating without an acquired-standard oracle. Acquiring the Basel CVA standard is filed as the closing action.
  - **CHALLENGED — IRRBB "where it touches your scope":** confirmed as in-scope (trading-/banking-book boundary, ΔEVE), but there is **no acquired Basel IRRBB standard text** (SRP31 / D368) in `Regulations/BCBS/`. The citable node that genuinely exists is the risk-taxonomy node `urn:risk:bank:RT-IRRBB` (citing BCBS D368 + Reg 30 + PA Directive 8/2023). Cited as such; the BCBS-standard-text acquisition is flagged as a gap.
