// tests/off-balance-sheet.test.ts
//
// D-REPORTING-CAPABILITY-M2-M3-BUILD-PLAN Slice 9 — exit-criterion tests
// for the off-balance-sheet (Off-Balance-Sheet Activities) generator + JSON renderer.
//
// Asserts:
...
//  10. JSON render: validates against OffBalanceSheetRenderSchema; schema URL correct.
...
//  13. Render one-shot helper: renderOffBalanceSheetCanonical produces render + bytes.
...
const DEFAULT_MAP: OffBalanceSheetClassificationMap = [
  {
    category: "fx-spot",
    section: "derivatives",
    formLine: "off-balance-sheet — FX spot notionals",
  },
];
...
const DEFAULT_GUARANTEES: OffBalanceSheetGuaranteesEntry[] = [
  {
...
const DEFAULT_COMMITMENTS: OffBalanceSheetCommitmentsEntry[] = [
  {
...
describe("D-REPORTING-CAPABILITY-SLICE-9 — per-entity isolation", () => {
  it("OFF_BALANCE_SHEET_BANK_ENTITIES contains only LE-ZA-HOZ-BANK", () => {
    expect(OFF_BALANCE_SHEET_BANK_ENTITIES).toEqual(["LE-ZA-HOZ-BANK"]);
  });
  ...
  it("rejects LE-ZA-HOZ-SECURITIES", () => {
    ...
    ).toThrow(OffBalanceSheetGeneratorError);
  });
  ...
  it("rejects duplicate classificationMap entries for the same category", () => {
    const dupMap: OffBalanceSheetClassificationMap = [
...
describe("D-REPORTING-CAPABILITY-SLICE-9 — derivatives section", () => {
  it("FX spot settlement obligation → fxRows entry with correct notional", () => {
    ...
    const out = generateOffBalanceSheet({
      entity: ENTITY_BANK,
      asOf: AS_OF,
      periodId: PERIOD_ID,
      functionalCurrency: CURRENCY,
...
describe("D-REPORTING-CAPABILITY-SLICE-9 — guarantees section", () => {
  it("financial + performance guarantees produce correct section totals", () => {
    const guarantees: OffBalanceSheetGuaranteesEntry[] = [
      {
        guaranteeType: "financial",
        remainingMaturityDays: 90,
        nominalMinor: 100_000_000,
...
describe("D-REPORTING-CAPABILITY-SLICE-9 — commitments section (CCF)", () => {
  it("irrevocable < 12 months → CCF 0%", () => {
    const commitments: OffBalanceSheetCommitmentsEntry[] = [
      {
        commitmentType: "irrevocable",
        originalTermMonths: 6,
        undrawnAmountMinor: 200_000_000,
...
describe("D-REPORTING-CAPABILITY-SLICE-9 — JSON render", () => {
  it("renders to canonical JSON validating against OffBalanceSheetRenderSchema", () => {
    const ob = makeFxObligation("ob-001", "2026-06-02", 7_500_000, "ZAR");
    const out = generateOffBalanceSheet({
      entity: ENTITY_BANK,
      asOf: AS_OF,
    ...
    const render = renderOffBalanceSheetToJson(out, { renderedAt });

    expect(() => OffBalanceSheetRenderSchema.parse(render)).not.toThrow();
    expect(render.$schema).toBe(OFF_BALANCE_SHEET_SCHEMA_URL);
    expect(render.meta.form).toBe("off-balance-sheet");
  ...
  it("empty obligations renders without error", () => {
    ...
    const render = renderOffBalanceSheetToJson(out, { renderedAt: "2026-05-17T10:00:00.000Z" });
    expect(() => OffBalanceSheetRenderSchema.parse(render)).not.toThrow();
  });
});
...
describe("D-REPORTING-CAPABILITY-SLICE-9 — canonicalisation determinism", () => {
  it("identical inputs → byte-identical canonical JSON across two runs", () => {
    ...
    const renderedAt = "2026-05-17T10:00:00.000Z";
    const a = renderOffBalanceSheetCanonical(generateOffBalanceSheet(input), { renderedAt });
    const b = renderOffBalanceSheetCanonical(generateOffBalanceSheet(input), { renderedAt });
  ...
  it("top-level key sort: $schema comes first ($ < letters in ASCII)", () => {
    ...
    const { canonicalJson } = renderOffBalanceSheetCanonical(out, {
...
describe("D-REPORTING-CAPABILITY-SLICE-9 — provenance passthrough", () => {
  ...
  it("classificationMapFingerprint differs when map changes", () => {
    const mapA: OffBalanceSheetClassificationMap = [
      { category: "fx-spot", section: "derivatives", formLine: "line A" },
    ];
    const mapB: OffBalanceSheetClassificationMap = [
      { category: "fx-spot", section: "derivatives", formLine: "line B" },
    ];
...
// =====================================================================
// 14. renderOffBalanceSheetCanonical one-shot helper.
// =====================================================================

describe("D-REPORTING-CAPABILITY-SLICE-9 — renderOffBalanceSheetCanonical", () => {
  it("returns render, canonicalJson, and canonicalBytes", () => {
    ...
    const result = renderOffBalanceSheetCanonical(out, { renderedAt: "2026-05-17T10:00:00.000Z" });
