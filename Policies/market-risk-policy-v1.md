...
date: 2026-05-13
summary: Standalone Market Risk Policy covering FRTB standardised approach as baseline, IMA desk-approval pathway, trading book / banking book boundary, VaR / ES / sensitivity-based risk appetite limits, NMRF / SES treatment, back-testing and PLA test governance, the 72.5% output floor + SA/IMA dual-reporting, revised CVA capital, no-proprietary-trading principle, and Market Risk Committee governance. Closes obligations ORG-PR-19, ORG-PR-20, ORG-PR-33, ORG-PR-56, ORG-PR-57, ORG-PR-58, ORG-PR-59, ORG-PR-60, and the market-risk BA-return obligations ORG-PR-RETURNS-012 (BA 320 market risk) and ORG-PR-RETURNS-013 (BA 325 Selected Risk Exposure). COMMENCEMENT-BIND.
...
### 4.6 Output Floor and SA/IMA Dual-Reporting (`ORG-PR-60`)

`ORG-PR-60` (`urn:obligation:bank:prudential:frtb-sa-ima-capital-reporting:v1`) is a two-limbed obligation: (i) report market-risk capital under **both** the Standardised Approach and the Internal Models Approach (where IMA is in force for a desk), and (ii) apply the Basel III **output floor** — IMA-derived capital must not fall below **72.5%** of the capital that the Standardised Approach would produce. This sub-section discharges both limbs at the policy layer; the §7 closure of `ORG-PR-60` is anchored here (not §4 / §6.2 in the abstract).

**Output-floor mechanic.** The output floor is a Basel III aggregate-RWA constraint (BCBS FRTB January 2019, output-floor provision `[citation: TBC — BCBS FRTB Art on the 72.5% output floor; document not in repo source docs; pending Imani (Legal-as-code engineer, engineering) + external counsel retrieval at the licence-application gate]`; Regulations Relating to Banks **Reg 28** output-floor sub-regulation `[citation: TBC — Reg 28 output-floor and SA/IMA dual-reporting sub-clause indices pending manual review; Chapter II of `rrb-structured.json` not yet extracted]`). For market risk specifically:

1. **Dual computation.** For each reporting period, Rohan computes both the FRTB SA capital (§4.1) and, for each PA-approved IMA desk, the IMA capital (ES + SES). Where no desk is IMA-approved (the position at commencement of trading), SA capital is the only figure and the output floor is non-binding by construction (IMA capital is undefined / equals SA).
2. **Floor comparison.** Once one or more desks are IMA-approved, the bank-wide floored market-risk capital is `max(IMA_capital, 0.725 × SA_capital)`. The floor is applied at the **aggregate** Basel III level (across all risk types), with the market-risk SA charge contributing to the SA-floor denominator; this policy's mechanic is the market-risk contribution to that aggregate, reconciled by Camille (Chief Financial Officer, governance) in the Capital Management Policy's aggregate output-floor computation.
3. **Floor add-on.** If `0.725 × SA_capital > IMA_capital`, the difference (the **output-floor add-on**) is added to risk-weighted assets and reported. The add-on is a `OutputFloorAddOnComputed { period, saCapital, imaCapital, flooredCapital, addOn }` typed event filed by Rohan as part of the monthly capital report.
4. **Dual-reporting cadence.** Both the SA and IMA market-risk capital figures are reported to the PA **quarterly** per `ORG-PR-60` (alongside the BA-return cadence in §6.2). The dual figures and any output-floor add-on are tabled at the Market Risk Committee monthly and form part of the quarterly BRC capital pack.

**Transitional note.** The Basel III output floor phases in over a transition window per the BCBS / PA implementation timeline; the 72.5% steady-state floor is the binding parameter once fully phased in. The applicable transitional floor percentage for each reporting period is sourced from the PA implementation directive (PA D/2025; `ORG-PR-33`) `[citation: TBC — transitional floor schedule pending PA directive ratification]`. Until any desk is IMA-approved, the output floor imposes no constraint and the SA capital governs in full; the mechanic above binds from the first IMA desk approval.
...
- **PA regulatory returns (`ORG-PR-60`, `ORG-PR-RETURNS-012`, `ORG-PR-RETURNS-013`):** Camille files the market-risk BA-return suite as part of the SARB reporting calendar. Under the Excel-canonical SARB form schedule (`D-BA-RETURN-NUMBERING-EXCEL-CANONICAL`), the market-risk returns this policy closes are: **BA 320** — Market Risk (the bank's primary market-risk capital return per D5/2025 §2.1.13; `ORG-PR-RETURNS-012`); and **BA 325** — Selected Risk Exposure Arising from Trading and Treasury Activities (the FRTB / sensitivity-based-method return per D5/2025 §2.1.14; `ORG-PR-RETURNS-013`). The SA capital (or IMA capital once a PA-approved Internal Models Approach is in force, with the §4.6 output floor applied) flows into these returns and onward into the BA 700 capital-adequacy summary. Each BA-return filing is a `RegulatoryReturnFiled { returnType, period, amount }` event. (The minimum-liquid-reserve / HQLA return, `ORG-PR-RETURNS-011`, is a *liquidity* return closed by the Liquidity Risk Management Policy — **not** by this policy; an earlier fabricated annotation that mis-classified that HQLA form as a standardised-approach capital return was corrected under `D-BA-RETURN-NUMBERING-EXCEL-CANONICAL`.) The reporting-substrate module filenames and any residual stale form-number references are reconciled to this Excel-canonical mapping under the reporting-code track of `D-BA-RETURN-NUMBERING-EXCEL-CANONICAL` (Mira (Compliance / RegTech engineer, engineering); Bea (Accounting & financial reporting engineer, engineering)) — code-side scope, tracked separately from this policy-layer closure.
...
| `ORG-PR-60` | SA/IMA capital reporting to PA + 72.5% output floor | **IN FORCE** (COMMENCEMENT-BIND) — closed | §4.6 (Output Floor and SA/IMA Dual-Reporting — both obligation limbs), §6.2 (Reporting) |
| `ORG-PR-RETURNS-012` | Submit form BA 320 (Market Risk return — primary market-risk capital return) to PA per D5/2025 §2.1.13 + Reg 28 | **IN FORCE** (COMMENCEMENT-BIND) — closed | §6.2 (PA regulatory returns); §4 (FRTB Capital Framework feeds the return) |
| `ORG-PR-RETURNS-013` | Submit form BA 325 (Selected Risk Exposure Arising from Trading and Treasury Activities / FRTB return) to PA per D5/2025 §2.1.14 | **IN FORCE** (COMMENCEMENT-BIND) — closed | §6.2 (PA regulatory returns); §4.1 (FRTB SA capital), §4.6 (SA/IMA dual-reporting) |
...
|---|---|---|---|
| v1.2 | 2026-06-09 | Helena (Chief Risk Officer, governance) — via Scrooge dispatch `brief:helena:close-seven-open-vera-audit-findings-liquidity-m:2026-06-09`; authority `D-OPEN-THREADS-CLOSEOUT-2026-06-09` (CEO session-delegation) | Remediation of two Vera (internal audit engineer) findings. **`F-HELENA-20260608-VINK`** (overstated closure): added §4.6 (Output Floor and SA/IMA Dual-Reporting) discharging both limbs of `ORG-PR-60` — the 72.5% Basel III output floor (floor comparison `max(IMA, 0.725 × SA)`, output-floor add-on, `OutputFloorAddOnComputed` event) and the SA/IMA quarterly dual-reporting cadence; corrected the §7 closure row to anchor `ORG-PR-60` on §4.6 rather than the abstract §4/§6.2. The 72.5% floor was previously absent from the policy body. **`F-MIRA-20260608-4M7U`** (orphaned market-risk return obligations): anchored the two *market-risk* return obligations `ORG-PR-RETURNS-012` (BA 320 — Market Risk, the primary capital return) and `ORG-PR-RETURNS-013` (BA 325 — Selected Risk Exposure / FRTB) in §6.2 and the §7 closure table, per the Excel-canonical SARB form schedule (`D-BA-RETURN-NUMBERING-EXCEL-CANONICAL`). `ORG-PR-RETURNS-011` (the minimum-liquid-reserve / HQLA return) is a *liquidity* return and is closed by the Liquidity Risk Management Policy §10, not by this policy — the original finding pre-dated Bea's Excel-canonical correction that re-identified that form as an HQLA / minimum-liquid-reserve return. The reporting-substrate module-filename reconciliation remains code-side scope (Mira / Bea). No change to the substantive risk-measurement framework. |
