...
**Substrate dependency (policy / build split).** The consolidation **rules** above are the policy-layer discharge of `ORG-PR-LCR-003/004/005` and are in force as policy from commencement of trading. The **consolidated-LCR computation engine** — the substrate that applies these rules across the group perimeter (entity summation, per-jurisdiction HQLA capping, intragroup elimination, 75% group inflow cap, Rand conversion, monthly cadence) — is **not yet built**; it is tracked as the explicit substrate follow-on `GAP-LCR-CONSOLIDATION-ENGINE` (Bea (Accounting & financial reporting engineer, engineering) / Ravi (Treasury / ALM engineer, engineering); W2 Slice 5). See §11.5. This finding's policy half (`F-HELENA-20260608-XF2F`) is closed by this section; the engine build is a separate downstream deliverable and is **not** in scope of this policy amendment.
...
|---|---|---|---|---|---|
| `GAP-LCR-CONSOLIDATION-ENGINE` | Consolidated-LCR computation engine implementing the D1/2022 §4.1.2 consolidation rules: per-entity summation of net cash outflows + HQLA, entity-level HQLA capping at the lower of jurisdiction/home minimum, transferability/convertibility evaluation of excess HQLA, intragroup-transaction elimination, 75% group-basis inflow cap, Rand conversion of all components, monthly cadence + month-end group position calculation. | **Closed** by §2.6 (rules specified at the policy layer; `F-HELENA-20260608-XF2F` policy half) | **Open** — engine not yet built. Tracked as a W2 Slice 5 deliverable; binds at the pre-licence go-live readiness gate. NOT in scope of the v1.2 policy amendment (PR closing the seven findings). A separate engineering dispatch builds the engine; until then the consolidated LCR is produced by the existing solo BA 300 substrate without the full multi-entity consolidation logic, with the gap declared. | Bea (Accounting & financial reporting engineer, engineering) / Ravi (Treasury / ALM engineer, engineering); governance Eitan (Treasurer, governance) + Helena (Chief Risk Officer, governance) | OPEN (build) / policy half closed |
