...
//
// Tests:
//   1. ba300LcrToXmlPayload() — produces a SarbXmlReportPayload with formId "BA300".
//   2. ba300LcrToXmlPayload() — body.Meta contains Form, Entity, PeriodId, etc.
//   3. ba300LcrToXmlPayload() — body.Hqla contains TotalStockHqlaMinor.
//   4. ba300LcrToXmlPayload() — body.CashFlows contains NetCashOutflowsMinor.
//   5. ba300LcrToXmlPayload() — LcrRatio is mapped (finite + Infinity cases).
//   6. ba300LcrToXmlPayload() — LcrCompliant is mapped correctly.
...
//  10. ba300LcrToXmlPayload() — optional trialBalanceSnapshotEventId is forwarded.
//
...
import type { Ba300LcrOutput } from "../platform/reporting/ba-300-lcr";
...
function makeMinimalBa300LcrOutput(): Ba300LcrOutput {
  return generateBa300Lcr({
    entity: ENTITY,
    asOf: AS_OF,
    periodId: PERIOD_ID,
...
describe("ba300LcrToXmlPayload()", () => {
  ...
  it("TC-2: formVersion, xsdUri, namespaceUri are set correctly", () => {
    ...
    expect(payload.xsdUri).toBe(BA_300_LCR_XSD_URI);
    expect(payload.namespaceUri).toBe(BA_300_LCR_NAMESPACE);
  });
...
describe("ba300LcrToXmlPayload() → renderSarbXml() round-trip", () => {
  it("TC-11: produces well-formed XML with BA300 root element", () => {
    ...
    expect(xml).toContain("</BA300>");
    expect(xml).toContain(`xmlns="${BA_300_LCR_NAMESPACE}"`);
  });
  ...
  it("TC-12: validateSarbXmlStructural() passes for well-formed BA 300 XML", () => {
    ...
    const result = validateSarbXmlStructural({
      ...
      requiredElements: BA_300_LCR_REQUIRED_ELEMENTS,
    });
  ...
  it("TC-13: validateSarbXmlStructural() catches missing required elements", () => {
    ...
    const result = validateSarbXmlStructural({
      ...
      formId: "BA300",
      namespaceUri: BA_300_LCR_NAMESPACE,
      requiredElements: [...BA_300_LCR_REQUIRED_ELEMENTS, "NonExistentSection"],
    });
