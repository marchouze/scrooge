...
// for the BA 100 (Capital Adequacy) generator + JSON renderer + per-account
...
describe("D-REPORTING-CAPABILITY-SLICE-4 — semantic entries register", () => {
  ...
  it("each ratio entry maps to a BA 100 cell", () => {
    for (const e of [commonEquityTier1Ratio, tier1CapitalRatio, totalCapitalRatio]) {
      expect(e.regulatoryCells?.some((c) => c.form === "BA 100")).toBe(true);
    }
  });
});
...
describe("D-REPORTING-CAPABILITY-SLICE-4 — end-to-end (events → close → BA 100)", () => {
  function setupClose(): {
    store: EventStore;
  ...
  it("computes a known BA 100 from synthetic events; ratios well above minimum", () => {
