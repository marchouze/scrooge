...
import type { DocumentStore } from "../../platform/document-store/types";
...
export async function generateAndRecordBa700ForPeriod(args: {
  ...
  /**
   * Document store the rendered SARB XML envelope is filed into. Defaults to the
   * composition singleton (`defaultDocumentStore`, resolved per the shared-store
   * ladder). Injected by tests for a hermetic tmpdir-backed store.
   */
  documentStore?: DocumentStore;
}): Promise<Ba700RunResult> {
  const { store, entity, closedPayload, documentStore } = args;
  ...
  // File the FULL rendered SARB XML envelope into the RMS content-addressed
  // document store + emit a `RecordFiled` (D-BA-RETURN-OF-RECORD-DOCUMENT-STORE).
  // Done UNCONDITIONALLY (before the submission idempotency check) so the
  // regulator-facing artefact is filed on every close — including replays of
  // periods that submitted before this filing path landed. Idempotent at both
  // the blob layer (content-addressing) and the event layer (per entity+form+
  // period). The envelope is rendered from the SAME `result.rawOutput`, never
  // re-derived. The RecordFiled references the ReportGenerated by event id +
  // attestable contentHash (linkage by reference — the envelope's own document
  // hash is NOT equal to the attestable contentHash).
  fileBa700ReturnArtefact({
    store,
    entity,
    reportingPeriod: periodId,
