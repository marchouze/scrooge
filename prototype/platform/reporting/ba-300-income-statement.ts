// platform/reporting/ba-300-income-statement.ts
//
// WS-FINANCE-BA-RETURNS-QUINTET — BA 300 (Income Statement) projection.
//
...
// explicit BA 300 line-classification map. No event side-effects; no
// document-store writes; no event-store reads. Callers (the CLI wrapper
// at `prototype/scripts/render-ba-300.ts`, downstream `ReportGenerated`
// event emitters) compose the inputs and consume the output.
//
// Pipeline (mirrors Slice 4's BA 700 architecture):
//
...
//      → SEMANTIC LAYER        — IFRS Slice 6 + BA 300 line mapping (caller-side)
//      → BA 300 PROJECTION     — this module — pure function
//      → RENDER + STORE        — `ba-300-render.ts` + RMS doc store
//
// Computation per Banks Act 94 of 1990 §75 + Regulations Relating to
// Banks Reg 32 (monthly returns) + SARB BA 300 published schema +
...
// scope at v0. Securities-firm and group-consolidated BA 300 deferred
// under `D-REGULATORY-PERIMETER`.
//
// Substrate gaps surfaced (forward-link in the decision record):
//   - **BA 300 line-classification map** is supplied externally for now.
...
//     extend to publish SARB BA 300 line mapping; until then the map is
...
//   - **OCI (Other Comprehensive Income)** deferred. SARB BA 300 form
//     scope at v0 is profit-or-loss only; the IFRS Slice 6 SoCI carries
//     OCI separately.
//   - **Group-consolidated BA 300** (LE-ZA-HOZ-GROUP look-through per
//     Banks Act §60) deferred. Solo entity only at v0.
//   - **SARB BA 300 XML adapter** deferred to a downstream slice; this
...
/**
 * BA 300 line classification. Each P&L leaf account maps to exactly one
 * BA 300 line bucket. Accounts not in the map are surfaced in
...
export interface Ba300LineClassification {
  readonly leafAccountId: string;
  readonly category: Ba300LineCategory;
  /**
   * Free-form sub-line label for the BA 300 form (e.g.
...
export type Ba300ClassificationMap = readonly Ba300LineClassification[];
...
export interface Ba300GeneratorInput {
  ...
  /** Per-account BA 300 classifications. */
  readonly classifications: Ba300ClassificationMap;
...
export interface Ba300LineCategorySection {
  readonly category: Ba300LineCategory;
  readonly totalMinor: number;
  readonly lineItems: readonly Ba300LineItem[];
}

export interface Ba300ClassificationGap {
  readonly leafAccountId: string;
  readonly currency: string;
  readonly amountMinor: number;
}

export interface Ba300IncomeStatement {
  readonly meta: {
  ...
  readonly interestIncome: Ba300LineCategorySection;
  readonly interestExpense: Ba300LineCategorySection;
  readonly netInterestIncomeMinor: number;
  readonly feeIncome: Ba300LineCategorySection;
  readonly tradingProfitLoss: Ba300LineCategorySection;
  readonly operatingExpenses: Ba300LineCategorySection;
  readonly otherIncome: Ba300LineCategorySection;
  readonly otherExpenses: Ba300LineCategorySection;
  readonly profitBeforeTaxMinor: number;
  readonly classificationGaps: readonly Ba300ClassificationGap[];
...
function assertBankEntity(entity: string): void {
  if (!BA_300_BANK_ENTITIES.includes(entity)) {
    throw new Ba300GeneratorError(
      `BA 300 (Income Statement) is bank-licence-bound; entity '${entity}' is not in BA_300_BANK_ENTITIES (${BA_300_BANK_ENTITIES.join(
        ", ",
...
/**
 * Generate the BA 300 (Income Statement) projection from a period-delta
 * trial balance + a caller-supplied line-classification map. Pure
 * function; deterministic.
 */
export function generateBa300IncomeStatement(input: Ba300GeneratorInput): Ba300IncomeStatement {
  ...
  if (!(input.periodStart < input.periodEnd)) {
    throw new Ba300GeneratorError(
      `BA 300 generator: periodStart (${input.periodStart}) must precede periodEnd (${input.periodEnd})`,
    );
  }

  const ccy = input.functionalCurrency;

  const classMap = new Map<string, Ba300LineClassification>();
  for (const c of input.classifications) {
    if (classMap.has(c.leafAccountId)) {
      throw new Ba300GeneratorError(
        `BA 300 generator: duplicate classification for account '${c.leafAccountId}'`,
      );
    }
  ...
  type Bucket = { lines: Ba300LineItem[]; total: number };
  const buckets: Record<Ba300LineCategory, Bucket> = {
  ...
  const classificationGaps: Ba300ClassificationGap[] = [];
  ...
  if (classificationGaps.length > 0) {
    placeholders.push(
      `[citation: TBC — BA 300: ${classificationGaps.length} trial-balance rows have no line classification; pending Mira's WS-INSTRUMENT-ANALYSES SARB BA 300 published-schema mapping]`,
    );
  }
  placeholders.push(
  ...
  const section = (cat: Ba300LineCategory): Ba300LineCategorySection => ({
    category: cat,
...
function fingerprintClassifications(classifications: Ba300ClassificationMap): string {
