...
asOf: 2026-05-30T04:21:36.147Z
decision-required: false
---

# Tomas — daily payments-readiness, 2026-05-30
...
Payments substrate is roughly two-thirds specified, one-third drafting, and zero live — no BIC lodged, no sponsor-bank contract signed, no scheme connectivity, and the 7-day event count is flat zeros across PaymentInitiated, PaymentSettled, ReconciliationCompleted, SAMOSWindowEntered and SWIFTMessageProcessed, as expected for build phase. The load-bearing block on Devon's first end-to-end resilience rehearsal is `capability:samos-rtl-gateway`: until a synthetic pacs.009 / pacs.002 round-trip clears the cut-off engine at the 06:00 / 16:00 SAST windows the SARB NPSD circulars pin, there is nothing for Bea's posting harness or Kai's trade feed to settle against. The indirect-participant posture under NPS Act 78 of 1998 collapses a great deal of would-be build work — SAMOS direct membership, BankservAfrica full participant onboarding, direct CLS membership — onto a single sponsor-bank API seam, so the contract artefact itself becomes the unblock for three capabilities at once.

Three observations rank above the rest. First, `capability:iso-20022-pacs-camt` is one ticket from green: envelope library exists, schema-version pin is the outstanding decision, and camt.053 / camt.054 are the declared recon source-of-truth — once pinned to the current scheme-published version (lock-step cadence per the ISO 20022 repository and BankservAfrica / SAMOS service definitions), the reconciler has a real input shape. Second, `obligation:swift:csp` has no live owning capability yet — the SWIFT CSP 2024 control framework's annual attestation cycle does not activate until first BIC, but the controls themselves bind the connector design now and are co-owned with Senna; this is a paper gap, not a build gap, but it should be marked as such rather than left implicit. Third, `capability:gl-ledger-reconciliation` is the cut-off / reconciliation gap that gates Bea's first cash-leg posting: the three-way contract (trade-leg ↔ payment-leg ↔ ledger-leg) is specified but not drafted, and Bea cannot post a cash leg against a payment event that has no reconciler to close it.

Next engineering move, in order: (1) draft `@platform/payments/samos-connector` as a synthetic SAMOS gateway emitting pacs.009 and consuming pacs.002, wired to the calendar engine's 06:00 / 16:00 SAST windows with the ±15 min tolerance per spec § 9; (2) pin the ISO 20022 envelope library to the current scheme-published pacs / camt versions so the synthetic flow has a stable shape; (3) commission the three-way recon harness against that synthetic flow, reading camt.053 / camt.054 as truth, so Bea has a closeable cash-leg the moment her posting engine lands. SWIFT CBPR+, Strate (Joint Notice 2 of 2024, 1 March 2027), PASA settlement finality and Banks Act 94 of 1990 prudential obligations remain ingested but downstream of the SAMOS rehearsal; CLS stays `n/a-yet` until Saskia / Eitan open the first FX position.
