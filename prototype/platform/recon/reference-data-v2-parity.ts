...
//
// recon:reference-data-v2-parity — Wave 2 batch-1 parity gate
// (D-BANK-WIDE-V2-MIGRATION).
//
// PURPOSE
// -------
// Proves byte-equivalence of the four MONEY-FREE reference-data domains migrated
// in Wave 2 batch-1 (regulatory, market-data, obligation-review,
// obligation-equivalence — 19 event types) between:
...
// (`foldReferenceDataRegister`): the sorted-by-event_id set of
// {event_id, type, payload} tuples scoped to the batch's 19 types. Because the
...
// byte-divergence surfaces as a parity fail. This is the same harness
// (`runParityCheck`) and the same v1→v2 read direction as the posture pilot.
//
// STATUS
// ------
// ENFORCING (Wave 2 batch-1 flip, 2026-06-16). All 19 types flipped
...
//
// CLEAN-STORE BEHAVIOUR: on a fresh store with no reference-data events, BOTH
// sides fold to an empty register → byte-equal → pass (vacuously). The gate
// becomes load-bearing once the V1 seeds + the generic backfill have run (the
// ci:migrate chain does exactly that).
//
...
import {
  REFERENCE_DATA_BATCH_1_TYPES,
  type ReferenceDataEventView,
  type ReferenceDataRegister,
  foldReferenceDataRegister,
} from "../../v2-core/reference-data";
...
const PIPELINE = "reference-data-v2-parity";

const BATCH_TYPE_SET: ReadonlySet<string> = new Set<string>(REFERENCE_DATA_BATCH_1_TYPES);
...
function readV1Register(): ReferenceDataRegister {
  const views: ReferenceDataEventView[] = [];
  for (const type of REFERENCE_DATA_BATCH_1_TYPES) {
  ...
  return foldReferenceDataRegister(views, BATCH_TYPE_SET);
}
...
function foldV2(events: readonly CpEvent[]): ReferenceDataRegister {
  const views: ReferenceDataEventView[] = events.map((ev) => ({
...
export function run(overridePath?: string): ReconResult {
  ...
  const vacuous = runParityCheck({
    label: "reference-data-v2-parity:structural-check",
    readV1: () => ({}),
    readV2: () => ({}),
  });
  if (vacuous.length > 0) {
    violations.push({
      subject: "reference-data-v2-parity:harness-structural-check",
      message:
  ...
  try {
    ...
    violations.push({
      subject: "reference-data-v2-parity:fold-error",
      message: `reference-data register fold threw: ${err instanceof Error ? err.message : String(err)}. Inspect the V1 store and the v2 control-plane store for malformed reference-data events.`,
      severity: "fail",
    });
  }
  ...
  result.asOf =
    `reference-data-v2-parity [ENFORCING]: ${REFERENCE_DATA_BATCH_1_TYPES.length} type(s); ` +
