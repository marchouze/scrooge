// tests/ba-325-xml-adapter.test.ts
//
// Unit tests for the BA 325 (LCR) XML adapter.
//
// Tests:
//   1. ba325ToXmlPayload() — produces a SarbXmlReportPayload with formId "BA325".
//   2. ba325ToXmlPayload() — body.Meta contains Form, Entity, PeriodId, etc.
//   3. ba325ToXmlPayload() — body.Hqla contains TotalStockHqlaMinor.
//   4. ba325ToXmlPayload() — body.CashFlows contains NetCashOutflowsMinor.
//   5. ba325ToXmlPayload() — LcrRatio is mapped (finite + Infinity cases).
//   6. ba325ToXmlPayload() — LcrCompliant is mapped correctly.
//   7. renderSarbXml() round-trip — output is valid XML with BA325 root element.
//   8. validateSarbXmlStructural() — passes for well-formed BA 325 XML.
//   9. validateSarbXmlStructural() — catches missing required elements.
//  10. ba325ToXmlPayload() — optional trialBalanceSnapshotEventId is forwarded.
//
...
import type { Ba325Output } from "../platform/reporting/ba-325-lcr";
...
import {
  ...
  BA_325_XSD_URI,
  ba325ToXmlPayload,
} from "../platform/reporting/ba-325-xml-adapter";
...
function makeMinimalBa325Output(): Ba325Output {
  return generateBa325Lcr({
    entity: ENTITY,
    asOf: AS_OF,
    periodId: PERIOD_ID,
...
describe("ba325ToXmlPayload()", () => {
  ...
  it("TC-2: formVersion, xsdUri, namespaceUri are set correctly", () => {
    ...
    expect(payload.xsdUri).toBe(BA_325_XSD_URI);
    expect(payload.namespaceUri).toBe(BA_325_NAMESPACE);
  });
...
describe("ba325ToXmlPayload() → renderSarbXml() round-trip", () => {
  it("TC-11: produces well-formed XML with BA325 root element", () => {
    ...
    expect(xml).toContain("</BA325>");
    expect(xml).toContain(`xmlns="${BA_325_NAMESPACE}"`);
  });

  it("TC-12: validateSarbXmlStructural() passes for well-formed BA 325 XML", () => {
    const output = makeMinimalBa325Output();
    const payload = ba325ToXmlPayload(output);
    const xml = renderSarbXml(payload, { renderedAt: AS_OF });
    const result = validateSarbXmlStructural({
      xml,
      formId: "BA325",
      namespaceUri: BA_325_NAMESPACE,
      requiredElements: BA_325_REQUIRED_ELEMENTS,
    });
  ...
  it("TC-13: validateSarbXmlStructural() catches missing required elements", () => {
    ...
    const result = validateSarbXmlStructural({
      ...
      formId: "BA325",
      namespaceUri: BA_325_NAMESPACE,
      requiredElements: [...BA_325_REQUIRED_ELEMENTS, "NonExistentSection"],
    });
