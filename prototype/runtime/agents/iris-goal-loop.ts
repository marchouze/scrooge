...
// ---------------------------------------------------------------------------
const IRIS_BRIEF_DISPATCH: GoalLoopBriefDispatchConfig = {
  agentSlug: "iris",
  selfExecutablePattern: /popia[\s-]?controls?/i,
  deliveredClassLabel: "POPIA-controls snapshot attestation",
  runHandler: irisPopiaControlsSnapshot,
};
...
const handler = async (ctx: AgentRunContext): Promise<AgentRunOutput> => {
  ...
  const openBriefs = shouldRunHandler && !ctx.dryRun ? openBriefsListForAgent("iris") : [];
  ...
  if (brief) {
    ...
    logger.info(
      ...
      "iris:goal-loop — run complete (brief-bound dispatch)",
    );
    return {
  ...
  const handlerCtx: AgentRunContext = {
    ...
    logger.info(
      ...
      "iris:goal-loop — cohort-3 run complete (cadence, instrumented)",
    );
    return {
  ...
  logger.info(
    ...
    "iris:goal-loop — cohort-3 run complete (deferred)",
  );
