// tests/ba-200-credit-risk.test.ts
//
// D-REPORTING-CAPABILITY-M2-M3-BUILD-PLAN Slice 11 — exit-criterion tests
// for the BA 200 (Credit-Risk Loans-and-Advances Breakdown) generator +
...
//      all 3 IFRS 9 stages) → generateBa200CreditRisk → renderBa200ToJson
...
//   9. Schema validation — rendered output validates against Ba200RenderSchema.
 ...
 */
const FIXTURE_PORTFOLIO: readonly Ba200LoanEntry[] = [
...
const FIXTURE_CLASSIFICATION_MAP: readonly Ba200ClassificationEntry[] = [
  {
    productCategory: "interbank",
...
const BASE_INPUT: Ba200GeneratorInput = {
...
describe("BA 200 — per-entity isolation", () => {
  it("accepts LE-ZA-HOZ-BANK", () => {
    expect(() => generateBa200CreditRisk(BASE_INPUT)).not.toThrow();
  });

  it("rejects LE-ZA-HOZ-SECURITIES", () => {
    expect(() =>
      generateBa200CreditRisk({ ...BASE_INPUT, entity: "LE-ZA-HOZ-SECURITIES" }),
    ).toThrow(Ba200GeneratorError);
  });

  it("rejects LE-ZA-HOZ-GROUP", () => {
    expect(() => generateBa200CreditRisk({ ...BASE_INPUT, entity: "LE-ZA-HOZ-GROUP" })).toThrow(
      Ba200GeneratorError,
    );
  });

  it("rejects arbitrary entity string", () => {
    expect(() => generateBa200CreditRisk({ ...BASE_INPUT, entity: "LE-ZA-UNKNOWN" })).toThrow(
      Ba200GeneratorError,
    );
  });

  it("BA_200_BANK_ENTITIES contains only LE-ZA-HOZ-BANK", () => {
    expect(BA_200_BANK_ENTITIES).toEqual(["LE-ZA-HOZ-BANK"]);
  });
});
...
describe("BA 200 — end-to-end known shapes", () => {
  const out = generateBa200CreditRisk(BASE_INPUT);

  it("meta fields are correct", () => {
    expect(out.meta.form).toBe("BA 200");
  ...
  it("corporate category aggregates correctly (3 entries)", () => {
    ...
    expect(corp?.ba200RowLabel).toBe("Loans and advances — corporate");
    expect(corp?.ba200SubCode).toBe("LA-CORP");
  });
...
describe("BA 200 — ECL invariant", () => {
  ...
  it("throws when stage-1 ACL does not equal ecl12Month (violation)", () => {
    ...
    expect(() => generateBa200CreditRisk({ ...BASE_INPUT, portfolio: badPortfolio })).toThrow(
      Ba200GeneratorError,
    );
  });
...
describe("BA 200 — stage aggregation", () => {
  const out = generateBa200CreditRisk(BASE_INPUT);
...
describe("BA 200 — coverage ratio", () => {
  const out = generateBa200CreditRisk(BASE_INPUT);
...
describe("BA 200 — classification gaps", () => {
  it("surfaces gaps for unmapped categories but does not throw", () => {
    const portfolioWithGap: Ba200LoanEntry[] = [
      ...FIXTURE_PORTFOLIO,
      {
    ...
    const out = generateBa200CreditRisk({
      ...BASE_INPUT,
  ...
  it("gap entry's row label contains TBC marker", () => {
    ...
    expect(gapCategory?.ba200RowLabel).toMatch(/citation: TBC/);
...
describe("BA 200 — determinism", () => {
  it("same input produces byte-identical canonical JSON", () => {
    const out1 = generateBa200CreditRisk(BASE_INPUT);
    const out2 = generateBa200CreditRisk(BASE_INPUT);
    const { canonicalJson: j1 } = renderBa200Canonical(out1, { renderedAt: RENDERED_AT });
    const { canonicalJson: j2 } = renderBa200Canonical(out2, { renderedAt: RENDERED_AT });
    expect(j1).toBe(j2);
  });
  ...
  it("canonical JSON is deterministic regardless of portfolio entry order", () => {
    ...
    const out1 = generateBa200CreditRisk(BASE_INPUT);
    const out2 = generateBa200CreditRisk({ ...BASE_INPUT, portfolio: reversed });
    const { canonicalJson: j1 } = renderBa200Canonical(out1, { renderedAt: RENDERED_AT });
    const { canonicalJson: j2 } = renderBa200Canonical(out2, { renderedAt: RENDERED_AT });
    expect(j1).toBe(j2);
  });
...
describe("BA 200 — schema validation", () => {
  it("rendered output validates against Ba200RenderSchema", () => {
    const out = generateBa200CreditRisk(BASE_INPUT);
    const render = renderBa200ToJson(out, { renderedAt: RENDERED_AT });
    expect(() => Ba200RenderSchema.parse(render)).not.toThrow();
  });

  it("rendered output has correct $schema URL", () => {
    const out = generateBa200CreditRisk(BASE_INPUT);
    const render = renderBa200ToJson(out, { renderedAt: RENDERED_AT });
    expect(render.$schema).toBe(BA_200_SCHEMA_URL);
  });
...
describe("BA 200 — empty portfolio", () => {
  const emptyInput: Ba200GeneratorInput = {
    ...BASE_INPUT,
    portfolio: [],
  };

  it("nplRatio and coverageRatio are Infinity when portfolio is empty", () => {
    const out = generateBa200CreditRisk(emptyInput);
...
describe("BA 200 — boundary errors", () => {
  it("throws on negative grossCarryingAmount", () => {
    const first = FIXTURE_PORTFOLIO[0];
    if (!first) throw new Error("fixture is empty");
    const bad: Ba200LoanEntry[] = [{ ...first, grossCarryingAmount: -1 }];
    expect(() => generateBa200CreditRisk({ ...BASE_INPUT, portfolio: bad })).toThrow(
      Ba200GeneratorError,
    );
  });
  ...
  it("throws on negative allowanceForCreditLoss", () => {
    const first = FIXTURE_PORTFOLIO[0];
    if (!first) throw new Error("fixture is empty");
    const bad: Ba200LoanEntry[] = [{ ...first, allowanceForCreditLoss: -1 }];
    expect(() => generateBa200CreditRisk({ ...BASE_INPUT, portfolio: bad })).toThrow(
      Ba200GeneratorError,
    );
  });
  ...
  it("throws on empty functionalCurrency", () => {
    expect(() => generateBa200CreditRisk({ ...BASE_INPUT, functionalCurrency: "" })).toThrow(
      Ba200GeneratorError,
    );
  });

  it("throws on 2-char functionalCurrency (not ISO-4217)", () => {
    expect(() => generateBa200CreditRisk({ ...BASE_INPUT, functionalCurrency: "ZA" })).toThrow(
      Ba200GeneratorError,
    );
  });
});
...
describe("BA 200 — citations and placeholders", () => {
  const out = generateBa200CreditRisk(BASE_INPUT);

  it("includes BA-200-related citations", () => {
...
// =====================================================================
// 13. canonicaliseBa200 — keys sorted
// =====================================================================

describe("BA 200 — canonicaliseBa200", () => {
  it("produces valid JSON", () => {
    const out = generateBa200CreditRisk(BASE_INPUT);
    const render = renderBa200ToJson(out, { renderedAt: RENDERED_AT });
    const canonical = canonicaliseBa200(render);
    expect(() => JSON.parse(canonical)).not.toThrow();
  });

  it("produces identical bytes for two independent renders", () => {
    const out = generateBa200CreditRisk(BASE_INPUT);
    const r1 = renderBa200ToJson(out, { renderedAt: RENDERED_AT });
    const r2 = renderBa200ToJson(out, { renderedAt: RENDERED_AT });
    expect(canonicaliseBa200(r1)).toBe(canonicaliseBa200(r2));
  });
});
