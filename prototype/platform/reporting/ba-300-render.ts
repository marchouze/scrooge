// platform/reporting/ba-300-render.ts
//
// WS-FINANCE-BA-RETURNS-QUINTET — JSON renderer for the BA 300 (Income
...
//
// Determinism mirror of `ba-700-render.ts` / `ba-100-render.ts`.
//
...
import type { Ba300IncomeStatement } from "./ba-300-income-statement";
...
export const Ba300RenderSchema = z.object({
  $schema: z.literal("https://hoz.bank/schemas/ba-300/v0.1-rehearsal.json"),
  meta: z.object({
...
export type Ba300Render = z.infer<typeof Ba300RenderSchema>;

export const BA_300_SCHEMA_URL = "https://hoz.bank/schemas/ba-300/v0.1-rehearsal.json";
export const BA_300_RENDERER_VERSION = "v0.1" as const;

// ---------------------------------------------------------------------------
// Rendering
// ---------------------------------------------------------------------------

export interface RenderBa300Options {
  readonly renderedAt: string;
}

export function renderBa300ToJson(
  output: Ba300IncomeStatement,
  opts: RenderBa300Options,
): Ba300Render {
  const meta: Ba300Render["meta"] = {
    ...
    rendererVersion: BA_300_RENDERER_VERSION,
  ...
  return Ba300RenderSchema.parse(candidate);
}

export function canonicaliseBa300(render: Ba300Render): string {
...
export function renderBa300Canonical(
  output: Ba300IncomeStatement,
  opts: RenderBa300Options,
): {
  readonly render: Ba300Render;
  readonly canonicalJson: string;
  readonly canonicalBytes: Uint8Array;
} {
  const render = renderBa300ToJson(output, opts);
  const canonicalJson = canonicaliseBa300(render);
