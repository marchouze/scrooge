...
|---|---|---|---|---|
| 1 | **Credit application.** Saskia (derivatives trading desk, engineering) submits a `CreditLimitApplicationSubmitted` event specifying: counterparty ID (from party register), requested limit amount and currency, proposed tenor (revolving / term), product types (IRS, CRS, bonds, etc.), and commercial rationale. | `agent` (Saskia) | `@platform/party-register` ✓ + `@platform/risk/credit-limit-engine` (`PLANNED`) | Counterparty must already be onboarded via `counterparty-onboarding-markets.md` (PROC-MK-CO-01) before a credit application can be submitted. |
...
| Saskia (derivatives trading desk, engineering) | Initiates credit applications; operates within approved limits |
