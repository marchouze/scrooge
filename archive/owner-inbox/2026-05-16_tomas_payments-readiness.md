...
asOf: 2026-05-16T04:46:24.685Z
decision-required: false
---

# Tomas — daily payments-readiness, 2026-05-16
...
Latest `OperationalResilienceSnapshot` event: 2026-05-14T05:54:29.365Z
...
|---|---|---|---|
| `capability:samos-rtl-gateway` (SAMOS-RTL gateway (RTGS message exchange)) | specified | @platform/payments/samos-connector — synthetic pacs.009 / pacs.002 round-trip against SAMOS-rehearsal endpoint. Indirect-participant: sponsor-bank API seam may collapse to a thin adapter. | Draft synthetic SAMOS gateway with pacs.009 envelope shape; pair to cut-off engine 06:00 / 16:00 SAST windows. |
...
Payments substrate is **specified-not-deployed across the board, zero live events, build phase**. Eight of nine capabilities have a substrate path drafted or specified; the load-bearing block on Devon's first end-to-end resilience rehearsal is `capability:samos-rtl-gateway` — until a synthetic pacs.009 / pacs.002 round-trip clears the cut-off engine at the 06:00 / 16:00 SAST SAMOS windows (per SARB NPSD circulars under the NPS Act 78 of 1998 § 4/4A), there is no payment-leg to rehearse against. The indirect-participant model collapses real build work to a single seam: the sponsor-bank API adapter stands in for direct SAMOS, BankservAfrica, and CLS membership, so the binding artefacts are the sponsor contract and the scheme-rulebook ingestion, not a SARB / PASA participant application. Banks Act 94 of 1990 obligations attach via the sponsor relationship, not directly, in this phase.

Three consequential observations, ranked by load-bearing weight on the SAMOS rehearsal. **First**, `capability:iso-20022-pacs-camt` is one ticket from green — schemas are drafted but not yet pinned to the current scheme-published version, and every other connector (SAMOS, Bankserv, SWIFT, reconciler) inherits its envelope from this library; the version-pin is the unblock for the synthetic SAMOS gateway and for camt.053 / camt.054 as recon source-of-truth. **Second**, `obligation:swift:csp` has no owning capability registered today — CSP 2024 control framework attestation activates at first BIC, co-owned with Senna, but until the BIC application is lodged under `capability:swift-mt-mx` the attestation cycle has no anchor and the obligation drifts; flag this to Devon as an obligation-without-capability gap, not a control failure. **Third**, the three-way recon (`capability:gl-ledger-reconciliation`) contract is specified but not drafted, and Bea's first cash-leg posting cannot be projected until trade-leg ↔ payment-leg ↔ ledger-leg closes; this is the cut-off / reconciliation gap that gates her first synthetic posting cycle. PASA settlement standards (finality, operating windows) are encoded in the calendar engine spec but not exercised against a synthetic Bankserv cycle.

Next engineering move, in order: (1) pin ISO 20022 envelope schemas to the current scheme-published `pacs.008 / pacs.009 / pacs.002 / camt.053 / camt.054` versions in `@platform/payments` — this single PR unblocks the next two; (2) draft the synthetic SAMOS gateway in `@platform/payments/samos-connector` against the rehearsal endpoint with pacs.009 outbound / pacs.002 inbound, wired to the cut-off engine's 06:00 / 16:00 SAST windows (± 15 min per spec § 9, per SARB NPSD circulars on SAMOS operating hours); (3) commission the three-way recon harness in `@platform/payments/reconciliation` against the first synthetic SAMOS round-trip, with camt.053 as ledger-leg truth — co-signed with Bea and Anya. Strate connector spec and BankservAfrica scheme-cycle table follow once the SAMOS gateway has cleared one synthetic cycle; CLS stays deferred until a first FX position appears on Saskia / Eitan's book. Zero payments-domain events this week is the expected build-phase signal — the dominant signal remains the substrate-ticket queue between today and Saskia's pre-licence go-live readiness gate.
