...
/** Append TradeMatured (FX-spot variant) for a given tradeId and legKind. */
...
describe("TC-FWD-5: settled forward excluded", () => {
  it("skips a forward with TradeMatured (FX-spot) (near leg)", () => {
    const store = makeStore();
...
describe("TC-SWP-3: FX-swap fully settled — excluded", () => {
  it("skips trade when far leg TradeMatured (FX-spot) exists", () => {
    const store = makeStore();
...
describe("TC-NDF-3: NDF settled — excluded", () => {
  it("skips NDF with TradeMatured (FX-spot)", () => {
    const store = makeStore();
