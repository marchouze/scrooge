...
date: 2026-05-11
summary: Standalone Capital Management Policy covering CET1/AT1/T2 structure, capital targets, RAS B1/B2 anchoring, ICAAP governance, Pillar 2A add-ons, capital conservation and countercyclical buffers, stress-testing cadence, and distribution controls. Closes obligations ORG-PR-01 through ORG-PR-05, ORG-PR-13, ORG-PR-37, ORG-PR-44, ORG-PR-46 (PARTIAL — SA threshold sections; IRB sections NOT_APPLICABLE), ORG-PR-31 (PARTIAL — SA implementation date; IRB implementation date NOT_APPLICABLE). LICENCE-BIND.
...
- **Events-first capital accounting.** Capital ratios are computed as queries over the event log, not as stored balances in a parallel ledger (Principle 1). The RWA engine (W2 Slice 3, under Bea (Accounting & financial reporting engineer, engineering)) produces credit, market, and operational RWA from event substrates; the BA-form generator produces the PA-required BA 700 (capital adequacy + leverage) / BA 200 (credit risk) / BA 320 (market risk) / BA 400 (operational risk) returns in the SARB-published schema.
- **Pillar 1 credit risk — Standardised Approach election.** Credit risk RWA (BA 200) is computed under the Standardised Approach (SA) for credit risk as set out in Regulation 23 of the Regulations Relating to Banks. The Bank has not applied for, and does not operate under, the Foundation or Advanced Internal Ratings-Based (IRB) approach; Regulation 23(10)(a) (prior SARB written approval for IRB) is not applicable. This election is recorded as `D-CREDIT-RISK-CAPITAL-APPROACH-V1` (CEO-approved 2026-05-28). Asset-class classification thresholds (retail, QRRE, corporate SME) are applied per D8/2023 (`ORG-PR-46`). The revised SA for credit risk implementation date per G3/2023 (`ORG-PR-31`) governs the Basel III/IV transition; the IRB implementation component of G3/2023 does not apply to this Bank.
...
| `ORG-PR-46` | D8/2023 Threshold amounts — revised SA + IRB approaches for credit risk + liquidity + IRRBB (replaces D1/2016) | **IN FORCE — PARTIAL.** SA threshold sections (§2 retail ≤R12.5m, §3 QRRE ≤R1.5m, §4–§5 corporate SME, §9 securitisation retail, §10 LCR/NSFR small-business, §11 IRRBB retail-deposit) — **closed.** IRB-specific threshold sections (§6 AIRB cut-off >R15bn, §7 AVC ≥R1.2tn, §8 FIRB effective maturity) — **NOT_APPLICABLE** (bank uses SA per `D-CREDIT-RISK-CAPITAL-APPROACH-V1`) | §1 (Standardised Approach election principle) |
| `ORG-PR-31` | G3/2023 Basel implementation dates — revised SA for credit risk + revised IRB approaches + FRTB + CVA + output floor | **IN FORCE — PARTIAL.** Revised SA for credit risk implementation date — **closed.** Revised IRB approaches for credit risk implementation date — **NOT_APPLICABLE** (bank uses SA per `D-CREDIT-RISK-CAPITAL-APPROACH-V1`) | §1 (Standardised Approach election principle) |
...
|---|---|---|---|
| v1.1 | 2026-05-28 | Scrooge (Chief of Staff) on behalf of CEO | §1 — new "Pillar 1 credit risk — Standardised Approach election" principle added, citing `D-CREDIT-RISK-CAPITAL-APPROACH-V1` (CEO-approved 2026-05-28). §6 obligations closure table — `ORG-PR-46` (PARTIAL — SA threshold sections closed; IRB sections NOT_APPLICABLE) and `ORG-PR-31` (PARTIAL — SA implementation date closed; IRB date NOT_APPLICABLE) rows added. Frontmatter summary updated accordingly. Policy alignment follows CEO decision to adopt the Standardised Approach for credit risk capital and decline IRB at this stage. |
