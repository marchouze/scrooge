...
const SECURITY_SUBSTRATE_STATE_GOAL = "Approve secure-SDLC pipeline configuration changes" as const;
...
export const sennaGoalDeriver: GoalDeriver = async (
  ...
  const procedureEntry = spec.procedureSteps.find((s) => s.procedurePath === SENNA_PROCEDURE_PATH);
  const stepId =
...
// ---------------------------------------------------------------------------
const SENNA_BRIEF_DISPATCH: GoalLoopBriefDispatchConfig = {
  agentSlug: "senna",
  selfExecutablePattern: /security[\s-]?substrate|substrate[\s-]?state/i,
  deliveredClassLabel: "security substrate-state attestation",
  runHandler: sennaSecuritySubstrateState,
};
...
const handler = async (ctx: AgentRunContext): Promise<AgentRunOutput> => {
  ...
  const openBriefs = shouldRunHandler && !ctx.dryRun ? openBriefsListForAgent("senna") : [];
  ...
  const handlerCtx: AgentRunContext = {
    ...
    const cadence = await dispatchCadenceRun(ctx, iterationId, SENNA_BRIEF_DISPATCH);
    logger.info(
      { agent: ctx.agent, iterationId },
      "senna:goal-loop — cohort-3 run complete (cadence, instrumented)",
    );
    return {
  ...
  logger.info(
    ...
    "senna:goal-loop — cohort-3 run complete (deferred)",
  );
