...
export const SUBSTRATE_GAP_REGISTER: readonly SubstrateGapRecord[] = [
  ...
  },
  {
    ...
    title:
      "BA 325 reg-29 FX residency-segmented detail block: NOW FOLDED (engine + sim); licence-day real-counterparty POPULATION pending",
    description:
      "BA 325 carries a large reg-29(3) foreign-currency detail block (R0610–R0750): commitments to purchase / sell foreign currency and the effective net-open-foreign-currency position, segmented by counterparty RESIDENCY (residents / non-residents / authorised dealers / SARB) × currency (USD/Euro/GBP/CHF/JPY/Other). NOW FOLDED (L2-FX, D-BA-RETURN-PER-PRODUCT-RICHNESS): platform/markets/fx/counterparty-residency.ts classifies each FX position's counterparty → residency bucket from the EVENT-SOURCED party register (jurisdictions / taxResidencies / classifications / kindAttributes.primaryRegulator) + client-onboarding register (jurisdiction / sector), validated against the SARB Currency & Exchanges Manual §A.1/§A.2 + Reg 29(3) external oracle; fail-closed on an unmappable counterparty. platform/reporting/ba-325-reg29-fx-residency-fold.ts folds the open FX FIL instance book into the residency × currency commitment blocks; the effective NOP (R0750) reconciles to the BA 320 FX sub-fold's net-open-position basis BY CONSTRUCTION (same open instances, same long−short net). Proven against the sim counterparty book (resident + non-resident + authorised-dealer + SARB legs) in ba-325-reg29-fx-residency-fold.test.ts; production-only provenance folds to the honest empty state. REMAINING DEFERRAL (DATA, not engine): the licence-day real-counterparty POPULATION — the residency buckets only carry real figures once real FX counterparties are onboarded at licence-day. The on-balance-sheet FC asset/liability rows (R0360–R0600) source from a separate FC nostro/deposit balance fold (not the FX trading book) and remain a named follow-on. Authority: D-BA-RETURN-PER-PRODUCT-RICHNESS; D-BA-RETURN-SIMULATOR-FIRST; Engineering Charter cmd 5; Reg 29(3) read with Reg 28(5).",
    severity: "medium",
    status: "in-flight",
    mitigation: "partial",
  },
