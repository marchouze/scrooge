...
describe("bea goal-loop — blocked-class classification", () => {
  // We verify the blocked gate via the exported `isSelfExecutableByBea`
  // classifier. A brief that is NOT self-executable closes its brief-bound run
  // blocked with followOnRoutes: [] — terminal, no follow-on brief issued. The
  // dispatch-level emission tests live in goal-loop-brief-dispatch.test.ts.
