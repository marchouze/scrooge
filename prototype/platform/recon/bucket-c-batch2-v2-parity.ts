...
//
// recon:bucket-c-batch2-v2-parity — Bucket C bulk batch-2 parity gate
// (D-BANK-WIDE-V2-MIGRATION).
//
// PURPOSE
// -------
// Proves byte-equivalence of the 68 non-load-bearing bucket-C substrate types
// (CISO/security-process + party/product-lifecycle + legal/privacy-process;
// sourced from BUCKET_C_BATCH2_TYPES) between:
...
// recon:bucket-c-batch1-v2-parity (#1408); identical comparable shape + harness.
//
...
// the money-free batches, and bucket-C batch 1.
//
// STATUS
// ------
// ENFORCING (bucket-C batch-2 flip, 2026-06-17). All 68 types flipped
...
// where real party/product/security/legal populations live). Because the codec
...
import { BUCKET_C_BATCH2_TYPES } from "../../v2-core/bucket-c-batch2";
...
const PIPELINE = "bucket-c-batch2-v2-parity";

const TYPE_SET: ReadonlySet<string> = new Set<string>(BUCKET_C_BATCH2_TYPES);
...
function readV1Tuples(): readonly VerbatimTuple[] {
  ...
  for (const type of BUCKET_C_BATCH2_TYPES) {
...
export function run(overridePath?: string): ReconResult {
  ...
  const vacuous = runParityCheck<readonly VerbatimTuple[]>({
    label: "bucket-c-batch2-v2-parity:structural-check",
    readV1: () => [],
    readV2: () => [],
  });
  if (vacuous.length > 0) {
    violations.push({
      subject: "bucket-c-batch2-v2-parity:harness-structural-check",
      message:
  ...
  try {
    ...
    const parity = runParityCheck<readonly VerbatimTuple[]>({
      label: "bucket-c-batch2-verbatim-tuples",
      readV1: () => v1Snap,
      readV2: () => v2Snap,
    });
    ...
    violations.push({
      subject: "bucket-c-batch2-v2-parity:fold-error",
      message: `bucket-c batch-2 verbatim tuple fold threw: ${err instanceof Error ? err.message : String(err)}. Inspect the V1 store and the v2 control-plane store for malformed events.`,
      severity: "fail",
    });
  }
  ...
  result.asOf =
    `bucket-c-batch2-v2-parity [ENFORCING]: ${BUCKET_C_BATCH2_TYPES.length} type(s); ` +
