...
//   4. BA 325 output is compliant (lcr ≥ 1.0) with custodian-derived SARB cash.
//
...
import { ba325PeriodCloseSubscriber } from "./period-close-subscriber";
...
describe("BA325 period-close subscriber — custodian-derived cash HQLA", () => {
  ...
  it("drops the cash line when the custodian confers no HQLA status (derived, not authored)", () => {
    ...
    expect(result.ba325Output.hqla.level1.stockMinor).toBe(0);
    expect(result.ba325Output.hqla.totalStockHqlaMinor).toBe(0);
    // 0 HQLA against a 50,000 outflow → not LCR-compliant.
    expect(result.ba325Output.lcrCompliant).toBe(false);
  });
});
