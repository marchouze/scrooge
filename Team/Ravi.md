...
| v1.8 | 2026-06-26 | Ravi (via Scrooge) | Domain-competence layer added (D-AGENT-DOMAIN-COMPETENCE, PROC-GOV-ADC-01; D-ADC-RISK-TREASURY-AUDIT-SEAT-UPGRADE; brief `brief:ravi:adc-18-20-upgrade-ravi-alm-quant-bind-bcbs-248-i:2026-06-26`). §§18–20 authored. BCBS ALM/liquidity standards bound as citable domain-truth oracles against real `Regulations/BCBS/` graph nodes — IRRBB ⇒ consolidated **SRP31** (`urn:reg:bcbs:srp:31`, legacy **BCBS d368** April 2016); LCR ⇒ `urn:reg:bcbs:lcr` (BCBS **d295**); NSFR ⇒ `urn:reg:bcbs:nsf` (BCBS **d335**); intraday ⇒ `ORG-PR-08` (BCBS **248** / **d575** family — source-doc STUB gap flagged). §20 CHALLENGES two imprecise premises in the framing: NSFR is **d335** (not d295/d365), IRRBB is **d368** (not d365); and records the live-code domain-truth finding that `platform/alm/repricing-gap.ts` mis-cites IRRBB as **d365** (correct = **d368**), to be fixed under a follow-on task. |
...
| `Regulations/_risk-taxonomy.md` rows `RT-IRRBB`, `RT-LQ`, `RT-LQ.IN` | Bank risk taxonomy (internal oracle) | `urn:risk:bank:RT-IRRBB`, `urn:risk:bank:RT-LQ`, `urn:risk:bank:RT-LQ.IN` | The bank's own authoritative mapping of these risks to their governing BCBS document numbers — the in-repo cross-check Ravi validates citations against (it independently records IRRBB = d368, LCR = d295, NSFR = d335, intraday = 248). |

- **Standards (authoritative oracles):** SRP31 (IRRBB, d368), LCR (d295), NSF (d335), BCBS 248 (intraday). These are the bodies of rule every ALM ratio, shock scenario, and HQLA classification Ravi publishes MUST conform to — validated against the standard's own numbers, not against internal balance.
...
- **(b) Golden worked-example library** — input/expected-output cases the engines must reproduce exactly:
  ...
  | LCR HQLA classification | BCBS d295 (`platform/collateral/`) | L1 / L2a / L2b eligibility + haircut |
...
- **Confirm-or-challenge gate — this dispatch:** the brief's *core premise* — that the BCBS ALM/liquidity standards must be bound as citable domain-truth oracles, with document numbers verified, not as §4 prose — is **CONFIRMED** (coherent with `D-AGENT-DOMAIN-COMPETENCE` and Principle 2). But two specifics in the brief's framing are **CHALLENGED** against §18, per its own §20 instruction that the seat's domain authority outranks the brief:
  - **CHALLENGE 1 — NSFR is BCBS d335, not "d295/d365".** The brief writes "NSFR (BCBS d295/d365 NSFR)". d295 is the **LCR** standard (Jan 2013); d365 is a 2014 operational-risk consultative paper, not NSFR. The NSFR final standard is **BCBS d335** (*Basel III: the net stable funding ratio*, Oct 2014), consolidated as **NSF** (`urn:reg:bcbs:nsf`). Corroborated by the bank's own `Regulations/_risk-taxonomy.md` `RT-LQ` row (NSFR = D335). §18 cites d335.
  ...
  - The brief's correct citations are confirmed: **LCR = BCBS d295** ✓ and **intraday = BCBS 248** ✓.
- **Live-code domain-truth finding (recorded, not dropped):** `platform/alm/repricing-gap.ts` currently mis-cites IRRBB as **"BCBS d365"** in its header and authority line (the ΔEVE engine `eve.ts` correctly cites d368). This is the exact wrong-but-plausible citation the brief warned of, present in the seat's own engine. Flagged for a same-domain follow-on fix (d365 → d368; and tighten the "BCBS 319" repricing-bucket reference to the d368 final standard). It does not affect the numbers the engine produces, only the citation provenance — but a wrong citation is a Principle-2 finding regardless.
