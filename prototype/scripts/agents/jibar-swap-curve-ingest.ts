// scripts/agents/jibar-swap-curve-ingest.ts
//
// JIBAR swap curve ingest script — loads the build-phase fixture and stores
// JIBAR swap curve daily snapshot ticks in the MarketDataStore (SQLite).
//
// This file exposes two entry points:
//   - `runJibarSwapCurveIngestForDate()` — pure synchronous function; used by
//     the Ravi scheduled handler `ravi:jibar-swap-curve-ingest`.
//   - `runJibarSwapCurveIngestAllDates()` — seeds all historical dates from the
...
//
// Usage:
//   bun run scripts/agents/jibar-swap-curve-ingest.ts [YYYY-MM-DD]
//   bun run scripts/agents/jibar-swap-curve-ingest.ts --all
...
export interface JibarSwapCurveIngestOptions {
...
export interface JibarSwapCurveIngestAllOptions {
  /** Path to the MarketDataStore SQLite file. */
  dbPath: string;
  /**
   * Logging sink. Defaults to `console.log`/`console.error`.
...
function loadFixture(): JibarSwapCurveFixtureShape {
  const fixturePath = resolve(import.meta.dir, "../../seeds/jibar-swap-curve.json");
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  return require(fixturePath) as JibarSwapCurveFixtureShape;
}
...
/**
 * Ingest the JIBAR swap curve snapshot for a single date from the build-phase
 * fixture. Synchronous — no network calls in the build phase.
 */
export function runJibarSwapCurveIngestForDate(
  opts: JibarSwapCurveIngestOptions,
): JibarSwapCurveIngestResult {
  const log =
    opts.log ?? ((level, msg) => (level === "error" ? console.error(msg) : console.log(msg)));

  let fixture: JibarSwapCurveFixtureShape;
  try {
    fixture = loadFixture();
  } catch (err) {
    const msg = `fixture load failed — ${String(err)}`;
    log("error", `[jibar-swap-curve-ingest] ERROR: ${msg}`);
  ...
  try {
    ...
    const result = runJibarSwapCurveIngest({ date: opts.date, source, marketDataStore: store });
    log(
      "info",
      `[jibar-swap-curve-ingest] date=${opts.date} appended=${result.ticksAppended} skipped=${result.ticksSkippedAsDuplicate}`,
    );
    return {
...
/**
 * Seed all historical JIBAR swap curve dates from the build-phase fixture.
 */
export function runJibarSwapCurveIngestAllDates(
  opts: JibarSwapCurveIngestAllOptions,
): JibarSwapCurveIngestResult {
  const log =
    opts.log ?? ((level, msg) => (level === "error" ? console.error(msg) : console.log(msg)));
  ...
  try {
    const source = makeFixtureJibarSwapCurveSource(fixture);
    const results = runJibarSwapCurveIngestAll({ source, marketDataStore: store });
    ...
    log(
      "info",
      `[jibar-swap-curve-ingest] all-dates: ${results.length} dates processed, ${totalAppended} appended, ${totalSkipped} skipped`,
    );
...
function main(): void {
  ...
  if (args[0] === "--all") {
    const result = runJibarSwapCurveIngestAllDates({ dbPath });
  ...
  const result = runJibarSwapCurveIngestForDate({ dbPath, date });
  if (!result.ok) process.exit(1);
}
