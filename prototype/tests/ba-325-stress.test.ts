// tests/ba-325-stress.test.ts
//
// Stress-test coverage for the BA 325 (LCR) SARB return projection.
// Closes pre-licence gate criterion G3.
//
// These tests exercise edge cases and boundary conditions in the BA 325
// generator and XML adapter that are not covered by the nominal-path tests
// in ba-325-xml-adapter.test.ts. Each test is an isolated pure-function call
...
//   ST-9:  ba325ToXmlPayload() round-trip — structural key check
//
...
import {
  ...
  applyHqlaCaps,
  generateBa325Lcr,
} from "../platform/reporting/ba-325-lcr";
...
function baseInput(overrides?: Partial<Ba325GeneratorInput>): Ba325GeneratorInput {
  return {
    entity: ENTITY,
    asOf: AS_OF,
    periodId: PERIOD_ID,
...
// verify via the Ba325Output interface: set totalStockHqlaMinor = 0 via
...
describe("BA 325 stress — ST-1: LCR = 0 scenario", () => {
  ...
  it("generator with empty HQLA + empty event store yields lcrRatio = Infinity (no outflows)", () => {
    ...
    const output = generateBa325Lcr(baseInput());
...
describe("BA 325 stress — ST-2: LCR = Infinity (zero denominator)", () => {
...
describe("BA 325 stress — ST-3: LCR < 80% (non-compliant)", () => {
...
describe("BA 325 stress — ST-4: Level-2A cap binding", () => {
...
describe("BA 325 stress — ST-5: Level-2B cap binding", () => {
...
describe("BA 325 stress — ST-6: Inflow 75%-cap binding", () => {
...
describe("BA 325 stress — ST-7: Net-outflow floor binding", () => {
...
describe("BA 325 stress — ST-8: Zero trial balance (empty arrays)", () => {
...
describe("BA 325 stress — ST-9: ba325ToXmlPayload() round-trip", () => {
  it("formId = 'BA325'", () => {
    const output = generateBa325Lcr(baseInput());
