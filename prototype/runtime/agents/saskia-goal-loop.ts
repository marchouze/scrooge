...
// ---------------------------------------------------------------------------
const SASKIA_BRIEF_DISPATCH: GoalLoopBriefDispatchConfig = {
  agentSlug: "saskia",
  selfExecutablePattern: /markets[\s-]?readiness/i,
  deliveredClassLabel: "markets-readiness snapshot attestation",
  runHandler: saskiaMarketsReadinessSnapshot,
};
...
const handler = async (ctx: AgentRunContext): Promise<AgentRunOutput> => {
  ...
  const openBriefs = shouldRunHandler && !ctx.dryRun ? openBriefsListForAgent("saskia") : [];
