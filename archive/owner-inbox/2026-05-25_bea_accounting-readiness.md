...
asOf: 2026-05-25T06:16:36.351Z
decision-required: false
---

# Bea — daily accounting readiness, 2026-05-25
...
The accounting substrate is at zero — no chart of accounts, no posting rules, no sub-ledger projection, and no events posted in the last seven days. The load-bearing block on Camille's first signed CloseApproved is the **monthly-close cycle**: until a sub-ledger projection exists over postable events with at least one published posting rule, none of the downstream cycles (BA return per Banks Act 94 of 1990 Reg 38, AFS per IAS 1, capital-plan refresh) can fire. All 26 Bea-owned obligations sit at PLANNED or DRAFTING — none IN FORCE, none even PARTIAL — which is the correct state for build phase but means every IFRS line (IFRS 9 classification and measurement, IFRS 7 disclosure, IFRS 13 fair-value hierarchy, IFRS 15/16 where they bind, IAS 12 deferred tax) is waiting on the same upstream substrate ticket.

The two most consequential observations: first, the monthly-close cycle is genuinely one engineering ticket away from a first synthetic green — a sub-ledger projection over postable events plus one posting rule will produce the first `SubLedgerEntryPosted` and unblock the CloseApproved producer. Second, the quarterly BA-return cycle is *substrate-blocked, not policy-blocked* — the BA 100 / 200 / 300 / 700 cell maps cannot be drafted as live artefacts until the sub-ledger they read from exists, so Anya's harness has nothing to bind to. The AFS engine and ECL staging cycle correctly defer; specifying their schemas in parallel costs nothing and saves a cycle later.

Next engineering move, in order: (1) specify the chart-of-accounts schema and commit it; (2) publish the first posting rule with Atlas — `FundingDrawn → Dr Cash / Cr Funding Liability` measured at amortised cost per IFRS 9 and presented per IAS 1, against the synthetic capital/funding line already in seeds; (3) build the sub-ledger projection over postable events so `SubLedgerEntryPosted` is emitted and the CloseApproved producer has something to close over; (4) in parallel, commission with Anya the BA 100 cell-map register against the same synthetic line per Banks Act Reg 38, so the BA-return harness lands the week the sub-ledger does. AFS line-item schema (IAS 1 structure, IFRS 7 disclosure pack, IAS 7 cash-flow derivation, IAS 12 deferred-tax bridge per Income Tax Act 58 of 1962) stays drafted-but-deferred until first close lands.
