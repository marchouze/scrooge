...
#### Two planes within the single graph

The "Regulation" source of the executable chain resolves into the **Regulatory Knowledge plane** (Plane A): a single reference graph of regulators, documents, provisions, and **source obligations** — what the world's regulations say — fed by pluggable extractors over one ontology (`D-REGULATORY-ARCHITECTURE-TWO-PLANE`). This is reference data, not events (Principle 1's reference-data rule).

The **obligations register** is the **Bank-Action plane** (Plane B): each bank obligation (`ORG-*`) is event-sourced — born from an `ObligationAdopted` decision and linked `DERIVES_FROM` the source obligation it implements — and carries the policy → procedure → capability lifecycle. Two new edges extend the chain: `DERIVES_FROM` (bank obligation → source obligation, the bridge between the planes) and `REALISES` (system capability → obligation, closing Rule 2's downward trace into live code). The single graph spans both planes; the bridge keeps it one graph. See `prototype/platform/regulatory/architecture.md`.
