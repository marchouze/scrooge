...
- **`appetite:liquidity:intraday` RAS appetite line** — the BCBS 248 metrics are now register-bound (see Closed below) but `platform/risk/ras-appetite-register.ts` still has no intraday-liquidity appetite line; the ZAR 50m intraday floor in `platform/alm/intraday-stress.ts` remains a build-phase constant pending `RASCalibrationChange` sourcing. Owner: Helena (Chief Risk Officer, governance) — calibration follow-on gated on the WS-TREASURER-WAVE1-SUBSTRATE merge; binds to `computeIntradayLiquidityMetrics` (`platform/alm/intraday-liquidity-metrics.ts`). Target: build-phase (Wave 1 follow-on).
