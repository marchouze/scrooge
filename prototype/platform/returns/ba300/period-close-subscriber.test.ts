...
// EventStore is not used by BA 300 (gross income is caller-supplied), but
// we keep the store instantiation for structural consistency with the
// BA 110 / BA 310 pattern. The subscriber does not take eventStore; the
// store reference is kept for potential future P1-compliant income fold.
// const _store = new EventStore(":memory:");  // BA400 does not use event store (gross income is caller-supplied)
...
describe("BA400 period-close subscriber — zero gross-income baseline", () => {
  ...
  it("all numeric cells are non-NaN", () => {
    ...
    expect(Number.isNaN(result.ba300Output.opRiskCapitalMinor)).toBe(false);
    expect(Number.isNaN(result.ba300Output.opRiskRwaMinor)).toBe(false);
    expect(Number.isNaN(result.ba300Output.bia?.capitalMinor ?? 0)).toBe(false);
    expect(Number.isNaN(result.ba300Output.bia?.nPositiveYears ?? 0)).toBe(false);
  });
  ...
  it("placeholders populated (rehearsal-grade marker)", () => {
    ...
    expect(result.ba300Output.placeholders.length).toBeGreaterThanOrEqual(1);
    expect(result.ba300Output.placeholders.some((p) => p.includes("[citation: TBC"))).toBe(true);
  });
});
...
describe("BA400 period-close subscriber — BIA arithmetic", () => {
  it("3 positive years ⇒ 15% × average; RWA = 12.5 × capital", () => {
    ...
    expect(result.ba300Output.bia?.nPositiveYears).toBe(3);
    expect(result.ba300Output.bia?.averagePositiveMinor).toBe(2_000_000);
    expect(result.ba300Output.bia?.capitalMinor).toBe(300_000); // 15% × 2m
    expect(result.ba300Output.opRiskCapitalMinor).toBe(300_000);
    expect(result.ba300Output.opRiskRwaMinor).toBe(3_750_000); // 12.5×
  });
  ...
  it("non-positive years excluded from BIA average", () => {
    ...
    expect(result.ba300Output.bia?.nPositiveYears).toBe(2);
    expect(result.ba300Output.bia?.averagePositiveMinor).toBe(2_000_000);
    expect(result.ba300Output.bia?.capitalMinor).toBe(300_000);
    expect(result.ba300Output.placeholders.some((p) => p.includes("2 positive years"))).toBe(true);
  });
  ...
  it("BIA aggregates gross income across business lines per year", () => {
    ...
    expect(result.ba300Output.bia?.averagePositiveMinor).toBe(1_000_000);
    expect(result.ba300Output.bia?.capitalMinor).toBe(150_000);
  });
  ...
  it("default approach is BIA when not specified", () => {
    ...
    expect(result.ba300Output.meta.approach).toBe("bia");
  });
});
...
describe("BA400 period-close subscriber — XML serialiser", () => {
  ...
  it("XML is byte-identical across runs (determinism)", () => {
    const makeXml = (): string => {
      ...
      return renderSarbXml(ba300ToXmlPayload(result.ba300Output), {
