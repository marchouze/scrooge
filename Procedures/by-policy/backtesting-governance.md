...
- **Related procedures:** `PROC-RISK-FRTB-SA-01` (`frtb-sa-capital-computation.md`) — SA capital applies on Red-zone entry; `PROC-RISK-PLA-01` (`pla-test-governance.md`) — RTPL divergence is a PLA test signal; `PROC-RISK-MRM-01` (`market-risk-monitoring.md`) — VaR estimate consumer; `PROC-RSK-MV-01` (`model-validation.md`) — Nadia's annual + ad-hoc validation; `PROC-FIN-BA-01` (`ba-return-generation.md`) — capital basis for BA-325 reflects the zone.
