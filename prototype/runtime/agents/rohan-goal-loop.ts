...
const handler = async (ctx: AgentRunContext): Promise<AgentRunOutput> => {
  ...
  if (shouldRunHandler && !ctx.dryRun) {
    const cadence = await dispatchCadenceRun(ctx, iterationId, ROHAN_BRIEF_DISPATCH);
    logger.info(
      { agent: ctx.agent, iterationId },
      "rohan:goal-loop — run complete (cadence, instrumented)",
    );
    return {
  ...
  logger.info(
    ...
    "rohan:goal-loop — run complete (deferred)",
  );
