// tests/ba-350-market-risk.test.ts
//
// D-REPORTING-CAPABILITY-M2-M3-BUILD-PLAN Slice 5 — exit-criterion tests
// for BA 350 (market risk) generator + XML render.
//
...
describe("D-REPORTING-CAPABILITY-SLICE-5 — BA 350 per-entity isolation", () => {
  it("BA_350_BANK_ENTITIES contains only LE-ZA-HOZ-BANK", () => {
    expect(BA_350_BANK_ENTITIES).toEqual(["LE-ZA-HOZ-BANK"]);
  });
...
describe("D-REPORTING-CAPABILITY-SLICE-5 — BA 350 sub-charge arithmetic", () => {
...
describe("D-REPORTING-CAPABILITY-SLICE-5 — BA 350 XML render round-trip", () => {
  ...
  it("XML is byte-identical across runs (determinism)", () => {
    ...
    const a = renderSarbXml(ba350ToXmlPayload(generateBa350MarketRisk(input)), {
      renderedAt: "2026-05-10T15:00:00.000Z",
    });
    const b = renderSarbXml(ba350ToXmlPayload(generateBa350MarketRisk(input)), {
      renderedAt: "2026-05-10T15:00:00.000Z",
    });
    expect(a).toBe(b);
  });
