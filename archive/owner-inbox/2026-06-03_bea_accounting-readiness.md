...
asOf: 2026-06-03T05:47:14.526Z
decision-required: false
---

# Bea — daily accounting readiness, 2026-06-03
...
The accounting substrate is empty — zero postings, zero classifications, zero close cycles fired, and all 64 Bea-owned obligations sit at PLANNED or DRAFTING with nothing yet IN FORCE or even PARTIAL. The load-bearing block on Camille's first signed CloseApproved is the **monthly close cycle**: until a sub-ledger projection exists over postable events and at least one posting rule is published, no downstream cycle (BA return per Banks Act Reg 38, AFS per IAS 1, capital-plan refresh) can fire end-to-end. Everything is specified; nothing is built.

The most consequential observation is that the monthly close is one engineering ticket away from having a spine: a sub-ledger projection over a `postable-events` stream, plus a single published posting rule, would turn the close engine from spec into a producer of `SubLedgerEntryPosted`. The synthetic `FundingDrawn → Dr cash / Cr funding-liability` rule (IFRS 9 initial recognition at amortised cost, IAS 1 presentation as a financial liability) is the natural first rule because it is the only economic event the seed substrate currently generates. Once that rule posts, the BA 100 cell-map and the IAS 1 line-item schema both have something real to project against; until then the AFS engine and ECL staging cycle are correctly deferred (IFRS 9 ECL has no exposure book to stage, and IFRS 7 disclosure needs a balance sheet to disclose).

Next engineering move, in order: (1) specify the chart-of-accounts schema and commit it to the substrate this cycle; (2) co-publish with Atlas the `FundingDrawn → Dr 1000-Cash / Cr 2100-FundingLiability` posting rule as the first entry in the posting-rule registry, tagged IFRS 9 / IAS 1; (3) build the sub-ledger projection over postable events so `SubLedgerEntryPosted` starts emitting; (4) commission with Anya the BA 100 cell-map register against the synthetic capital line per Reg 38, so the first `BAReturnGenerated` dry-run can fire the moment the sub-ledger projection lands. IAS 7 cash-flow, IAS 12 deferred tax (co-owned with Yael, Income Tax Act 58 of 1962), IFRS 13 fair-value hierarchy, IFRS 15/16 revenue and lease postings, and hedge accounting (co-owned with Ravi) remain correctly queued behind the first posted entry.
