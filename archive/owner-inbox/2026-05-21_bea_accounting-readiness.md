...
asOf: 2026-05-21T06:00:02.002Z
decision-required: false
---

# Bea — daily accounting readiness, 2026-05-21
...
Substrate is at zero — no postings, no classifications, no closes, no signed returns, and 14 of 26 Bea-owned obligations sit at PLANNED with 2 at DRAFTING and nothing yet IN FORCE. The load-bearing block on Camille's first CloseApproved is the **monthly close cycle**, and specifically the sub-ledger projection: until postable events fan out to a chart-of-accounts-aware ledger projection, neither the close orchestrator, the BA-return generator, nor the AFS engine has anything to read. Everything downstream of Camille's first signed close is queued behind this one projection.

Two observations rank above the rest. First, the monthly close is one engineering ticket from amber: the chart-of-accounts schema and the first posting rule (synthetic `FundingDrawn` → Dr cash / Cr funding-liability, per IFRS 9 initial recognition and IAS 1 presentation) are scoped, owners are named (Bea with Atlas), and the `CloseApproved` producer is wireable the moment the projection lands. Second, the BA 100 cell-map under Banks Act 94 of 1990 Reg 38 is the gating artefact for the quarterly return — it can be drafted today against the synthetic capital line without waiting for the sub-ledger, and doing so de-risks the first `BAReturnGenerated` dry-run. The AFS engine (IAS 1 structure, IFRS 7 disclosure pack) and the capital-plan refresh (CET1/RWA/Pillar 2A under Reg 38) both correctly defer behind the close; IFRS 9 ECL staging and IAS 12 deferred tax stay parked until Saskia books a counterparty and Yael's temporary-difference register has something to read.

Next engineering move, in order: (1) publish the chart-of-accounts schema and the `FundingDrawn` posting rule jointly with Atlas this cycle; (2) build the sub-ledger projection over postable events, with `SubLedgerEntryPosted` as the producer Vera will reconcile against; (3) commission the BA 100 cell-map register with Anya in parallel — cells mapped to sub-ledger account codes plus the synthetic capital line, so the harness is ready the day the projection emits its first entry. AFS line-item schema (IAS 1) gets drafted alongside, not before.
