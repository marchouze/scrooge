...
**Activated obligation status:** `ORG-PR-06` (BCBS D295 / BA 110) — **IN FORCE** from commencement of trading.

**Consolidated obligation:** `ORG-BNK-ILAAP-CONS` (consolidated ILAAP submission) — the bank entity submits a consolidated ILAAP to PA that aggregates liquidity across `Hoz Bank Limited` + `Hoz Securities Limited`. Both the solo and consolidated LCR are produced by the BA 110 projection substrate.
...
HQLA eligibility is assessed on the basis of the BCBS D295 §§ 49–66 criteria `[citation: TBC — precise paragraph indices]` and Reg 26(7) `[citation: TBC]`. The BA 110 LCR substrate at `prototype/platform/reporting/ba-110-lcr.ts` implements the closed-form cap arithmetic per BCBS D295 § 47.
...
| BA-return cadence | BA 110 submitted to PA per the SARB BA-return schedule (`ORG-PR-29` D2/2024) | Via the BA 110 LCR projection substrate; per [Owner Inbox/2026-05-10_bea-eitan-anya_d-reporting-capability-slice-3-ba-110-lcr.md](2026-05-10_bea-eitan-anya_d-reporting-capability-slice-3-ba-110-lcr.md) |

The BA 110 computation is implemented in `prototype/platform/reporting/ba-110-lcr.ts` (Bea's Reporting Capability Slice 3 substrate). The computation is a pure function over the event-derived trial balance; no parallel manual spreadsheet is maintained. LCR is a **projection** (a query over the event log), not a stored number — per Principle 1 (events are the only source of truth).
...
- **ZAR** — the primary and functional currency; the main BA 110 LCR is ZAR-denominated.
...
**Activated obligation status:** `ORG-PR-07` (BCBS D335 / BA 120) and `ORG-PR-43` (PA D1/2023) — **IN FORCE** from commencement of trading.
...
| BA-return cadence | BA 120 submitted to PA per the SARB BA-return schedule | Via the NSFR projection substrate (W2 Slice 5) |
...
- BA 110 (LCR) and BA 120 (NSFR) projection and submission via the reporting substrate.
...
- **LCR computation model (Bea's BA 110 substrate)** — Tier 1 model. Pre-deployment validation + annual revalidation. Validation covers: HQLA cap arithmetic correctness; outflow/inflow rate schedule completeness; 75% inflow cap + 25% floor application; per-entity isolation; schema conformance.
...
|---|---|---|---|
| `ORG-PR-06` | BCBS D295 / BA 110 | Maintain LCR ≥ 100% (PA minimum); buffer to internal floor (B2 / B3 calibration) | Section 2 — LCR governance framework; internal floor §2.1 and §2.5; computation §2.3; BA 110 projection substrate |
| `ORG-PR-07` | BCBS D335 / BA 120 | Maintain NSFR ≥ 100% (PA minimum); buffer to internal floor | Section 3 — NSFR governance framework; internal floor §3.1; computation §3.4; PA D1/2023 phase-out tracking §3.3 |
...
- BA 110 LCR substrate spec — [Owner Inbox/2026-05-10_bea-eitan-anya_d-reporting-capability-slice-3-ba-110-lcr.md](2026-05-10_bea-eitan-anya_d-reporting-capability-slice-3-ba-110-lcr.md).
