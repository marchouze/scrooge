...
| Basel **SRP31** — Interest-rate risk in the banking book (IRRBB) | Standard (Basel Framework chapter) | `urn:reg:bcbs:srp:31` | The banking-book rate-shock and ΔEVE / ΔNII outlier test Helena governs (co-owned measurement with Eitan (Treasurer) and Ravi (Treasury / ALM engineer)); the RAS §B4 ΔEVE line is calibrated to it. |
...
| Basel **LCR** — Liquidity Coverage Ratio | Standard (Basel Framework chapter) | `urn:reg:bcbs:lcr:*` (chapter node; SARB transposition BA 300/325 family) | The liquidity-adequacy threshold the RAS LCR line and ILAAP are calibrated against; co-governed with Ravi (Treasury / ALM engineer) and Eitan (Treasurer). |
