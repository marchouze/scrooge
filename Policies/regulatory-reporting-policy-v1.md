...
**BA 100 — Balance Sheet.** The BA 100 is the balance-sheet return: it reports the Bank's assets, liabilities, and equity at period-end in the SARB-prescribed line structure. It is the period-end position return that underpins several of the risk and capital returns. Citation: D2/2024 Schedule 1 `[citation: TBC — exact schedule reference within D2/2024; Imani (Legal-as-code engineer, engineering) + external counsel to confirm]` + Reg 46 `[citation: TBC — exact Reg 46 sub-clause]` + `ORG-PR-29`.

**BA 120 — Income Statement.** The BA 120 reports the Bank's income-statement line items for the period in the SARB-prescribed structure. Citation: D2/2024 `[citation: TBC — schedule reference]` + `ORG-PR-29`.

**BA 200 — Credit Risk RWA.** The BA 200 reports risk-weighted assets for credit exposures — on-balance-sheet loans, off-balance-sheet commitments, and contingent liabilities — computed under the standardised approach (SA-CR) per Basel III/IV and the Regulations Relating to Banks. BA 200 also carries the counterparty-credit-risk (SA-CCR) sub-forms — replacement cost and potential future exposure for OTC derivatives; as an IRD and bond trading bank, Hoz Bank's OTC derivative portfolio generates material CCR exposures. BA 200 is the largest single driver of RWA for most banks; for Hoz Bank, the composition will shift toward market-risk-heavy returns once the global-markets book scales. Citation: D2/2024 `[citation: TBC — schedule reference]` + `ORG-PR-29`.

**BA 210 — Credit Concentration / Large Exposures.** The BA 210 reports the Bank's large exposures — any single counterparty or group-of-connected-counterparties exposure exceeding 10% of eligible capital — against the large-exposure limit framework (Banks Act + Reg 46 + D2/2024 large-exposure provisions `[citation: TBC — exact large-exposure limit provision in Reg 46]`), including the watch-list. For an institutional trading bank, concentration exposures to sovereign issuers, large financial institution counterparties, and cleared market-infrastructure entities are the primary large-exposure populations. Citation: D2/2024 `[citation: TBC]` + `ORG-PR-29`.

**BA 300 — Liquidity Risk (LCR and NSFR).** The BA 300 reports the Bank's liquidity-risk position, including the Liquidity Coverage Ratio (LCR) and the Net Stable Funding Ratio (NSFR) — the two Basel III liquidity standards as adopted by the PA. LCR measures 30-day stress liquidity; NSFR measures structural funding stability over a one-year horizon (carried in the BA 300 series). The Liquidity Management Policy governs the LCR and NSFR targets and breach-response; this policy governs the reporting obligation. The minimum liquid-asset reserve and HQLA detail are carried on the companion BA 310. Citation: D2/2024 `[citation: TBC]` + `ORG-PR-29`.

**BA 320 — Market Risk.** The BA 320 reports market-risk capital — interest-rate risk, equity risk, foreign-exchange risk (incl. the FX net open position), and commodities risk in the trading book — computed under the standardised approach (SA-MR, the revised standardised approach per BCBS FRTB D352 as adopted by the PA through D2/2024). For Hoz Bank as an institutional global-markets trading bank, BA 320 is a material return from the first trading period. The companion BA 325 (Selected Risk Exposure Arising from Trading and Treasury Activities) carries selected trading-and-treasury risk detail. Citation: D2/2024 `[citation: TBC]` + `ORG-PR-29`.

**BA 700 — Capital Adequacy, Leverage and TLAC.** The BA 700 is the headline prudential return: it reports the Bank's CET1, AT1, T2, Tier 1, and Total capital ratios against RWA, the regulatory minima, Pillar 2A add-ons, and any buffers (CCB, CCyB), together with the leverage ratio (Tier 1 capital / total leverage exposure, a non-risk-based backstop per BCBS Basel III/IV and PA D5/2021) and TLAC where applicable. BA 700 is the return the PA uses to assess capital adequacy at a point in time; it is the summary of the more detailed RWA computations in the credit (BA 200), market (BA 320), and operational-risk returns. Citation: D2/2024 `[citation: TBC]` + `ORG-PR-29`.
...
**Operational risk loss events.** Operational risk loss events of at least R1m (or such threshold as D4/2022 prescribes `[citation: TBC]`) are reported individually, with event date, business line, Basel II loss event category (internal fraud, external fraud, employment practices, clients-products-business-practices, damage to physical assets, business disruption and systems failures, execution-delivery-process management), gross loss, recovery, and net loss. The operational loss event taxonomy in the Bank's event substrate must align with the D4/2022 categorisation; misalignment is corrected before the first Risk Return submission. Helena (Chief Risk Officer, governance) sponsors; Rohan (Risk engineer, engineering) curates the operational-loss event taxonomy and the SMA RWA computation under the risk-engine operational-risk module; Bea (Accounting & financial reporting engineer, engineering) generates the Risk Return schedule from the typed events; Vera (Internal audit engineer, engineering) provides continuous-assurance coverage over loss-event coding integrity per `D-OPRISK-ENGINEER-ROLE` (Option B approved 2026-05-21, PR #671).

**Market risk sensitivities.** Market risk sensitivity data — DV01, CS01, equity delta, FX delta — for the Bank's trading book positions, reported at the granularity D4/2022 prescribes `[citation: TBC]`. The market-risk sensitivity data is produced by the Bank's risk engine (under Devon (Chief Operating Officer)) from the position event substrate. Sensitivity data must be consistent with the BA 300 market risk RWA computation; the same underlying position data feeds both.

**Credit risk large-exposure movements.** Movements in the Bank's credit-risk large exposures — counterparties or groups of connected counterparties where the exposure exceeds 10% of eligible capital — reported quarter-on-quarter. The large-exposure data is the same population as BA 210; the Risk Return reports the quarterly delta and the current-quarter breakdown. Consistency with BA 210 is a prerequisite for submission.
...
**Return-level consistency.** Cross-return consistency checks verify that a figure appearing in multiple returns is identical. BA 700 capital ratios must be consistent with the BA 200 (credit, incl. CCR), BA 320 (market), and BA 400 (operational) RWA totals. BA 210 large exposures must be consistent with the BA 200 credit-risk portfolio. BA 300 LCR and NSFR must be consistent with the liquidity-management event substrate. The BA-form generator runs automated cross-return consistency checks before generating the `ReturnDraftGenerated` event.
...
ALCO receives the regulatory-reporting quality summary quarterly as part of the capital and liquidity reporting pack. ALCO's focus is on the BA 300 (LCR/NSFR) and BA 700 (capital ratios) in the context of the liquidity and capital management framework.
...
- **BA-form generator (Bea (Accounting & financial reporting engineer, engineering) and Camille).** Translates the event substrate into BA 100, 120, 200, 210, 300, 320, 400, 700 return format under D2/2024. Discharge exit signal: `ReturnDraftGenerated { returnId, period, sourceEventIds[] }` event on synthetic quarter-end fixture; cross-return consistency checks green; recon `recon:ba-form-schema-validation` passes against D2/2024 schedule schema.
- **Risk Return generator (Bea (Accounting & financial reporting engineer, engineering)), drawing from Rohan (Risk engineer, engineering)'s operational-loss event projection, the markets-risk-sensitivity projection, and the credit large-exposure events; sponsored by Helena (Chief Risk Officer, governance) per `D-OPRISK-ENGINEER-ROLE` Option B (approved 2026-05-21, PR #671).** Translates operational-loss events, market-risk sensitivities, and credit large-exposure events into D4/2022 Risk Return format. Discharge exit signal: `ReturnDraftGenerated { returnId: "risk-return", period }` event on synthetic quarter-end fixture.
...
1. Exact schedule references within D2/2024 for each BA return (BA 100, 120, 200, 210, 300, 320, 400, 700).
...
| v1.1 | 2026-06-09 | Mira (Compliance / RegTech engineer, compliance) under Zara (Chief Compliance Officer, governance) | BA-return form-number remediation only (no substantive policy change). Re-numbered the §2.1 return inventory and all cross-references to the Excel-canonical SARB schedule defined in `Regulations/SARB-PA/ba-returns/_canonical-register.md` (authority `D-BA-RETURN-NUMBERING-EXCEL-CANONICAL`), which supersedes the fabricated D5/2025-derived numbering. See the canonical register §2 for the corrected form→measure map; the prior inventory's fabricated attributions and the non-existent intervening trading-series form were removed. |
