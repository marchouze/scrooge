...
Register coverage stands at 39 liquidity-adjacent obligations indexed — LCR (10), NSFR (12), IRRBB / Pillar 3 (3), liquidity returns (5), and the foundational ORG-PR-06/07/08/11/14/15 policy spine — none in PARTIAL/deferred as a substantive state; all six policy-spine items remain `[TBD]` pending the Liquidity Risk Management Policy and Funding Strategy Policy drafts landing, which is the expected build-phase posture, not a slippage. In the last 24h the engine produced zero `HQLAReported`, zero `LiquidityReport`, zero `LCRRatioProjection`, zero `NSFRRatioProjection`, zero `FXPositionReported`, and zero `NostroFundingApproved`, with 10 `IRRBBChecked` and two prior `LiquiditySnapshot` runs on the 7-day horizon. Degraded-mode is functioning as the § 6 stand-in: the daily heartbeat is landing, and the projection-side silences are correctly attributable to schemas not yet shipped rather than to an inactivity-SLA breach. That distinction needs to be held tightly — the moment Ravi lands the LCR/NSFR projections, the same zero counts flip from "expected" to a § 6 breach on ORG-PR-LCR-001 and ORG-PR-NSFR-001.

The single observation that warrants flagging now is the 10 `IRRBBChecked` events against zero downstream `HedgeProgrammeApproved` or `ALCODecision` over 7 days. In build-phase that is benign — there is no rate-shock excursion to read into it because we have no Δ-EVE or ΔNII projection schema yet — but it does mean ORG-PR-11 and ORG-PR-RETURNS-015 (BA 340) are running on raw check events with no aggregation layer. Of the policy-spine `[TBD]`s, ORG-PR-08 (Funding Strategy) and ORG-PR-14 (ILAAP) are the two that gate the most downstream procedure: ORG-PR-14 in particular is the parent for ORG-PR-36 (LCR), ORG-PR-43 (NSFR) and ORG-PR-38 (externally-facilitated stress sim), so until ILAAP scope is drafted, the planned `Procedures/by-policy/lcr-nsfr-liquidity-stress.md` cannot be authored against a register-linked parent. ORG-PR-NSFR-004 (ZAR financial-corporate ASF phase-out) is the one date-dependent item I want surfaced to Camille before it becomes a surprise — the factor steps through calendar gates and needs to be wired into the NSFR projection schema, not added later.

Next substrate step, in order: (1) Ravi to land `LCRRatioProjection` schema first — it is the schema that converts the most zeroes into either signal or breach, and it unblocks ORG-PR-LCR-001/004/006/007 simultaneously; (2) `NSFRRatioProjection` second, with the ASF phase-out date table embedded per ORG-PR-NSFR-004 rather than bolted on; (3) I will draft the ORG-PR-08 Funding Strategy Policy skeleton this week so ORG-PR-14 (ILAAP) has a register-linked parent and the planned LCR/NSFR/stress procedure can be authored against a real obligation chain. ALCO prep item: nothing for the next pack beyond ratifying that degraded-mode heartbeat is the SLA stand-in until projections land — I do not want ALCO signing on substantive ratio commentary while the engine is silent by design.
