...
| 5 | Convene tabletop exercise for each scenario: IBS owners, technical leads, Helena (CRO), Devon (COO); Senna (Security engineer, engineering) for cyber scenarios; Zara (Chief Compliance Officer) for regulatory-reporting IBS | `Devon` (chair) + `Helena` + IBS owners | `@platform/resilience/tabletop-facilitation` (`PLANNED`) | Tabletop exercises are the default test mode; live system failover is only used if the Board explicitly approves a live test and Senna confirms the test window is safe. Exercise outputs recorded as structured session notes in RMS. |
