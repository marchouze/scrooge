...
asOf: 2026-05-15T05:51:10.897Z
decision-required: false
---

# Ravi — ALM readiness, 2026-05-15
...
Latest `LiquiditySnapshot` event: 2026-05-14T06:53:57.623Z
...
Headline: zero ALM-domain events on the log in the last seven days and zero shadow-fires under Eitan's snapshot — we are still in build-phase substrate work, and degraded-mode (Eitan's snapshot running on the absence of `HQLAObserved` / `LCRComputed` / `NSFRComputed`) is functioning as designed as the daily-funding-event SLA stand-in. The single load-bearing block on Eitan's first end-to-end live LCR sign-off is `alm:hqla-inventory`: until the HQLA inventory projection emits `HQLAObserved` against a synthetic capital line with Banks Act Reg 26 Level-1 / 2A / 2B classification and haircuts applied, `alm:lcr-net-outflow` cannot compute and `LCRComputed` cannot fire. NSFR sits one step further back behind the same projection runtime — ASF/RSF factor tables per Banks Act Reg 27 / BCBS d335 are specified but waiting on a synthetic liability/asset book in the event log.

Three observations worth ranking. First, `alm:hqla-inventory` is genuinely one ticket from green in principle — the Reg 26 eligibility table is the only substantive specification gap, and once it lands the LCR ratio engine inherits a clean upstream contract; this is where the next engineering hour earns the most. Second, `alm:ftp-attribution` is in `drafting`, not `specified`, and the binding gap is not the attribution module itself but deferred market-rate feed integrations (ZARONIA, JIBAR, OIS, FX) — until vendor selection clears, the FTP curve registry is a shell; this is the connector that gates `ORG-PR-08` and `ORG-PR-15` (Funding Strategy Policy) moving past PARTIAL. Third, `alm:samos-funding` remains correspondent-mediated under the indirect-participant posture (not direct SAMOS membership), and the correspondent-bank API contract with Tomas is undrafted — this is the BCBS 248 intraday surface and the `ORG-MK-08` Excon-funding surface; `FundingDrawnDown` cannot fire without it. All seven Ravi-owned obligations (`ORG-PR-06/-07/-08/-11/-14/-15`, `ORG-MK-08`) are PARTIAL pending these substrate items — none of them are mine to close governance-side (Helena/Eitan own the policy text), but each is engine-gated on my side.

Next engineering move, in order: (1) specify the HQLA-eligibility table per Banks Act Reg 26 (Level-1 / 2A / 2B with associated haircuts) and wire the inventory projection against the synthetic capital line so `HQLAObserved` can fire — this unblocks `LCRComputed` and clears the load-bearing item for `ORG-PR-06` / `ORG-PR-07`; (2) co-specify the ASF/RSF factor tables per Banks Act Reg 27 / BCBS d335 in the same projection-runtime pass with Anya, so `NSFRComputed` follows on the same synthetic book; (3) draft the correspondent-bank SAMOS-mediation API contract with Tomas (per `project_indirect_participant_posture.md`), defining at minimum `FundingDrawnDown` event shape, intraday-position polling cadence per BCBS 248, and Currency & Exchanges Manual section A.4 reporting hooks for `ORG-MK-08`. EVE shocks per BCBS d365 for `alm:irrbb-repricing-gap` (`ORG-PR-11`) and the ZARONIA/JIBAR feed vendor selection for FTP sit in the next sprint behind these three.
