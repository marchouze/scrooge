...
- **Engineering-Charter citation.** Every dispatch prompt cites the Engineering Charter ([`Engineering-Charter.md`](Engineering-Charter.md), `D-ENGINEERING-INTEGRITY-CHARTER`) alongside this section, so the ten no-shortcut commands and the Definition of Done bind the dispatched agent's work — not just in-session work. The agent's deliverable is *done* only when the Charter's Definition of Done holds.
- **Premise citation & challenge (`D-AGENT-DOMAIN-COMPETENCE`, `PROC-GOV-ADC-01`).** Every dispatch brief states its domain *premise* explicitly with a citation (the standard, principle, or worked example it rests on). The receiving agent must **confirm or challenge** that premise against its own knowledge base (spec §18) *before* implementing — silently executing a wrong premise is a finding. On domain questions the seat's domain authority **outranks the brief, including a brief from Scrooge** (the orchestrator is as capable of a wrong premise as any seat — the FX settlement-realisation error originated in a Scrooge brief and was executed unchallenged). Validate work against domain-truth oracles, not mere internal consistency (a consistent-but-wrong result is a finding).
...
- **Recall-before-dispatch (`D-SCROOGE-RECALL-BEFORE-DISPATCH`, CEO-approved 2026-06-27).** Before every `Agent(...)` dispatch, Scrooge runs `bun run dispatch:recall --agent <seat>` (from `prototype/`, `BANK_EVENT_DB` → shared store) and pastes the returned mandate-filtered agent-memory slice into the brief, so the dispatched agent inherits the bank's accumulated domain memory — not only Scrooge's in-context knowledge. The manual recall and the autonomous runtime read share one `readMyMemory` code path so they cannot drift; this manual step stands in until the autonomous read-hook (`WorldStateSnapshot.myMemory` per iteration) is live. Recall is mandatory, not optional: dispatched agents do NOT auto-read the memory store today, so claiming the store "feeds dispatches" automatically is false. After substantive sessions, curated durable lessons are backfilled into the agent-memory store (`seeds/agent-memory/notes.seed.json` → `bun run backfill:agent-memory-from-notes`, bodies copied to the shared home doc store) so recalls compound. Authority: `D-SCROOGE-RECALL-BEFORE-DISPATCH` under `D-AGENT-MEMORY-PERSISTENCE`.
...
All team member profiles live in `/Team/`. Each file is the **operating spec for a standing autonomous agent** (Principle 6). The canonical structure has 20 sections — sections 1–5 carry the legacy character data (Identity, Persona, Mandate, Areas of expertise, Working style); sections 6–20 are the operating spec proper (Cadence, Triggers, Inputs, Decisions in scope, Decisions that escalate, Outputs, System capabilities called, Procedures owned, Data contracts, Independence/conflicts, Substrate gaps, Change log, Authoritative knowledge base & sources, Domain-truth validation, Premise-challenge duty). Sections 18–20 are the domain-competence layer (`D-AGENT-DOMAIN-COMPETENCE`, `PROC-GOV-ADC-01`): they bind each seat to its authoritative domain standards + worked examples, require validation against domain-truth oracles (not mere internal consistency), and impose the duty to reject a wrong brief premise. The template at `Team/_agent-spec-template.md` is the canonical authoring location; new personas use it from the start, and existing character-sheet personas are upgraded as they are touched. Persona files lacking sections 6–20 are findings until upgraded (Vera Wave-4 #10 agent-spec-integrity recon pipeline, planned; `recon:agent-spec-domain-competence` asserts §18–20 at `warn` until the grooming backlog closes, then ratchets to `fail`).
