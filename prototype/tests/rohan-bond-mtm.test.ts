...
//         → exactly one BondPositionRevalued is emitted with correct fields.
//   TC-2: Idempotency — second call for the same (tradeId, revalDate) produces
//         no additional BondPositionRevalued.
//   TC-3: Banking-book bonds are excluded (amortised-cost, IFRS 9 §4.1.2).
//   TC-4: No price tick available → zero BondPositionRevalued, descriptive
//         skip reason added to skipReasons.
...
import { makeBondSold, makeBondTradeExecuted } from "../platform/event-store/event-types/bond-accounting";
...
// R186 — SA Govt 10.5% due 2026-12-21 (still live as of test dates)
const ISIN_R186 = "ZAG000030108";
// R2030 — SA Govt 7.75% due 2030-01-31
const ISIN_R2030 = "ZAG000057547";

/** A simple trading-book BondTradeExecuted for the given tradeId and ISIN. */
function makeTrade(
  tradeId: string,
  isin: string,
  opts: {
...
describe("TC-1: happy path — BondPositionRevalued emitted for trading-book position", () => {
  it("emits one BondPositionRevalued per live trading-book position with price tick", () => {
    const mdStore = new MarketDataStore(":memory:");
    const revalDate = "2026-06-01";
    const tradeId = "BND-TEST-001";

    eventStore.append(makeTrade(tradeId, ISIN_R186, { cleanPricePercent: 101.5, dirtyPricePercent: 101.8 }));
    ...
    ].filter((e) => (e.payload as { tradeId: string }).tradeId === tradeId);
...
describe("TC-2: idempotency — no duplicate BondPositionRevalued for the same date", () => {
  it("second run on same revalDate is a no-op for already-revalued positions", () => {
    const mdStore = new MarketDataStore(":memory:");
    const revalDate = "2026-06-02";
    ...
    const first = runEodBondRevaluation(
      eventStore as unknown as import("../platform/event-store/store").EventStore,
      mdStore,
      revalDate,
    );
    ...
    const second = runEodBondRevaluation(
      eventStore as unknown as import("../platform/event-store/store").EventStore,
      mdStore,
      revalDate,
    );
    ...
    const events = [
      ...eventStore.replay({ type: "BondPositionRevalued" }),
    ].filter((e) => (e.payload as { tradeId: string; revalDate: string }).tradeId === tradeId && (e.payload as { revalDate: string }).revalDate === revalDate);
    expect(events).toHaveLength(1);
  });
});
...
describe("TC-4: no price tick → zero BondPositionRevalued, skip reason recorded", () => {
  it("records a descriptive skip reason when no tick is available", () => {
    const mdStore = new MarketDataStore(":memory:"); // empty — no ticks
    const revalDate = "2026-06-04";
...
describe("TC-5: matured bond excluded from revaluation", () => {
  it("skips a position whose maturityDate is on or before the revalDate", () => {
    const mdStore = new MarketDataStore(":memory:");
    const revalDate = "2026-06-05";
    // Bond matured before the revalDate.
...
describe("TC-7: P&L sign — buy position gains when price rises", () => {
  it("unrealisedPnlZarMinor > 0 when currentCleanPrice > bookCleanPrice for a buy", () => {
    ...
    const event = [
      ...eventStore.replay({ type: "BondPositionRevalued" }),
    ].find((e) => (e.payload as { tradeId: string }).tradeId === tradeId);
...
describe("TC-8: P&L sign — sell position gains when price falls", () => {
  it("unrealisedPnlZarMinor > 0 when currentCleanPrice < bookCleanPrice for a sell", () => {
    const mdStore = new MarketDataStore(":memory:");
    const revalDate = "2026-06-08";
    const tradeId = "BND-PNL-008";

    // Booked at 102.00% clean price, current market at 100.00% (price fell).
    eventStore.append(
      makeTrade(tradeId, ISIN_R2030, {
        cleanPricePercent: 102.0,
        dirtyPricePercent: 102.5,
        side: "sell",
        maturityDate: "2030-01-31",
      }),
    );
    seedProductionTick(mdStore, ISIN_R2030, "100.00", revalDate);
