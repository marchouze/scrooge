...
const ACCEL_ID = "D-RMS-PHASE-2-3-ACCELERATE";
if (!hasPhase(ACCEL_ID, "approved")) {
  recordDecision({
    ...
    category: "governance",
    recommendation: "Approve RMS Phase 2 and Phase 3 as effective immediately upon spec approval, with no mandatory one-agent-week soak period.",
...
if (!hasPhase(PHASE2_ID, "requested")) {
  requestDecision({
    ...
    authority: "CEO",
    authorityRef: "marc@tgv.co.za",
    title: "RMS Phase 2 — route all agent dispatches through AgentBriefIssued; retire direct Team Inbox authoring",
    category: "governance",
    recommendation: "Approve Phase 2: all new agent dispatches emit AgentBriefIssued event first; Team Inbox markdown files become derived renders of the Briefs register only.",
    rationale: "Phase 1 shipped the dispatch CLIs and Briefs register. Phase 2 makes events-first dispatch mandatory. Acceptance criterion per D-RMS-PHASE-2-3-ACCELERATE: immediate on approval.",
    sourceDocHashes: [],
...
if (!hasPhase(PHASE3_ID, "requested")) {
  requestDecision({
    ...
    authority: "CEO",
    authorityRef: "marc@tgv.co.za",
    title: "RMS Phase 3 — route all deliverables through RecordFiled; retire direct Owner Inbox authoring",
    category: "governance",
    recommendation: "Approve Phase 3: all new deliverables stored in document substrate via RecordFiled event; Owner Inbox markdown files become derived renders only.",
    rationale: "Phase 1 shipped the RecordFiled event type and document store. Phase 3 makes it mandatory for all deliverables. Acceptance criterion per D-RMS-PHASE-2-3-ACCELERATE: immediate on approval.",
    sourceDocHashes: [],
    citations: ["D-RMS-PHASE-1", "D-RMS-PHASE-2-3-ACCELERATE"],
    recordedVia: "scrooge:session-delegation",
    actor: { type: "human", id: "marc@tgv.co.za" },
...
console.log(JSON.stringify({ level: "info", msg: "phase-2-3-accelerate: complete" }));
