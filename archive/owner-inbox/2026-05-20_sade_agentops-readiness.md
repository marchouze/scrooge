...
asOf: 2026-05-20T06:55:43.767Z
decision-required: false
---

# Sade — AgentOps readiness snapshot, 2026-05-20
...
The AgentOps substrate is, honestly, a roster of zeros against a non-trivial gap inventory — and that is the correct shape for this week. No agents are folded into the registry because `AgentRegistered` has no producer yet; no retirements, no key rotations, no permission policies published, because those event types are specified but not wired. What's load-bearing on a Sade-signed event and not yet built: the agent-retirement path (no `AgentRetired` producer means we cannot evidence revocation under BCBS 2015 Principle 5 on accountability, nor satisfy the audit-trail expectation Vera will test); the fit-and-proper attestation event for agents whose operating specs gate sensitive capabilities (Banks Act 94 of 1990 §60-§60B reads onto natural persons, but the SARB supervisory posture extends the doctrine to autonomous agents acting in regulated roles, and we have committed to mirror it); and the capability-assignment register, which today exists only as Atlas's substrate sketch — there is no event that says "agent X may call capability Y as of timestamp Z."

Three observations rank above the rest. First, the BCEA 75 of 1997, EE Act 55 of 1998 and B-BBEE obligations sitting in my register as PLANNED/PARTIAL are paused-but-registered by design under the build-phase model — they bind the bank at licence-day against a workforce that does not yet exist, and the human-HR slice of my mandate is correctly silent until a CHRO is appointed and the employment relationship is real. This is not a finding; flagging it so it is not later misread as one. Second, the AgentOps capability gap that most directly gates a downstream procedure is `AgentRetired`: without it I cannot author `agent-retirement.md` with evidentiary teeth, and without that procedure Atlas cannot close the loop on key revocation under POPIA §19 (security safeguards) when an agent is decommissioned. Third, the obligations register slice for AgentOps fit-and-proper (Banks Act §60 doctrine, extended) is registered but has no attestation event behind it — a coherence gap Vera's Wave-4 #10 pipeline is the right instrument to close.

Next AgentOps move, concretely: (a) author `Procedures/agent-retirement.md` against Atlas's substrate spec, specifying the `AgentRetired` event shape, the key-revocation handshake, and the obligations-register hooks; (b) wire `AgentFitAndProperAttested` as a first-class event type, with the attestation payload referencing the agent's operating spec hash and the Banks Act §60-derived criteria; (c) commission Vera's Wave-4 #10 pipeline — fit-and-proper coherence over agent operating specs — so that attestation has something to evidence against rather than a bare signature. Wave-4 #15, the audit over my own AgentOps decisions, follows naturally once #10 is producing.
