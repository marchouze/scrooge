...
// Eighth SARB BA-form return rendered end-to-end (after BA 325 / BA 350 /
// BA 600 / BA 700; the BA-quintet batch adds BA 100 + BA 120 + BA 300 +
// BA 310 + BA 320).
//
...
// (Slice 2) plus an explicit BA 100 line-classification map. No event
// side-effects; no document-store writes; no event-store reads. Callers
// (the CLI wrapper at `prototype/scripts/render-ba-100.ts`, downstream
...
//      → SEMANTIC LAYER        — IFRS Slice 6 + BA 100 line mapping (caller-side)
...
// Banks Reg 32 (monthly returns) + SARB BA 100 published schema:
//
...
// scope at v0 (mirrors BA 700's solo-entity caveat). Hoz Securities
...
// a BA 100 for a non-bank entity.
//
// Substrate gaps surfaced (forward-link in the decision record):
//   - **BA 100 line-classification map** is supplied externally for now.
...
//     extend to publish SARB BA 100 line mapping; until then the map is
...
//     only at v0.
//   - **SARB BA 100 XML adapter** deferred to a downstream slice; this
//     module emits the canonical JSON contract only (mirrors the BA 325 /
//     BA 700 sequencing — XML adapters landed in a later slice).
//   - **Per-currency disaggregation** is surfaced in the output but the
//     SARB BA 100 form is presented in `functionalCurrency` only at v0;
...
/**
 * BA 100 top-level section. Each leaf account that contributes to the form
...
/**
 * Per-leaf-account BA 100 line classification. An account belongs to one
 * section + one form line (free-form label awaiting Mira's WS-INSTRUMENT-
 * ANALYSES SARB BA 100 published taxonomy ingestion).
 *
...
export interface Ba100LineClassification {
  ...
  /**
   * Free-form sub-line label for the BA 100 form (e.g.
...
/** Caller-supplied classification map for the BA 100 generator. */
...
export interface Ba100GeneratorInput {
  ...
  /** Per-account BA 100 line classifications. Accounts without an entry surface in `classificationGaps`. */
...
export interface Ba100Section_Output {
  readonly section: Ba100Section;
...
export interface Ba100BalanceSheet {
  ...
  readonly assets: Ba100Section_Output;
  readonly liabilities: Ba100Section_Output;
  readonly equity: Ba100Section_Output;
...
function assertBankEntity(entity: string): void {
  if (!BA_100_BANK_ENTITIES.includes(entity)) {
    throw new Ba100GeneratorError(
      `BA 100 (Balance Sheet) is bank-licence-bound; entity '${entity}' is not in BA_100_BANK_ENTITIES (${BA_100_BANK_ENTITIES.join(
        ", ",
      )}). See Regulations/_legal-entity-tree.md + D-REGULATORY-PERIMETER. Group-consolidated BA 100 lands at a downstream slice.`,
    );
  }
}
...
/**
 * Generate the BA 100 (Balance Sheet) projection from a trial balance + a
 ...
 * for forensic transparency. SARB BA 100 form is functional-currency-only
...
export function generateBa100BalanceSheet(input: Ba100GeneratorInput): Ba100BalanceSheet {
  ...
  if (tolerance < 0 || !Number.isFinite(tolerance)) {
    throw new Ba100GeneratorError(
      `BA 100 generator: tolerateImbalanceMinor must be a finite non-negative integer, got ${tolerance}`,
    );
  }
  ...
  if (!balanced && tolerance > 0) {
    placeholders.push(
      `[citation: TBC — BA 100: balance-sheet invariant violated by ${differenceMinor} ${ccy} (cents); tolerance ${tolerance}; surfaced under tolerateImbalanceMinor rehearsal posture]`,
    );
  }
  if (!balanced && tolerance === 0) {
    throw new Ba100GeneratorError(
      `BA 100 generator: balance-sheet invariant violated — assets (${assetsTotal}) ≠ liabilities (${liabilitiesTotal}) + equity (${equityTotal}); difference ${differenceMinor} ${ccy} (cents) exceeds tolerance ${tolerance}. Set tolerateImbalanceMinor to surface as a placeholder instead.`,
    );
  }
  if (classificationGaps.length > 0) {
    placeholders.push(
      `[citation: TBC — BA 100: ${classificationGaps.length} trial-balance rows have no line classification; pending Mira's WS-INSTRUMENT-ANALYSES SARB BA 100 published-schema mapping]`,
    );
  }
  placeholders.push(
 ...
 * reproducibility. Sorted-stable JSON. Mirrors BA 700 Slice 4.
 */
