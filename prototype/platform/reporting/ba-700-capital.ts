...
// CLI wrapper at `prototype/scripts/render-ba-700.ts`, downstream
// `ReportGenerated` event emitters in Slice 5) compose the inputs and
// consume the output.
//
// Pipeline (mirrors Slice 3's BA 325 architecture — one harness, two
// returns now):
//
...
//      → BA 700 PROJECTION     — this module — pure function
//      → RENDER + STORE        — `ba-700-render.ts` + RMS doc store
//
...
// generator throws if asked to produce a BA 700 for a non-bank entity.
//
...
//     once Mira's `WS-INSTRUMENT-ANALYSES` lands the SARB BA 700 published-
...
//   - **Group-consolidated BA 700** (LE-ZA-HOZ-GROUP look-through per
...
/**
 * Per-leaf-account capital classification for the BA 700 generator. An
...
export interface AccountCapitalClassification {
  ...
  /** Free-form sub-category label for line-by-line BA 700 render. */
 ...
 *
 * `category` is a free-form label for the BA 700 sub-line render
 * (e.g. "goodwill", "other-intangibles", "deferred-tax-assets",
 * "significant-investments-above-threshold"). The recon pipeline will
 * gate the category vocabulary against the Mira-resolved SARB BA 700
...
export interface Ba700GeneratorInput {
  ...
  /** ISO 8601 — the period-end as-of date the BA 700 is reported at. */
...
/**
 * One line item on the BA 700 form. Lines compose into sections; sections
...
/**
 * The full BA 700 generator output.
 *
...
/**
 * Bank-licence-bound entity short-ids that are in scope for BA 700. The
 ...
 * BA 700 are out of scope at this slice — see `D-REGULATORY-PERIMETER`
...
function assertBankEntity(entity: string): void {
  if (!BA_700_BANK_ENTITIES.includes(entity)) {
    throw new Ba700GeneratorError(
      `BA 700 (Capital Adequacy) is bank-licence-bound; entity '${entity}' is not in BA_700_BANK_ENTITIES (${BA_700_BANK_ENTITIES.join(", ")}). See Regulations/_legal-entity-tree.md + D-REGULATORY-PERIMETER. Group-consolidated BA 700 lands at Slice 7.`,
    );
  }
}
...
/**
 * Generate the BA 700 (Capital Adequacy) projection from a trial balance
...
export function generateBa700Capital(input: Ba700GeneratorInput): Ba700Output {
  ...
  for (const c of input.classifications) {
    if (classMap.has(c.leafAccountId)) {
      throw new Ba700GeneratorError(
        `BA 700 generator: duplicate classification for account '${c.leafAccountId}'`,
      );
    }
  ...
  for (const d of sortedDeductions) {
    if (d.currency !== ccy) {
      throw new Ba700GeneratorError(
        `BA 700 generator: deduction '${d.category}' is in '${d.currency}', expected functional currency '${ccy}'. Multi-currency deductions land at Slice 6+.`,
      );
    }
    if (d.amountMinor < 0) {
      throw new Ba700GeneratorError(
        `BA 700 generator: deduction '${d.category}' amountMinor must be non-negative, got ${d.amountMinor}. Deductions are reported as positive magnitudes; the generator subtracts them.`,
      );
    }
    const lineItem: Ba700LineItem = {
  ...
  if (input.classifications.some((c) => c.subCategory === undefined)) {
    placeholders.push(
      "[citation: TBC — capital-tier classification subCategory missing for one or more accounts; Mira's WS-INSTRUMENT-ANALYSES will resolve to SARB-published BA 700 line labels]",
    );
  }
  placeholders.push(
    "[citation: TBC — exact SARB BA 700 line-numbering pending Mira's WS-INSTRUMENT-ANALYSES schema ingestion]",
  );
...
function validateBuffers(b: BufferRequirements): void {
  ...
  for (const [name, value] of fields) {
    if (!Number.isFinite(value) || value < 0 || value > 1) {
      throw new Ba700GeneratorError(
        `BA 700 generator: buffer-requirement '${String(name)}' must be a finite ratio in [0,1], got ${value}`,
      );
    }
  }
  if (b.baseCet1Ratio > b.baseTier1Ratio || b.baseTier1Ratio > b.baseTotalRatio) {
    throw new Ba700GeneratorError(
      `BA 700 generator: buffer-requirement base-ratio ordering violated; expected baseCet1Ratio ≤ baseTier1Ratio ≤ baseTotalRatio, got ${b.baseCet1Ratio} ≤ ${b.baseTier1Ratio} ≤ ${b.baseTotalRatio}`,
    );
  }
}
