...
- **Goal-deriver defer rate** (2026-05-21 → addressed 2026-05-27): was 98% (334/340 iterations); root causes identified and fixed — (1) `openBriefsAddressedToAtlas` threshold reduced from 2h to 30 min so any pending brief triggers action sooner; (2) Candidate 1 bug: when worldState findings existed but Candidate 0b's spec closed-set check failed, the deriver incorrectly returned `null` (deferred) — now selects `ATLAS_SCHEMA_GOAL`/`ATLAS_PR_GOAL` instead; (3) new Candidate 0c fires on recent platform recon violations (last 4h); (4) SubstrateStateSnapshot cadence reduced from 24h to 4h. T-12 cadence violations in the event store (Vera 19921ms, Bea 18009ms, Owen 198ms, Atlas 119ms) are pre-enforcement historical events — the 60s floor is enforced going forward; those events cannot be retroactively removed.
