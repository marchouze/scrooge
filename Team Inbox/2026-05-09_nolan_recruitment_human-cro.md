---
title: Recruitment scope — Human Chief Risk Officer (CRO)
author: Nolan
date: 2026-05-09
summary: Priority-1 search for a separate human CRO to be appointed before SARB licence-application lodgment. Drives Joint Standard 1 of 2024 operational independence; resolves the JS-1-2024 finding identified in Mira+Zara's confirmation paper. Sequenced first of the six thin-human-layer hires.
seat: human-cro
priority: 1
cadence: appointed before SARB licence-application lodgment
research-source: "pending: PAX (substrate input from `Owner Inbox/2026-05-09_owen-imani_thin-human-layer-minimum-possible.md` §2.3 + `Owner Inbox/2026-05-09_mira-zara_concentration-risk-conduct-confirmation.md` §3.1; PAX to fold into formal role-brief format on next research cycle)"
decision-required: false
---

# Recruitment scope — Human Chief Risk Officer (CRO)

**From:** Nolan (recruiter) · reports to Devon (COO).
**For:** the licence-application file; named human CRO accountable to SARB Prudential Authority.
**Status:** Priority-1 of the six thin-human-layer searches authorised under D-THIN-HUMAN-LAYER-MINIMUM (CEO-approved 2026-05-08). Sequenced to lodge first.

---

## 1. Why this seat exists

The Joint Standard 1 of 2024 (PA / FSCA Cybersecurity and Cyber Resilience) read with BCBS Corporate Governance Principles for Banks (2015) treats CRO operational independence from the CEO as a **structural** expectation at licence-day, not a SARB-precedent challenge that can be answered by a post-licence hiring commitment. Mira+Zara's confirmation paper (`Owner Inbox/2026-05-09_mira-zara_concentration-risk-conduct-confirmation.md` §3.1) reads interim-CRO-as-CEO as a JS-1-2024 finding rather than a flagged risk — the CEO's adoption of that read is the substantive change in D-THIN-HUMAN-LAYER-MINIMUM relative to Owen+Imani's original five-human draft.

The seat is the **human face** of the bank's prudential second-line. Helena (engineering CRO) and Rohan (risk engineer) carry the engineering load; the human CRO's outputs flow through Helena's substrate. The human seat is what SARB sees.

## 2. Regulatory anchor

| Source | Section / clause | Bearing on this seat |
|---|---|---|
| Banks Act 94 of 1990 | s.60 (Board composition; "sufficient size and composition" to discharge fiduciary duties) | The Board's risk-oversight discharge depends on a CRO present at risk-committee proceedings. [citation: TBC — exact PA fit-and-proper sub-instrument] |
| Joint Standard 1 of 2024 (PA / FSCA Cybersecurity and Cyber Resilience) | §6 (responsible-person designation); §7 (operational independence) | CRO operational independence from the CEO is the binding constraint behind this hire. [register: ORG-GV-CRO-INDEPENDENCE — gap, Mira-owned closure] |
| BCBS Corporate Governance Principles for Banks (2015) | Principle 6 (risk-management function); Principle 8 (risk identification, monitoring, controlling) | Substantive independence reading — financially-dependent NEDs / merged executives are challenged. |
| SARB Regulations Relating to Banks | Reg. 39 (internal-models for capital) | IRB / IMM / IMA approvals require a CRO function organisationally independent of model development. |
| Banks Act s.70 (risk-management process and audit) | s.70(2A)(b) | The bank's risk-management process and audit framework discharge requires a named CRO. |

## 3. Mandate boundary

**In scope:**

- Named accountable person for the bank's risk-management function under Joint Standard 1 of 2024 §6 + Banks Act s.70.
- Chair of the Board Risk Committee (or its proxy — Interim Risk Forum until Board AC and BRC are constituted).
- ICAAP / ILAAP signing authority for SARB Prudential Authority engagement (pre-application gate).
- Risk Appetite Statement & Framework approval (CEO-approved today; CRO-approved at licence-day; Helena's engineering output flows into).
- Model-risk governance final accountability (Helena builds; Nadia validates; the human CRO signs the model-risk policy).
- SARB Prudential Authority engagement on risk matters (capital, liquidity, large exposures, market risk, IRRBB, operational risk, model risk).
- Veto over risk-decisions reserved for second-line (RAS limit breach disposition; production-use of unvalidated models; appetite-line breach dispositions).

**Out of scope:**

- First-line risk-taking — the CEO and the Head of Global Markets (Saskia / future human Markets seat) own first-line risk-taking.
- Compliance / AML-CFT — Zara (CCO) owns. The CCO is a separate seat.
- Internal audit — Thandiwe (CAE) owns; third-line independence is non-negotiable.
- Day-to-day risk engineering — Helena (engineering CRO) and Rohan (risk engineer) own, reporting through this seat.
- Model building — Rohan owns.
- Independent model validation — Nadia owns; reports to this seat with functional independence from Rohan.

## 4. Fit-and-proper requirements

The human CRO must satisfy SARB Prudential Authority fit-and-proper standards under the Banks Act + Regulations Relating to Banks read with Joint Standard 1 of 2024.

| Requirement | Source |
|---|---|
| Banks Act + Regs Relating to Banks fit-and-proper for designated officers (CRO is a named designated officer in `_obligations-register.md` ORG-GV-11) | Banks Act 94 of 1990; PA fit-and-proper sub-instruments. [citation: TBC — exact PA fit-and-proper directive number] |
| **Joint Standard 1 of 2024 operational-independence test** — CRO must be organisationally independent of the CEO; cannot be a CEO direct-report whose performance assessment is CEO-controlled in a way that compromises challenge. | Joint Standard 1 of 2024 §6–§7. [register: ORG-GV-CRO-INDEPENDENCE — Mira-owned closure] |
| BCBS Corporate Governance Principles for Banks substantive-independence reading | BCBS Corp Gov Principles 2015 §3. |
| Fit-and-proper file structure under ORG-GV-11 | `_obligations-register.md` ORG-GV-11. |
| Appropriate banking-regulatory experience — minimum 10 years senior risk-management experience in a regulated bank or equivalent prudentially-regulated financial institution. [citation: TBC — convention, not statute] |
| Prior CRO or Deputy-CRO seat at a SARB-regulated bank, or equivalent under a comparable-regime regulator (FCA / PRA / OCC / ECB / APRA / MAS), is the strong-fit shape. [citation: TBC — convention] |
| Not concurrently a director or officer of a competitor bank or material counterparty — conflicts test under King IV. [citation: King IV Code on Corporate Governance, Principle 11] |

## 5. Reporting line and committee membership

- **Reports to:** CEO (Marc, today; future human CEO if rotation occurs). Functional independence from CEO under JS 1 of 2024 §7 — performance-assessment carve-out documented in engagement letter.
- **Direct reports (engineering line):** Helena (engineering CRO — dotted line in current substrate; solid line at licence-day). Rohan (risk engineer) and Nadia (independent-validation engineer) report through Helena's engineering substrate.
- **Board committee membership:** Chair of Board Risk Committee (or its proxy — Interim Risk Forum). Standing attendee at Audit Committee. Standing attendee at ALCO (chaired by Treasurer / Eitan-equivalent human seat at licence-day).
- **Regulator engagement:** named CRO contact for SARB Prudential Authority on prudential matters; co-engages with Zara (CCO) on Joint-Standard interlocks.

## 6. Compensation / remuneration framing

`[deferred to Camille]` — compensation framing is a CFO-side input. The seat is a Banks Act designated-officer seat, not a NED seat; remuneration is fixed + variable with risk-adjusted deferral per BCBS principles on remuneration. Camille produces the framing as part of the licence-day human-cost budget.

## 7. Search criteria

**Must-have:**

- 10+ years senior risk-management experience in a SARB-regulated bank or comparable-regime regulated bank.
- Demonstrated CRO or Deputy-CRO seat in a regulated bank.
- Banks Act + PA fit-and-proper credible (no prior fit-and-proper findings; clean SARB engagement history).
- Demonstrated independence from the CEO — performance-assessment structure must be governable by Board committee, not CEO.
- IFRS 9 ECL governance experience (the bank's first capital-model is the credit-risk ECL).
- Joint Standard 1 of 2024 fluent — operational independence, responsible-person designation.
- BCBS Corporate Governance Principles 2015 fluent.
- ICAAP / ILAAP signing experience.
- South African resident (or willing-to-relocate before licence-application lodgment).

**Nice-to-have:**

- Prior experience at a small-bank or institution-in-formation (the bank's R300m-target capital posture is small-bank shape).
- Trading-bank experience (institutional global-markets posture per `project_strategic_foundation`).
- AI / agent-substrate fluency — the bank's CRO oversees an engineering CRO (Helena-the-agent) and a model-validation agent (Nadia-the-agent); cultural fit with autonomous-by-default operating model is a strong-fit signal.
- B-BBEE positive contribution (under Companies Reg. 43 S&E read).

**Automatic disqualifiers:**

- Prior fit-and-proper findings against the candidate at any SA or comparable-regime regulator.
- Concurrent directorship at a competitor SA bank or material counterparty.
- Failed reference check on integrity / honesty.
- Cannot satisfy Banks Act + PA fit-and-proper documentation (criminal record; insolvency; tax-compliance).

## 8. Cadence with SARB pre-application gate

The human CRO must be **named in the licence-application file**. The application file is assembled by Owen + Imani (corporate-form / human-roster sections) and Mira + Zara (conduct + AML/CFT section), reviewed by external counsel at the licence-application gate (per `Owner Inbox/2026-05-09_imani_external-counsel-licence-application.md` §5.4).

- **Search opens:** immediately on PR merge.
- **Shortlist target:** before SARB pre-application engagement opens.
- **Appointment target:** before licence-application lodgment.
- **Interim posture:** Marc retains formal CRO accountability while Helena (engineering CRO) operates the function. This is a documented interim under `CLAUDE.md` "Operating model — what is real, deferred, paused"; the interim ends when this seat is filled.

## 9. Substrate gaps (Principle 7 — autonomous-by-default reframe)

Per Principle 7, a fully-autonomous run of this recruitment scope would require:

- **Search-vendor agent** — the bank does not have an agent that engages executive-search vendors and runs candidate-funnel management; this scope is currently dispatched as a Scrooge-coordinated in-session run. Owner: Nolan + future search-vendor-management agent. Target: post-runtime.
- **Fit-and-proper attestation pipeline for human officers** — Sade's AgentOps pipeline does this for agents; a human-officer analogue is not yet built. Owner: Sade (HR engineer) + Imani (engagement-letter substrate) + Owen (governance secretariat). Target: pre-licence-application.
- **Joint-Standard-1-of-2024 register row** — `_obligations-register.md` ORG-GV-CRO-INDEPENDENCE is a gap (Mira-owned closure per `2026-05-09_mira-zara_concentration-risk-conduct-confirmation.md` §5).

## 10. Authority

- `Owner Inbox/2026-05-08_scrooge_ceo-decision-record_d-thin-human-layer-minimum.md` — CEO decision approving the 6-human composition with separate human CRO before licence-application.
- `Owner Inbox/2026-05-09_owen-imani_thin-human-layer-minimum-possible.md` §2.3, §3 (item 6), §4.1 — composition substrate.
- `Owner Inbox/2026-05-09_mira-zara_concentration-risk-conduct-confirmation.md` §3.1 — JS-1-2024 finding that drives the priority-1 sequence.
- Banks Act 94 of 1990; Joint Standard 1 of 2024; BCBS Corporate Governance Principles for Banks (2015); SARB Regs Relating to Banks Reg. 39.
- `CLAUDE.md` — Principle 7 (autonomous by default); Operating model (statutory humans kept to the minimum the law requires).
- `Team/Helena.md` — engineering CRO mandate; reporting interlock with the human CRO seat.

—Nolan
