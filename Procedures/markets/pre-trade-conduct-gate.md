...
|---|---|---|---|---|
| 1 | **Check 1 — Counterparty mandate:** Query the approved-counterparty registry (built by PROC-MK-CIL-01) to confirm: (a) counterparty is on the approved list; (b) counterparty is approved for FX spot; (c) counterparty's credit limit is not exhausted; (d) ISDA Master Agreement is in place and current | `agent` | `@platform/markets/counterparty-registry` (PLANNED) | If any sub-check fails: emit `ConductGateBlocked { intentId, reason: 'CounterpartyNotApproved' | 'CreditLimitExhausted' | 'IsdaNotInPlace', blockedAt }`. Stop. |
