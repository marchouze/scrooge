// scripts/render-ba-610.ts
//
// CLI wrapper for the BA 610 (Income Statement) generator + renderer.
//
// Usage:
//   bun run scripts/render-ba-610.ts \
...
function parseArgs(argv: readonly string[]): CliArgs {
  ...
  if (!entity || !periodId || !periodStart || !periodEnd) {
    throw new Error(
      "render-ba-610: --entity, --period-id, --period-start, --period-end are required.",
    );
  }
...
function loadJson<T>(path: string, label: string): T {
  ...
  if (parsed === null || parsed === undefined) {
    throw new Error(`render-ba-610: --${label} file at ${path} parsed to null/undefined`);
  }
  return parsed as T;
}
