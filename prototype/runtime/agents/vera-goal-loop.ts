...
// ---------------------------------------------------------------------------
const VERA_BRIEF_DISPATCH: GoalLoopBriefDispatchConfig = {
  agentSlug: "vera",
  selfExecutablePattern: /overnight[\s-]?recon/i,
  deliveredClassLabel: "overnight-recon attestation",
  runHandler: veraOvernightRecon,
};
...
const handler = async (ctx: AgentRunContext): Promise<AgentRunOutput> => {
  ...
  const openBriefs = shouldRunHandler && !ctx.dryRun ? openBriefsListForAgent("vera") : [];
  ...
  if (brief) {
    ...
    logger.info(
      ...
      "vera:goal-loop — run complete (brief-bound dispatch)",
    );
    return {
