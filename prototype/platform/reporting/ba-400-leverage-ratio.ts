...
// risk-based capital ratios computed by `ba-100-capital.ts`. It catches
...
//   compliant by convention — same as the BA 100 divide-by-zero case).
//
...
// `ba-100-capital.ts` (RWA inputs caller-supplied at v0 — see C-3
...
// is supplied by the BA-100 generator caller.
//
// Authority:
 ...
 * reproducibility.
 *
 * `placeholders` mirrors the BA 100 generator's convention: any caller-
...
// ---------------------------------------------------------------------------
// Per-entity scope guard — same set as BA 100 (bank-licence-bound)
// ---------------------------------------------------------------------------
...
function assertBankEntity(entity: string): void {
  if (!LEVERAGE_RATIO_BANK_ENTITIES.includes(entity)) {
    throw new LeverageRatioGeneratorError(
      `Leverage ratio is bank-licence-bound; entity '${entity}' is not in LEVERAGE_RATIO_BANK_ENTITIES (${LEVERAGE_RATIO_BANK_ENTITIES.join(", ")}). See Regulations/_legal-entity-tree.md + D-REGULATORY-PERIMETER. Group-consolidated leverage lands alongside Slice 7 BA 100 consolidation.`,
    );
  }
}
