...
async function main(argv: readonly string[]): Promise<number> {
  ...
  try {
    ...
    console.log(
      `  LCR compliant: ${ba325Output.lcrCompliant ? "YES" : "NO"}`,
    );
    ...
    console.error(`  FAILED to generate or submit BA 325: ${err instanceof Error ? err.message : String(err)}`);
    anyFailed = true;
  }
  ...
  try {
    ...
    const fmtRatio = (r: number) =>
      Number.isFinite(r) ? (r * 100).toFixed(2) + "%" : "∞";
    console.log(`  CET1 ratio:    ${fmtRatio(ratios.cet1Ratio)} (min ${fmtRatio(ratios.cet1RatioRequiredMinimum)}) — ${ratios.cet1Compliant ? "PASS" : "BREACH"}`);
    console.log(`  Tier1 ratio:   ${fmtRatio(ratios.tier1Ratio)} (min ${fmtRatio(ratios.tier1RatioRequiredMinimum)}) — ${ratios.tier1Compliant ? "PASS" : "BREACH"}`);
    console.log(`  Total ratio:   ${fmtRatio(ratios.totalRatio)} (min ${fmtRatio(ratios.totalRatioRequiredMinimum)}) — ${ratios.totalCompliant ? "PASS" : "BREACH"}`);
    ...
    console.error(`  FAILED to generate or submit BA 700: ${err instanceof Error ? err.message : String(err)}`);
    anyFailed = true;
  }
