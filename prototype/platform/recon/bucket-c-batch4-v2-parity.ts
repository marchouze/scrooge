...
//
// recon:bucket-c-batch4-v2-parity — Bucket C bulk batch-4 parity gate
// (D-BANK-WIDE-V2-MIGRATION).
//
// PURPOSE
// -------
// Proves byte-equivalence of the 4 (FINAL) non-load-bearing bucket-C substrate
// types — the reference-data-process boundary set: SanctionsListPublished,
// PepListPublished, AdverseMediaPublished, TaxClassificationPublished (sourced
// from BUCKET_C_BATCH4_TYPES) — between:
...
// recon:bucket-c-batch3-v2-parity (#1410); identical comparable shape + harness.
//
...
// the money-free batches, and bucket-C batches 1 + 2 + 3.
//
// STATUS
// ------
// ENFORCING (bucket-C batch-4 flip, 2026-06-17). All 4 types flipped
...
// where real reference-data populations live). Because the codec is verbatim
// (identity), the empty-store flip is safe — there is no codec to get wrong on
// real data; the wiring is what CI proves, the byte-equivalence is proven the
// moment data exists. Per scope §5.1 the empty-population flips are recorded as
// ONE batch-level tracked gap (Vera re-asserts at first real append).
//
...
import { BUCKET_C_BATCH4_TYPES } from "../../v2-core/bucket-c-batch4";
...
const PIPELINE = "bucket-c-batch4-v2-parity";

const TYPE_SET: ReadonlySet<string> = new Set<string>(BUCKET_C_BATCH4_TYPES);
...
function readV1Tuples(): readonly VerbatimTuple[] {
  ...
  for (const type of BUCKET_C_BATCH4_TYPES) {
...
export function run(overridePath?: string): ReconResult {
  ...
  const vacuous = runParityCheck<readonly VerbatimTuple[]>({
    label: "bucket-c-batch4-v2-parity:structural-check",
    readV1: () => [],
    readV2: () => [],
  });
  if (vacuous.length > 0) {
    violations.push({
      subject: "bucket-c-batch4-v2-parity:harness-structural-check",
      message:
  ...
  try {
    ...
    const parity = runParityCheck<readonly VerbatimTuple[]>({
      label: "bucket-c-batch4-verbatim-tuples",
      readV1: () => v1Snap,
      readV2: () => v2Snap,
    });
    ...
    violations.push({
      subject: "bucket-c-batch4-v2-parity:fold-error",
      message: `bucket-c batch-4 verbatim tuple fold threw: ${err instanceof Error ? err.message : String(err)}. Inspect the V1 store and the v2 control-plane store for malformed events.`,
      severity: "fail",
    });
  }
  ...
  result.asOf =
    `bucket-c-batch4-v2-parity [ENFORCING]: ${BUCKET_C_BATCH4_TYPES.length} type(s); ` +
