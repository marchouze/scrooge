...
| 5 | **Event emission.** Emit one `ALMRunCompleted { runId: "ALM-RUN-<date>", … }` summarising the run, then one `IRRBBChecked` per metric/shock combination (6 EVE + 4 NII = 10), each carrying `breach` flags and the standing citations (`BANKS-ACT-94-1990`, `BANKS-REG-26`, `BANKS-REG-27`, `BCBS-D368-IRRBB`). | `system` | `@platform/event-store` (`event-types/alm.ts` — `makeALMRunCompleted`, `makeIRRBBChecked`) | Events are the canonical record (Principle 1); the markdown deliverable in step 6 is a render. |
