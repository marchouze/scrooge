...
// classifyUnmarkable is the only V1 daily-pnl export still consumed: it is a
// pure helper (FinancialInput classification of unmarkable positions) shared by
// both engines. The V1 compute/runner (computeDailyPnL / runDailyPnLReport) are
// no longer the serving path — the FX daily-P&L route is V2-authoritative
// (WS-FX-OTC-CLOSURE A2; D-FX-OTC-CLOSURE-BACKLOG). They remain exported from
// the module for historical replay but are not imported here.
import { classifyUnmarkable } from "../platform/product-control/daily-pnl";
import { computeDailyPnLV2, runDailyPnLReportV2 } from "../platform/product-control/daily-pnl-v2";
...
function bootDerive(): DashboardState {
  try {
    ...
    //
    // V2-AUTHORITATIVE CUTOVER (WS-FX-OTC-CLOSURE A2; D-FX-OTC-CLOSURE-BACKLOG).
    // The boot-derive report is now generated by the V2 runner so the persisted
    // `latestReport` the route surfaces (read at /api/product-control/daily-pnl)
    // is coherent with the V2 serving path. The V1 runner (runDailyPnLReport)
    // folds `FxPositionRevalued`/`FxTradeExecuted`, both retired-by-construction
    // (un-emittable on main), so it would persist a vacuous 0-position report
    // that then overrode the real V2 figure via `latestReport ?? freshPayload`.
    // Authority: D-FX-SALES-TRADING-FRONTEND; D-FIL-BOOK-COMPOSITE-VALUATION;
    //   D-BANK-WIDE-V2-MIGRATION; IFRS 9 §5.7.1.
    try {
      runDailyPnLReportV2(eventStore, marketDataStore, nowUtc);
    } catch (pnlErr) {
...
const server = Bun.serve({
  ...
  async fetch(req) {
    ...
    if (url.pathname === "/api/product-control/daily-pnl" && req.method === "GET") {
      ...
      //
      // V2-AUTHORITATIVE CUTOVER (WS-FX-OTC-CLOSURE A2; D-FX-OTC-CLOSURE-BACKLOG,
      // CEO-approved 2026-06-19). The FX daily-P&L serving path now reads the V2
      // engine UNCONDITIONALLY (the `useV2Store` flag no longer gates THIS route).
      //
      // Why this is the honest cutover, not a flag flip:
      //   - The V1 source events `FxPositionRevalued` (per-trade EOD delta) and
      //     `DailyPnLReportGenerated` are flipped to v2Status "v2-replaced"
      //     RETIRED-BY-CONSTRUCTION (D-V1-REMOVAL-FLIP-BASIS-RBC): their schemas
      //     carry required numeric `*Minor` fields, so they are UN-EMITTABLE on
      //     main (blocked by recon:no-residual-minor-encoding, no allowlist).
      //   - Consequently `computeDailyPnL` (V1) is structurally dead — it folds
      //     `FxPositionRevalued`/`FxTradeExecuted`, both of which can no longer
      //     exist, so it returns a vacuous 0-position report on any clean store.
      //   - `computeDailyPnLV2` reads the FIL-instance projection
      //     (FilInstrumentCreated/Terminated FX) + the MarketDataStore snapshot —
      //     the sole live FX valuation path (D-FIL-BOOK-COMPOSITE-VALUATION). It
      //     yields the real figures. Serving V1 here would mean serving an empty
      //     report while the registry declares V2 authoritative — the exact
      //     registry↔route incoherence this A2 cutover closes.
      //   - V2 returns the IDENTICAL DailyPnLResult shape, so the downstream
      //     classification + response shape are unchanged.
      // The V1 engine (`computeDailyPnL`) remains importable for historical replay
      // of any pre-purge `FxPositionRevalued` events; it is simply no longer the
      // serving path for the fresh on-demand report.
      const {
