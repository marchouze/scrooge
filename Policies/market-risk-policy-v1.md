...
- **PA regulatory returns (`ORG-PR-60`, `ORG-PR-RETURNS-011/012/013`):** Camille files the market-risk BA-return suite as part of the SARB reporting calendar. Under the Excel-canonical SARB form schedule (`D-BA-RETURN-NUMBERING-EXCEL-CANONICAL`), the market-risk returns this policy closes are: **BA 320** — market risk (standardised position-risk method per D5/2025 §2.1.12, the bank's primary market-risk capital return; `ORG-PR-RETURNS-011`) and its alternative / simplified small-trading-book treatment (D5/2025 §2.1.13; `ORG-PR-RETURNS-012`); and **BA 325** — Selected Risk Exposure (the FRTB / sensitivity-based-method return per D5/2025 §2.1.14; `ORG-PR-RETURNS-013`). The SA capital (or IMA capital once a PA-approved Internal Models Approach is in force, with the §4.6 output floor applied) flows into these returns and onward into the BA 700 capital-adequacy summary. Each BA-return filing is a `RegulatoryReturnFiled { returnType, period, amount }` event. The reporting-substrate module filenames and any residual stale form-number references are reconciled to this Excel-canonical mapping under the reporting-code track of `D-BA-RETURN-NUMBERING-EXCEL-CANONICAL` (Mira (Compliance / RegTech engineer, engineering); Bea (Accounting & financial reporting engineer, engineering)) — code-side scope, tracked separately from this policy-layer closure.
...
| `ORG-PR-RETURNS-011` | Submit form BA 320 (standardised market-risk return) to PA per D5/2025 §2.1.12 + Reg 28 | **IN FORCE** (COMMENCEMENT-BIND) — closed | §6.2 (PA regulatory returns); §4 (FRTB Capital Framework feeds the return) |
| `ORG-PR-RETURNS-012` | Submit form BA 320 (alternative / simplified market-risk return) to PA per D5/2025 §2.1.13 | **IN FORCE** (COMMENCEMENT-BIND) — closed | §6.2 (PA regulatory returns); §4.1 (SA computation, proportional small-trading-book treatment) |
| `ORG-PR-RETURNS-013` | Submit form BA 325 (Selected Risk Exposure / FRTB market-risk return) to PA per D5/2025 §2.1.14 | **IN FORCE** (COMMENCEMENT-BIND) — closed | §6.2 (PA regulatory returns); §4.1 (FRTB SA capital), §4.6 (SA/IMA dual-reporting) |
