// v2-core/core/money-wire.ts
//
// The canonical v2-core control-plane MoneyWire primitive.
//
// RELOCATED (Bucket C prep, D-BANK-WIDE-V2-MIGRATION): this module formerly
// lived at `v2-core/bucket-a-a2/money-wire.ts`. It is no longer A2-specific —
// `moneyWireSchema` / `moneyWireFromMajorString` are the REUSABLE control-plane
// money primitive used by `V2RiskAppetiteSet.floor` (#1404, a non-A2 bucket-A
// type) as well as the batch-A2 store-tee codecs and Bucket C. It now sits in
// the canonical `v2-core/core/` shared-primitive layer. A thin re-export shim
// is retained at the old `v2-core/bucket-a-a2/money-wire.ts` path for one cycle
// so existing imports keep working without a big-bang churn.
//
// MoneyWire builders for the v2-core store-tee codecs and control-plane events.
//
