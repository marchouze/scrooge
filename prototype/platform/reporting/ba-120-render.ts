...
// BA 120 (Off-Balance-Sheet Activities) projection.
//
...
//
// The render layer takes the typed `Ba120OffBalanceSheet` from
// `ba-120-off-balance-sheet.ts` and produces a JSON document that:
//   - validates against the declared `Ba120RenderSchema` (Zod);
...
//
//   BA 120 PROJECTION (typed Ba120OffBalanceSheet)
...
//
// Determinism mirror of `ba-700-render.ts` and `ba-325-render.ts`: keys
...
import type { Ba120OffBalanceSheet } from "./ba-120-off-balance-sheet";
...
/**
 * Canonical JSON schema for a rendered BA 120.
...
export const Ba120RenderSchema = z.object({
  $schema: z.literal("https://hoz.bank/schemas/ba-120/v0.1-rehearsal.json"),
  meta: z.object({
...
export const BA_120_SCHEMA_URL = "https://hoz.bank/schemas/ba-120/v0.1-rehearsal.json";
...
/**
 * Render the BA 120 projection to a typed JSON object validated against
 * `Ba120RenderSchema`. Pure function; deterministic for fixed `renderedAt`.
 */
export function renderBa120ToJson(
  output: Ba120OffBalanceSheet,
 ...
 * is over these bytes.
 *
 * Mirrors the canonicaliser in `ba-700-render.ts`.
 */
