...
| `ORG-CD-05` (FIC Act s.28 — STR obligation) | If a market abuse alert constitutes a suspicious activity that may involve proceeds of crime or terrorist financing, the bank must file a Suspicious Transaction Report with the FIC; MLRO (Rashida) leads STR filings. |
...
| 11 | **FIC STR (if `FicStrRequired`).** Zara briefs Rashida (MLRO, governance); Rashida determines whether a Suspicious Transaction Report is required under FIC Act s.28; if required, Rashida files the STR within the FIC-mandated window | `human` (Rashida — MLRO, governance, lead) + `human` (Zara — supporting evidence) | `@regulatory/fic-goaml` (PLANNED) | STR filing is an MLRO obligation; it runs in parallel with FSCA referral. The STR is confidential; tipping-off prohibition applies. |
...
| Rashida (MLRO, governance) | STR determination and filing under FIC Act s.28; tipping-off compliance |
...
| STR filing overdue | Rashida + Zara + CEO | Per FIC Act deadline |
