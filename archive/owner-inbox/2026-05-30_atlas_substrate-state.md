...
**Headline:** 120014 events across 172 types; 31/31 personas have operating specs; 128 runtime handlers registered; 508 files in /Owner Inbox/; 7 substrate gaps tracked.

## Event store

Path: `/Users/marc/.local/share/bank/event.db` · Total events: 120014

| Event type | Count | Earliest | Latest |
|---|---|---|---|
| `ProvenanceReclassified` | 44284 | 2026-05-27 | 2026-05-29 |
| `InboundMessageReceived` | 13423 | 2026-05-25 | 2026-05-28 |
| `SubstrateAgentRunStarted` | 5534 | 2026-05-25 | 2026-05-30 |
| `BusDispatched` | 5440 | 2026-05-25 | 2026-05-29 |
| `SubstrateAgentRunCompleted` | 5372 | 2026-05-25 | 2026-05-29 |
...
| `WorkstreamRegistered` | 852 | 2026-05-07 | 2026-05-29 |
...
| `DashboardProjectionRefreshed` | 622 | 2026-05-25 | 2026-05-29 |
...
| `ScheduledTrigger` | 540 | 2026-05-25 | 2026-05-30 |
...
| `AgentGoalEvaluated` | 444 | 2026-05-25 | 2026-05-30 |
...
| `SubstrateStateSnapshot` | 106 | 2026-05-07 | 2026-05-29 |
...
| `AgentGoalSelected` | 87 | 2026-05-25 | 2026-05-30 |
...
The substrate is broadly operational and now mostly closes on itself: 172 event types in store, 120,014 events, 31/31 personas specced, 128 runtime handlers registered across 28 agents, and the four event-driven trigger paths (scheduled / event-driven / on-request / readiness) all first-class. What's closing is the projection layer — Anya's `projection-refresh` is live, the curated dashboard seed is out of the commit graph, and Atlas is now emitting one typed `RiskRaised` per substrate gap on the weekly run. What's blocking remains narrow but real: cross-process event-bus dispatch is still M8 work, and several typed payload schemas (`AgentDecision`, `WorkstreamRegistered`, `AgentEscalation`) exist in `event-types.ts` but are not yet consistently constructed at the handler edges.

Three consequential changes are visible in this snapshot. First, `ScheduledTrigger` is now present in the store at 540 events (latest 2026-05-30) — the A2.1 substrate scheduler is emitting, which is the load-bearing step toward retiring the cron shims. Second, the agent-ops loop has become event-shaped: `AgentGoalEvaluated` (444), `AgentGoalSelected` (87), `AgentGoalDeferred` (357), `AgentPerformanceEvaluated` (91), `AgentFeedbackIssued` (91), and `AgentPromptOptimizationApplied` (13) are all now flowing, meaning Sade's fleet-optimisation and Atlas's goal-loop have substrate to reason from rather than narrative-only. Third, the readiness mesh closed out: ten distinct `*ReadinessSnapshot` / domain-snapshot types (ALM, Payments, Legal, Tax, Markets, Cyber, POPIA, OperationalResilience, FinancialPosition, AuditCommitteePack) each appeared on 2026-05-29, which is the first run where Thandiwe's audit-committee-prep has a complete cross-domain input set.

Ranked by downstream load-bearing: `AgentEscalation` at 77 events is now non-trivial and `AgentEscalationDecided` is appearing (13), so Vera's audit pipelines #14/#15 have actual substrate to bind against — that gap is no longer the blocker it was, but the typed payload schema needs to be the constructor at every emit site before Vera can rely on shape. `WorkstreamRegistered` (852) and `WorkstreamCompleted` (7) are the more asymmetric pair: registration is flowing freely, completion is barely emitted, which means the dashboard's workstream-lifecycle view will skew toward "open forever" until completion paths are wired in handlers that currently terminate silently. The Neon hardening conditions (§5.1 role downgrade, §5.2 IP allowlist) under exception `TM-NEON-EVENT-STORE-001` remain deferred and are load-bearing on any sensitive-event flow — they must close before KYC/POPIA payloads move off local sqlite into the shared store. Cross-process event-bus dispatch remains M8 and gates any handler chain that crosses workflow boundaries.

Next: wire `WorkstreamCompleted` emission into the handlers that already emit `WorkstreamRegistered`, so the lifecycle closes end-to-end before M8.
