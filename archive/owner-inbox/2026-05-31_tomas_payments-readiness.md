...
asOf: 2026-05-31T04:21:49.262Z
decision-required: false
---

# Tomas — daily payments-readiness, 2026-05-31
...
Latest `OperationalResilienceSnapshot` event: 2026-05-29T10:20:54.326Z
...
Substrate is roughly two-thirds specified, one-third drafting, with zero live scheme connectivity and zero payments events in the last seven days — we are squarely in build phase, and the dominant signal is the queue of substrate tickets between today and the first synthetic SAMOS rehearsal under Saskia's pre-licence go-live readiness gate. The load-bearing block on Devon's first end-to-end resilience rehearsal is `capability:samos-rtl-gateway` — still at *specified*, not yet *drafting* — because every downstream leg (cut-off engine windows, three-way recon, Bea's first cash-leg posting) hangs off a synthetic pacs.009 / pacs.002 round-trip we have not yet stood up. The indirect-participant model under NPS Act 78 of 1998 collapses what would otherwise be a direct-SAMOS membership programme into a thin sponsor-bank adapter seam; the binding artefact is the sponsor contract, not SARB direct-participant onboarding, and the same collapse applies to BankservAfrica (PASA settlement standards, scheme rulebooks) and CLS (deferred until first FX position).

Three observations rank above the rest. First, `capability:iso-20022-pacs-camt` is one ticket from green — pinning the envelope library to the current scheme-published pacs / camt versions unblocks both the SAMOS gateway draft and the reconciler's camt.053 / camt.054 source-of-truth; this is the cheapest move on the board. Second, `obligation:swift:csp` (SWIFT CSP 2024 control framework) has no owning capability yet because the attestation cycle only activates at first BIC — but the obligation is registered as *planned* with no engineering substrate behind it, and co-ownership with Senna needs to be pinned before the BIC application lodges, not after. Third, the three-way recon (trade-leg ↔ payment-leg ↔ ledger-leg) is *specified* but uncontracted: Bea cannot post her first cash leg until the recon contract names which camt statement closes which payment event against which Kai trade event, and that contract gates the first synthetic SAMOS rehearsal end-to-end.

Next engineering move, in order: (1) draft `@platform/payments/samos-connector` as a synthetic SAMOS gateway emitting pacs.009 and consuming pacs.002, wired to the cut-off engine's 06:00 / 16:00 SAST windows per current SARB NPSD circular, tolerance ± 15 min; (2) commission the three-way reconciler against synthetic flows, camt.053 as end-of-day source-of-truth, co-signed with Bea and Anya before first rehearsal; (3) ingest the current BankservAfrica scheme rulebook revision and pin ISO 20022 envelope versions to the scheme-published cycle, so the synthetic generators don't drift off scheme cadence. Strate connector specification stays paired to Kai's reportable-trades feed against the 1 March 2027 Joint Notice 2 of 2024 cutover; CLS remains *n/a-yet* until Saskia / Eitan activate a first FX position. Banks Act 94 of 1990 prudential perimeter is unchanged by any of the above — these are all build-phase substrate moves under Saskia's pre-licence readiness gate, not live-scheme commitments.
