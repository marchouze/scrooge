...
asOf: 2026-05-23T06:00:58.595Z
decision-required: false
---

# Bea — daily accounting readiness, 2026-05-23
...
Substrate is at zero — no postings, no classifications, no sub-ledger entries in the last seven days, and Camille's snapshot shows `never` across every CFO-line readiness flag. The load-bearing block on her first signed `CloseApproved` is the **monthly close cycle**, and within that, the missing primitive is the sub-ledger projection over postable events. Until that projection exists, nothing downstream — BA 100/200/300/700 cell-mapping (Banks Act 94 of 1990 Reg 38), IAS 1 statutory line-items, or the IFRS 9 ECL staging engine — can fire end-to-end, because none of them have a posted entry to read.

The most consequential observation is that the monthly close is genuinely one engineering ticket away from a green dry-run: chart-of-accounts schema + one published posting rule + a `CloseApproved` producer wired to Camille's snapshot. All 26 Bea-owned obligations sit at PLANNED (14) or DRAFTING (2) with zero IN FORCE and zero PARTIAL — meaning the policy backlog is not yet the constraint; the engine is. The BA-return cycle is a step behind it (waiting on the sub-ledger to read from), AFS is correctly deferred behind the first close per IAS 1, and the IFRS 9 ECL staging cycle is rightly parked until Saskia books a first counterparty exposure — methodology drafted with Rohan in parallel, but no substrate fires before there is an EAD to stage.

Next engineering move, in order: (1) specify the chart-of-accounts schema and commit it; (2) publish the first posting rule jointly with Atlas — synthetic `FundingDrawn` → debit cash / credit funding-liability, classified per IFRS 9 amortised-cost and presented per IAS 1, with the IAS 7 cash-flow tag set at posting time so the cash-flow statement is a projection, not a reconstruction; (3) stand up the sub-ledger projection over postable events and wire the `CloseApproved` producer into Camille's `FinancialPositionSnapshot`; (4) in parallel, commission with Anya the BA 100 cell-map register against the synthetic capital line per Reg 38, so the first `BAReturnGenerated` dry-run can fire the moment the sub-ledger lands. Deferred-tax wiring with Yael under IAS 12 / Income Tax Act 58 of 1962 stays drafted but not built until there is a taxable temporary difference to recognise — i.e., until a posting exists.
