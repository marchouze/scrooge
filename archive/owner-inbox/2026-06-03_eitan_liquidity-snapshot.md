...
asOf: 2026-06-03T07:13:21.575Z
decision-required: false
---

# Eitan — liquidity snapshot, 2026-06-03
...
Headline: 39 liquidity-adjacent obligations indexed across BA 110 / 120 / 125 / 130 / 325 / 326 / 330 / 340 / 600 surfaces, with the LCR (D1/2022), NSFR (D1/2023) and IRRBB Pillar 3 (D1/2024) families fully enumerated and owner-mapped; zero are in declared PARTIAL state, but every one of them is materially `[TBD]` at the procedure layer — none of `Procedures/by-policy/lcr-nsfr-liquidity-stress.md`, the IRRBB measurement procedure, or the Pillar 3 IRRBB disclosure procedure exists yet. Over the last 24h the daily snapshot recorded 10 `IRRBBChecked` events and three prior `LiquiditySnapshot` heartbeats this week; `HQLAReported`, `LiquidityReport`, `LCRRatioProjection`, `NSFRRatioProjection`, `FXPositionReported` and `NostroFundingApproved` are all zero. That is consistent with build-phase: the snapshot heartbeat is functioning as the § 6 inactivity-SLA stand-in for the daily-funding-event surface, and the IRRBB cadence is the only live signal.

Three observations rank. First, ORG-PR-LCR-001 (HQLA maintenance ≥ 100%), ORG-PR-LCR-006 (75% inflow cap), ORG-PR-LCR-008 (monthly LCR) and ORG-PR-NSFR-001 (NSFR formula) are the load-bearing ratios and they have no projection schema behind them yet — degraded-mode is covering a control surface where, post-engine, a zero-day on `LCRRatioProjection` or `NSFRRatioProjection` would be a § 6 breach, not a quiet day. Second, ORG-PR-LCR-010 (non-compliance reporting to PA) and ORG-PR-NSFR-008 (supervisory approval for interdependent assets) both presuppose a breach/escalation pathway through Iris and Zara that the current substrate does not name as an event type — there is no `LCRBreachNotified` or `SupervisoryApprovalRequested` projection. Third, ORG-PR-RETURNS-015 / ORG-PR-P3-001 (BA 340 + Pillar 3 IRRBB, D1/2024) are the next ALCO-chair item: 10 `IRRBBChecked` events in 24h is a healthy pulse, but with no `IRRBBExcursion` or Δ-EVE / Δ-NII projection schema, I cannot tell Helena whether any of those 10 sit inside or outside her appetite — the events are observability without interpretation.

Next concrete step, in order: (1) land the `LCRRatioProjection` and `NSFRRatioProjection` schemas with Ravi this week — fields for HQLA stock, net outflows, inflow-cap-bound numerator, ASF/RSF totals, currency split (ORG-PR-LCR-007), and consolidated-vs-solo flag (ORG-PR-LCR-003/-004/-005) — so degraded-mode can be retired surface-by-surface rather than wholesale; (2) ask Anya to add `IRRBBExcursion` (Δ-EVE / Δ-NII against Helena's appetite bands) as the interpretive layer above `IRRBBChecked`, which is the prerequisite for ORG-PR-P3-001 disclosure substance; (3) stand up `Procedures/by-policy/lcr-nsfr-liquidity-stress.md` as the procedure of record before the next ALCO, since it is the named gating procedure on ORG-PR-36, ORG-PR-38, ORG-PR-43 and the four NSFR computation obligations. ALCO prep item I am carrying into the next pack: a one-page register-to-projection crosswalk showing which of the 39 obligations have a live event-type behind them and which are still heartbeat-only — that is the honest read of where the build phase sits.
