...
function renderTable(rows) {
  ...
  if (!rows.length) {
    body.innerHTML = `<tr><td colspan="9" class="muted" style="padding:18px;text-align:center;">No obligations match the current filter.</td></tr>`;
    ...
    .map((o) => {
      ...
      const linkedHtml =
        (o.linkedPolicies ?? []).length === 0
          ...
          : (o.linkedPolicies ?? [])
              .map((p) => `<a href="policies.html?policy=${encodeURIComponent(p)}" class="oblig-tag oblig-policy-link">${esc(p)}</a>`)
              .join(" ");
      const chainHtml =
        (o.gaps ?? []).length === 0
          ? '<span class="oblig-tag" style="background:#e8f5e9;color:#2e6b34;border-color:#bce0c0;">COMPLETE</span>'
      ...
      <tr class="${gapClass}">
        ...
        <td>${linkedHtml}</td>
        <td><span class="oblig-tag">${esc(o.family)}</span></td>
        <td>${o.bind ? `<span class="${bindClass(o.bind)}">${esc(o.bind)}</span>` : '<span class="muted">—</span>'}</td>
        <td>${o.source ? `<span class="${sourceClass(o.source)}">${esc(o.source)}</span>` : '<span class="muted">—</span>'}</td>
        <td>${esc(o.status)}</td>
        <td>${chainHtml}</td>
      </tr>`;
    })
...
function renderFindings() {
  ...
  summary.innerHTML = `
    <b>${findings.length}</b> obligation${findings.length === 1 ? "" : "s"} with at least one citation-chain gap ·
    <b>${noFul}</b> NO-FULFILMENT ·
    <b>${noOwn}</b> NO-OWNER ·
    <b>${noFam}</b> NO-FAMILY (citation didn't match any source-instrument rule — extend
    <code>FAMILY_RULES</code> in <code>obligations-view.ts</code>).
  `;
  const body = $("obligFindingsBody");
  if (!findings.length) {
    body.innerHTML = `<tr><td colspan="4" class="muted" style="padding:18px;text-align:center;">No findings — every obligation has a complete chain at the register layer.</td></tr>`;
    return;
  }
...
async function load() {
  ...
  try {
    ...
    renderSummary();
    renderHistograms();
    if ($("filterFamily").options.length <= 1) {
...
function wireFilters() {
  for (const id of ["filterFamily", "filterBind", "filterChain"]) {
