// tests/ba-700-stress.test.ts
//
// Stress-test coverage for the BA 700 (Capital Adequacy) SARB return projection.
// Closes pre-licence gate criterion G3.
//
// These tests exercise edge cases and boundary conditions in the BA 700
// generator and XML adapter that are not covered by the nominal-path tests
// in ba-700-xml-adapter.test.ts. Each test is an isolated pure-function call
...
//   ST-6:  ba700ToXmlPayload() round-trip — structural key check
//
...
describe("BA 700 stress — ST-1: CET1 ratio < 4.5% minimum", () => {
...
describe("BA 700 stress — ST-2: Leverage ratio < 3% floor", () => {
...
describe("BA 700 stress — ST-3: Zero RWA denominator", () => {
  ...
  it("ba700ToXmlPayload() serialises Infinity ratios as the string 'Infinity'", () => {
    const output = generateBa700Capital(
      baseInput({
        trialBalance: [
...
describe("BA 700 stress — ST-4: Zero capital", () => {
...
describe("BA 700 stress — ST-5: Buffer requirements — conservation buffer", () => {
...
describe("BA 700 stress — ST-6: ba700ToXmlPayload() round-trip", () => {
  it("formId = 'BA700'", () => {
    const output = generateBa700Capital(baseInput());
    const payload = ba700ToXmlPayload(output);
