...
const CONSTRUCTION_CARVE_OUT_FILES: ReadonlySet<string> = new Set([
  ...
  // BA-return cell-contract recon (D-BA-RETURN-DATA-CONTRACT) — read-only
  // replay of ProductApproved events to enumerate the approved-product set the
  // BA 100 cell-data-requirement contract sources from (assertion 3: every
  // sourced cell's product refs are real approved products). Gate is a no-op
  // on replay. Citation: D-BA-RETURN-DATA-CONTRACT, P4-SECURITY-DESIGNED-IN.
  "platform/recon/ba-return-cell-contract.ts",
  // NPA post-approval finding review recon (D-NPA-POST-APPROVAL-FINDING-REVIEW) —
  // read-only replay of ProductPostApprovalFinding + ProductDimensionRetrospectiveReview
  // events to assert every finding has a review within SLA. Gate is a no-op on
  ...
  // WS-V2-AUTHORITATIVE S7 — V2 capital GL engine test. Same build-phase fixture
  // pattern as the bond/money-market V2 GL tests above: raw EventStore(":memory:")
  // seeds the V1-primary CapitalContributionRecorded trigger, runs the engine,
  ...
  // BA-310 FX-NOP period-close handler unit test — co-located per-module test
  ...
  // BA 700 capital-adequacy period-close handler unit test (WS-RETURNS-
  // SUBMISSION-WIRING Wave B) — co-located per-module test convention. Raw
  ...
  // Obligation-applicability helper unit test (W8 Slice C) — co-located
  // per-module test convention. Raw EventStore(":memory:") is a build-phase
  // fixture for the posture-reader + candidate-context derivation + idempotency
  ...
  // V2 regulation-detail view-level test (reg-viewer markdown tables + outline)
  // — co-located per-module test convention. Raw EventStore(":memory:") is a
  // build-phase fixture for the read-side detail DTO (asserts verbatim blocks
