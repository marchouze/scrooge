...
- **Principle 2 — Every action traces to a source.** No procedure, control, posting, or report line without a structured citation; the obligations register holds typed, versioned references. → [`Principles/2-citation-discipline.md`](Principles/2-citation-discipline.md)
...
- **Principle 4 — Security designed in from the start.** Threat-modelling per design, zero-trust, least-privilege, defence-in-depth, secure SDLC; aligned with PA/FSCA Joint Standard 1 of 2024 + POPIA s.19–22. → [`Principles/4-security-designed-in.md`](Principles/4-security-designed-in.md)
...
- **Principle 6 — Single-graph discipline.** Every artefact sits in one citable bidirectional graph. Presentations derive downward (Data → Process → Standard → Policy → Presentation); capabilities justify upward (Regulation → Policy → Procedure → System Capability). No orphans. → [`Principles/6-single-graph-discipline.md`](Principles/6-single-graph-discipline.md)
- **Principle 7 — Autonomous by default; humans oversee the residual.** Every persona is a standing autonomous agent; default actor in every procedure step is an agent; human-in-the-loop steps carry P2 citations; escalations are first-class typed channels. → [`Principles/7-autonomous-by-default.md`](Principles/7-autonomous-by-default.md)
