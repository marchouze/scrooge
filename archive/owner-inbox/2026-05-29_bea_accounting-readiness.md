...
Substrate is at zero — no sub-ledger projection, no posting rules published, no chart-of-accounts schema, and consequently no `SubLedgerEntryPosted`, no `IFRSClassificationAssigned`, no `CloseApproved` ever fired. All 64 Bea-owned obligations sit at PLANNED or DRAFTING; none are IN FORCE because there is no posting engine for them to bind to. The load-bearing block on Camille's first signed close is the **monthly close cycle** — specifically the sub-ledger projection over postable events. Until that projection exists and emits its first `SubLedgerEntryPosted`, BA returns (Banks Act 94 of 1990 Reg 38), AFS (IAS 1), and the capital plan all remain substrate-blocked downstream; they cannot be built against an empty ledger.

The most consequential observation is that the monthly close is one engineering ticket away from a green path, not three: chart-of-accounts schema → first posting rule → close orchestrator → `CloseApproved` producer, in that order. The first posting rule — synthetic `FundingDrawn` → debit cash / credit funding-liability, classified per IFRS 9 as amortised-cost financial liability and presented per IAS 1 — is the unlock, because it is the minimum rule that turns a domain event into a double-entry posting and proves the projection end-to-end. Second-order: the BA 100 cell-map (Reg 38) can be drafted *in parallel* against the synthetic capital seed without waiting for the close to land, and the AFS line-item schema (IAS 1 structure, IFRS 7 disclosure shells) can be specified now so the generator is ready when the first close clears. The IFRS 9 ECL cycle and IAS 12 deferred tax remain correctly deferred — no exposure book, no temporary differences to recognise — but the methodology specs should stay drafted so they wire the day Saskia books a counterparty.

Next engineering move, concretely: (1) publish the chart-of-accounts schema this week, with cash, funding-liability, equity, and a P&L spine sufficient for IAS 1 presentation; (2) co-publish with Atlas the first posting rule — `FundingDrawn` → Dr Cash / Cr Funding-Liability at amortised cost per IFRS 9 §4.2.1 — as the canonical worked example for the rule registry; (3) commission Anya's BA-return projection harness with a BA 100 cell-map register pinned to Reg 38, dry-runnable against the synthetic capital seed; (4) stand up the sub-ledger projection over postable events and wire the `CloseApproved` producer so Camille's next `FinancialPositionSnapshot` can move `last CloseApproved` off `never`. Deferred-tax (IAS 12) and ECL (IFRS 9) stay parked behind the first exposure event; hedge accounting (IFRS 9 §6) stays parked behind Ravi's first designated hedge.
